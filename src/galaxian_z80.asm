...
The term "@ 0x" means "at memory address in hexadecimal". 
e.g. @ 0x1234 means "refer to memory address 1234" or "program code @ memory location 1234" 

The term "#0x" means "immediate value in hexadecimal". It's a habit I have kept from 6502 days.
e.g. #0x60 means "immediate value of 60 hex" (96 decimal)

If I don't prefix a number with 0x or #0x in my comments, treat the value as a decimal number.
...
SCRIPTS are documented below - see docs for SCRIPT_NUMBER (0x4005)
...
DIP_SWITCH_1_2_STATE                EQU 0x4000         | holds state of dip switches 1 & 2 in bits 0 & 1.
COIN_COUNT                          EQU 0x4001         | counts up to number of coins per credit as set by dip switches. When it reaches that value, resets to 0 
NUM_CREDITS                         EQU 0x4002         | number of credits
COIN_CONTROL                        EQU 0x4003         | is used to output to DRIVER|COIN CONTROL (see 0x1974)
UNPROCESSED_COINS                   EQU 0x4004         | bumps up when coin inserted. See 0x190B and 0x1931.
...
*
* The NMI interrupt handler uses SCRIPT_NUMBER (0x4005) to identify what script to run and, depending on the script, SCRIPT_STAGE (0x400A) to 
...
*
* see 0x00CA for the NMI script handler. 

SCRIPT_NUMBER                       EQU 0x4005         | 0-based index into pointer table beginning @ 0x00CE
IS_GAME_IN_PLAY                     EQU 0x4006         | If set to 1, game is in play with a human in control.
IS_GAME_OVER                        EQU 0x4007         | Set to 1 when GAME OVER message appears. TODO: Check if set any other place than GAME OVER 
TEMP_COUNTER_1                      EQU 0x4008         | temporary counter used for delays, such as waiting before transitioning to next stage of a script
TEMP_COUNTER_2                      EQU 0x4009         | temporary counter used for delays

SCRIPT_STAGE                        EQU 0x400A         | Identifies what stage of the script we are at.  
                                                      | 0-based index into script tables located @ 0x0164, 0x0400, 0x0540, 0x0785
TEMP_CHAR_RAM_PTR                   EQU 0x400B         | pointer to character RAM. Used by screen-related routines (e.g. power on colour test) to remember where to plot characters on next call.
                                                                                                           
CURRENT_PLAYER                      EQU 0x400D         | 0 = PLAYER ONE, 1 = PLAYER TWO
IS_TWO_PLAYER_GAME                  EQU 0x400E         | 0 = One player game, 1 = 2 player game 
IS_COCKTAIL                         EQU 0x400F         | 0 = upright, 1 = Cocktail 
PORT_STATE_6000                     EQU 0x4010         | copy of state for memory address 6000 (SW0)          
PORT_STATE_6800                     EQU 0x4011         | copy of state for memory address 6800 (SW1 & SOUND)
PORT_STATE_7000                     EQU 0x4012         | copy of state for memory address 7000 (DIPSW)

PREV_PORT_STATE_6000                EQU 0x4013         | holds the previous state of memory address 6000 (SW0)  
PREV_PORT_STATE_6800                EQU 0x4014         | holds the previous state of memory address 6800 (SW1 & SOUND)
PREV_PREV_PORT_STATE_6000           EQU 0x4015         | holds the previous, previous (!) state of memory address 6000 (SW0) 
PREV_PREV_PREV_STATE_6000           EQU 0x4016         | holds the previous, previous, previous state of memory address 6000 (SW0)

DISPLAY_IS_COCKTAIL_P2              EQU 0x4018         | set to 1 when in cocktail mode and it's player 2's turn, so the screen's upside down.
PUSH_START_BUTTON_COUNTER           EQU 0x4019         | On inserting credit or GAME OVER: if you have credit, how long to wait before PUSH START BUTTON appears.  
DIAGNOSTIC_MESSAGE_TYPE             EQU 0x401A         | Read by the NMI handler. Refer to code @1BCD for docs.  

RAND_NUMBER                         EQU 0x401E         | TENTATIVE NAME. Random number used in tests and in-game 
DIP_SWITCH_5_STATE                  EQU 0x401F         | holds cached state of dip switch 5 in bit 0
...
* the back buffer is copied to the hardware's OBJRAM by an LDIR operation - see 0x0079.
...
*
* From 0x4020 - 405f: column scroll and colour attributes. Maps directly to 0x5800 - 0x583F. 
*    Note: Even numbered addresses hold scroll offsets, odd numbered addresses colour attributes. 
* From 0x4060 - 407F: 8 entries of type INFLIGHT_ALIEN_SPRITE. Maps directly to 0x5840 - 0x585F.
* From 0x4080 - 409F: alien bullets and player bullet sprite state. Maps directly to 0x5860 - 0x587F. 

OBJRAM_BACK_BUF                     EQU 0x4020            
OBJRAM_BACK_BUF_SPRITES             EQU 0x4060 
...
OBJRAM_BACK_BUF_BULLETS             EQU 0x4080
    OBJRAM_BUF_PLAYER_BULLET_Y      EQU 0x409D
    OBJRAM_BUF_PLAYER_BULLET_X      EQU 0x409F
OBJRAM_BACK_BUF_END                 EQU 0x409F                        


PLAYER_ONE_SCORE                      EQU 0x40A2       | stored as 3 BCD bytes, 2 digits per byte: 0x40A2 = last 2 digits of score (tens), 0x40A3 = 3rd & 4th digits, 0x40A4 = 1st & 2nd
                                                      | e.g. a score of 123456 would be stored like so:
                                                      | 0x40A2: 56
                                                      | 0x40A3: 34
                                                      | 0x40A4: 12

PLAYER_TWO_SCORE                      EQU 0x40A5       | stored as 3 BCD bytes, 2 digits per byte: same format & order as PLAYER_ONE_SCORE
HI_SCORE                            EQU 0x40A8         | stored as 3 BCD bytes, 2 digits per byte: same format & order as PLAYER_ONE_SCORE

CAN_BLINK_1UP_2UP                   EQU 0x40AB         | When IS_GAME_IN_PLAY is set to 1, this flag is set to 1 to allow 1UP or 2UP to "blink". See @0x20A7
BONUS_GALIXIP_FOR                   EQU 0x40AC         | stored as BCD in 1 byte. e.g. 07 = bonus galixip for 7000, 20 = bonus galixip for 20000. 
PLAYER_ONE_AWARDED_EXTRA_LIFE         EQU 0x40AD       | Set to 1 if player one has been awarded an extra life. No more extra lives will be given. 
PLAYER_TWO_AWARDED_EXTRA_LIFE         EQU 0x40AE       | Set to 1 if player two has been awarded an extra life. No more extra lives will be given. 


IS_COLUMN_SCROLLING                 EQU 0x40B0         | Set to 1 if a column is being scrolled. For example when points are scrolled into view on the WE ARE THE GALAXIANS screen
COLUMN_SCROLL_ATTR_BACKBUF_PTR      EQU 0x40B1         | pointer to scroll attribute data to update in OBJRAM_BACK_BUF. 
COLUMN_SCROLL_NEXT_CHAR_PTR         EQU 0x40B3         | pointer to ordinal of next character to scroll on
COLUMN_SCROLL_CHAR_RAM_PTR          EQU 0x40B5         | pointer to character RAM where next character will be plotted. 


* Phil Murray (PhilMurr on UKVAC) gave me a heads up on this.  
*
* 0x40C0 to 0x40FF is reserved for a circular queue. The queue is comprised of byte pairs representing a command and parameter.
* NB: I term the byte pair a *Queue Entry* in the code @0x08f2 and 0x200A.
*
...
* 
* 0x40C0: command A
* 0x40C1: parameter for command A 
* 0x40C2: command B
* 0x40C3: parameter for command B
* 0x40C4: command C
* 0x40C5: parameter for command C
* ..and so on.
*
* See docs @ 0x08f2 for info about what commands are available, and how to add commands to the queue.
* See docs @ 0x200A for info about how commands are processed.
*


CIRC_CMD_QUEUE_PTR_LO           EQU 0x40A0             | low byte of a pointer to a (hopefully) vacant entry in the circular queue. See 0x08F2 
CIRC_CMD_QUEUE_PROC_LO          EQU 0x40A1             | low byte of a pointer to the next entry in the circular queue to be processed. See 0x200C
CIRC_CMD_QUEUE_START            EQU 0x40C0
CIRC_CMD_QUEUE_END              EQU 0x40FF
...
ALIEN_SWARM_FLAGS                   EQU 0x4100         | 128 bytes, occupying 0x4100 to 0x417F in RAM


* When it's player 1's turn, the packed swarm definition PLAYER_ONE_PACKED_SWARM_DEF is unpacked to ALIEN_SWARM_FLAGS and the fields comprising PLAYER_ONE_STATE (0x4190-4197) 
* are written to their counterparts in CURRENT_PLAYER_STATE (see docs @0x4218)                
PLAYER_ONE_PACKED_SWARM_DEF           EQU 0x4180       | Used to track state of the swarm for player one, e.g. so swarm can be restored after player two's turn is over 
PLAYER_ONE_STATE                      EQU 0x4190
PLAYER_ONE_DIFFICULTY_COUNTER_1       EQU 0x4190         
PLAYER_ONE_DIFFICULTY_COUNTER_2       EQU 0x4191
PLAYER_ONE_DIFFICULTY_EXTRA_VALUE     EQU 0x4192           
PLAYER_ONE_DIFFICULTY_BASE_VALUE      EQU 0x4193         
PLAYER_ONE_LEVEL                      EQU 0x4194
PLAYER_ONE_LIVES                      EQU 0x4195
PLAYER_ONE_FLAGSHIP_SURVIVOR_COUNT    EQU 0x4196         
PLAYER_ONE_LFO_FREQ_BITS              EQU 0x4197         


* When it's player 2's turn, the packed swarm definition PLAYER_TWO_PACKED_SWARM_DEF is unpacked to ALIEN_SWARM_FLAGS and the fields comprising PLAYER_TWO_STATE (0x41B0-41B7) 
* are written to their counterparts in CURRENT_PLAYER_STATE (see docs @0x4218)                
PLAYER_TWO_PACKED_SWARM_DEF           EQU 0x41A0       | Used to track state of the swarm for player two, e.g. so swarm can be restored after player one's turn is over
PLAYER_TWO_STATE                      EQU 0x41B0
PLAYER_TWO_DIFFICULTY_COUNTER_1       EQU 0x41B0         
PLAYER_TWO_DIFFICULTY_COUNTER_2       EQU 0x41B1
PLAYER_TWO_DIFFICULTY_EXTRA_VALUE     EQU 0x41B2           
PLAYER_TWO_DIFFICULTY_BASE_VALUE      EQU 0x41B3         
PLAYER_TWO_LEVEL                      EQU 0x41B4
PLAYER_TWO_LIVES                      EQU 0x41B5
PLAYER_TWO_FLAGSHIP_SURVIVOR_COUNT    EQU 0x41B6         
PLAYER_TWO_LFO_FREQ_BITS              EQU 0x41B7         


SOUND_VOL                           EQU 0x41C0         | Bit 0 and 1 are written to !SOUND Vol of F1 and !SOUND Vol of F2 respectively. See 0x1712
PITCH_SOUND_FX_BASE_FREQ            EQU 0x41C1         | used to write to !pitch  Sound Fx base frequency. See 0x171F
ENABLE_ALIEN_ATTACK_SOUND           EQU 0x41C2         | When set to 1, turns on alien attack noise, see 0x17D0
UNKNOWN_SOUND_41C3                  EQU 0x41C3          
UNKNOWN_SOUND_41C4                  EQU 0x41C4         | Seems to affect the pitch of the alien attack noise. 

PLAY_EXTRA_LIFE_SOUND               EQU 0x41C7         | when set to 1, play the sound of an extra life being awarded. See 0x184F
EXTRA_LIFE_SOUND_COUNTER            EQU 0x41C8            
PLAY_PLAYER_CREDIT_SOUND            EQU 0x41C9         | when set to 1, play the sound of player credits being added. See 0x1876
PLAYER_CREDIT_SOUND_COUNTER         EQU 0x41CA         | The higher the value, the longer the player credit sound plays.
                                    EQU 0x41CB          
PLAY_PLAYER_SHOOT_SOUND             EQU 0x41CC         | When set to 1, play the sound of the player's bullet. See 0x1723
IS_COMPLEX_SOUND_PLAYING            EQU 0x41CD         | When set to 1, a sequence of sounds, or a melody, is playing. 
PLAYER_SHOOT_SOUND_COUNTER          EQU 0x41CE         | The higher the value, the longer the player spaceship bullet sound plays.
                                    EQU 0x41CF 
RESET_SWARM_SOUND_TEMPO             EQU 0x41D0         | When set to 1, resets the tempo of the "swarm" sound to slow again. See 0x1898
PLAY_GAME_START_MELODY              EQU 0x41D1         | When set to 1, plays the game start tune. 
                                    EQU 0x41D2         | sound related
COMPLEX_SOUND_POINTER               EQU 0x41D3         | If music or complex sound effect is playing, this points to the current sound/musical note being played. See 0x1782
                                    EQU 0x41D5         | Used to set !Pitch Sound FX base frequency
DELAY_BEFORE_NEXT_SOUND             EQU 0x41D6         | counter. When counts to zero the next sound/musical note is played. See 0x177B
ALIEN_DEATH_SOUND                   EQU 0x41DF         | Tentative name. When set to 0x06: plays alien death sound. When set to 0x16, plays flagship death sound. See @0x1819
                                    EQU 0x41E8
...
HAVE_ALIENS_IN_ROW_FLAGS            EQU 0x41E8
NEVER_USED_ROW_1                    EQU 0x41E8
NEVER_USED_ROW_2                    EQU 0x41E9

HAVE_ALIENS_IN_6TH_ROW              EQU 0x41EA         | flag set to 1 if there are any aliens in the bottom row (blue aliens)
HAVE_ALIENS_IN_5TH_ROW              EQU 0x41EB         | flag set to 1 if there are any aliens in the 5th row (blue aliens)
HAVE_ALIENS_IN_4TH_ROW              EQU 0x41EC         | flag set to 1 if there are any aliens in the 4th row (blue aliens)
HAVE_ALIENS_IN_3RD_ROW              EQU 0x41ED         | flag set to 1 if there are any aliens in the 3rd row (purple aliens)
HAVE_ALIENS_IN_2ND_ROW              EQU 0x41EE         | flag set to 1 if there are any aliens in the 2nd row (red aliens)
HAVE_ALIENS_IN_TOP_ROW              EQU 0x41EF         | flag set to 1 if there are any aliens in the top row (flagships)
...
* 
* In a nutshell:
* 0x41F0..0x41F2: unused. Always set to 0.
* 0x41F3: set to 1 if any aliens are in the rightmost column of the swarm.
* 0x41F4: set to 1 if any aliens are in the 2nd rightmost column of the swarm.
* 0x41F5: set to 1 if any aliens are in the 3rd rightmost column of the swarm.
* ..
* 0x41FC: set to 1 if any aliens are in the leftmost column of the swarm.
* 0x41FD..0x41FF: unused. Always set to 0.
*
* The flags have three purposes: 
* 1: To halt the swarm when a bullet is getting too close (see 0x0936)
* 2: to calculate how far the swarm can scroll before it needs to change direction (see 0x093E)
* 3: to find aliens at the swarm edges to attack the player (see code from 0x137B onwards) 
*
*
...
*   BBBBBBBBBB 
*
* Press PAUSE in MAME and open the memory debugger at location 0x41F0.
...
* If you were to shoot the blue aliens in the rightmost column, you would see the first 01 (at memory address 0x41F3) turn into a 0, 
...
ALIEN_IN_COLUMN_FLAGS               EQU 0x41F0          
ALIEN_IN_COLUMN_FLAGS_END           EQU 0x41FF     


HAS_PLAYER_SPAWNED                  EQU 0x4200         | set to 1 when player has spawned. (Also set in attract mode) 
IS_PLAYER_DYING                     EQU 0x4201         | set to 1 when player is in the process of exploding horribly. See 0x1327
PLAYER_Y                            EQU 0x4202         | Player Y coordinate. Used to set scroll offsets for column containing ship characters. See 0x0865
 
IS_PLAYER_HIT                       EQU 0x4204         | When set to 1, player has been hit by a missile or collided with an alien.         
PLAYER_EXPLOSION_COUNTER            EQU 0x4205         | Only evaluated when IS_PLAYER_DYING is set to 1. Determines how long the player explosion animation lasts. 
                                                      | When it counts down to 0, explosion animation stops. See 0x132C
PLAYER_EXPLOSION_ANIM_FRAME         EQU 0x4206         | Set by 0x12FE 
HAS_PLAYER_BULLET_BEEN_FIRED        EQU 0x4208         | set 1 when the player has fired a bullet and the bullet is still onscreen. See 0x08BC
PLAYER_BULLET_X                     EQU 0x4209         | Current X coordinate of player bullet. 
PLAYER_BULLET_Y                     EQU 0x420A         | Current Y coordinate of player bullet. 
IS_PLAYER_BULLET_DONE               EQU 0x420B         | set 1 when player bullet goes as far as it can upscreen (see 0x08CD), or hits an alien (see 0x0B4F & 0x125B).

SWARM_DIRECTION                     EQU 0x420D         | Direction of swarm (really? |) )  0 = Moving left, 1 = moving right . See 0x0945              
SWARM_SCROLL_VALUE                  EQU 0x420E         | 16 bit value. Used to set the scroll values for the character columns containing the swarm.                            
SWARM_SCROLL_MAX_EXTENTS            EQU 0x4210         | Used to limit the scrolling of the swarm so no alien goes "off screen". See 0x09CE 

* INFLIGHT_ALIEN_SHOOT_EXACT_X and MINFLIGHT_ALIEN_SHOOT_RANGE_MUL are used to determine if an alien can shoot a bullet. See 0x0E54 for information.
INFLIGHT_ALIEN_SHOOT_RANGE_MUL      EQU 0x4213         | Range multiplier.   
INFLIGHT_ALIEN_SHOOT_EXACT_X        EQU 0x4214         | Exact X coordinate that calculated value must match for alien to shoot.

ALIENS_ATTACK_FROM_RIGHT_FLANK      EQU 0x4215         | Flag used to determine what side of swarm aliens break off from. (0=break from left, 1=break from right). See 0x136f and 0x1426 
                                    EQU 0x4217         | 

* 0x4218 - 0x421F holds important, albeit transient, state for the current player such as number of lives and difficulty level.
CURRENT_PLAYER_STATE                EQU 0x4218                    

* These 2 counters are used to gradually increase the DIFFICULTY_EXTRA_VALUE over time. See 0x14F3 for algorithm details.
DIFFICULTY_COUNTER_1                EQU 0x4218         | Counts down to zero. 
DIFFICULTY_COUNTER_2                EQU 0x4219         | Counts down to zero. When it reaches zero, DIFFICULTY_EXTRA_VALUE is incremented.

* These values determine how often aliens attack (see 0x1524 and 0x1583), and how many can attack at one time (see 0x1352). 
DIFFICULTY_EXTRA_VALUE              EQU 0x421A         | DIFFICULTY_EXTRA_VALUE is incremented during the level. Maximum value of 7. See 0x1509.  
DIFFICULTY_BASE_VALUE               EQU 0x421B         | DIFFICULTY_BASE_VALUE is incremented when you complete a level. Maximum value of 7. See 0x1656.

PLAYER_LEVEL                        EQU 0x421C         | Current player's level. Starts from 0. Add 1 to get true value. See 0x252C.
PLAYER_LIVES                        EQU 0x421D         | current player's lives
FLAGSHIP_SURVIVOR_COUNT             EQU 0x421E         | When starting a new level, how many surviving flagships can we bring over from the previous level? Maximum value 2.  See 0x166C
LFO_FREQ_BITS                       EQU 0x421F         | Value used to set !DRIVER Background lfo frequency ports (0-3) for the "swarm" noise

CURRENT_PLAYER_STATE_END            EQU 0x421F                

HAVE_NO_ALIENS_IN_SWARM             EQU 0x4220         | Set to 1 when 0x4100 - 0x417F are set to 0. Aliens are either all dead, or are in flight and out of the swarm. See 0x0A0F
HAVE_NO_BLUE_OR_PURPLE_ALIENS       EQU 0x4221         | When set to 1, all the blue and purple aliens have died, or are in flight. See 0x09FA and 0x1571  
LEVEL_COMPLETE                      EQU 0x4222         | When set to 1, the level is treated as complete. See @0x1621, 0x1637
NEXT_LEVEL_DELAY_COUNTER            EQU 0x4223         | After all aliens have fled or been killed, this counts down to give the player breathing space. When it hits 0, the next wave starts. See 0x1637
HAVE_AGGRESSIVE_ALIENS              EQU 0x4224         | when set to 1, inflight aliens will not return to swarm and keep attacking player until they die - or you die. See 0x16B8
HAVE_NO_INFLIGHT_OR_DYING_ALIENS    EQU 0x4225         | When set to 1, there are no aliens inflight, or dying. See 0x06BC
HAVE_NO_INFLIGHT_ALIENS             EQU 0x4226         | When set to 1, no aliens have broken off from the swarm to attack the player.
CAN_ALIEN_ATTACK                    EQU 0x4228         | When set to 1, a single alien should break off from the swarm to attack the player. See 0x1344.
CAN_FLAGSHIP_OR_RED_ALIENS_ATTACK   EQU 0x4229         | When set to 1, a flagship should attack the player, with an escort if possible. If no flagships alive, send red aliens.  See 0x140C.
FLAGSHIP_ESCORT_COUNT               EQU 0x422A         | Number of red aliens escorting the flagship. Max value of 2. See 0x0D58.
...
* No aliens will leave the swarm while 0x422B is set to 1 and 0x422C is non-zero. 
IS_FLAGSHIP_HIT                     EQU 0x422B         | Set to 1 when you've shot a flagship in flight. See 0x127C  
ALIENS_IN_SHOCK_COUNTER             EQU 0x422C         | When 0x422B is set to 1, this counter decrements. When it hits 0, 0x422B will be set to 0, meaning aliens can leave the swarm again.  
FLAGSHIP_SCORE_FACTOR               EQU 0x422D         | When you shoot a flagship, this is used to compute your score. Couldn't think of a better name! See 0x127C

ENABLE_FLAGSHIP_ATTACK_SECONDARY_COUNTER      EQU 0x422E         | when set to 1, FLAGSHIP_ATTACK_SECONDARY_COUNTER is allowed to decrement.             
FLAGSHIP_ATTACK_SECONDARY_COUNTER   EQU 0x422F         | Counts down to 0. When reaches zero, CAN_FLAGSHIP_OR_RED_ALIENS_ATTACK will be set to 1.

DISABLE_SWARM_ANIMATION             EQU 0x4238         | When set to 1, alien swarm won't animate. See 0x2067 for docs. 
ATTRACT_MODE_FAKE_CONTROLLER        EQU 0x423F         | used to simulate a players movements on the ATTRACT MODE screen. Contains bit values that map to SW0.
ATTRACT_MODE_SCROLL_ID              EQU 0x4241         | Identifies what points values are being scrolled in on attract mode. 1:Flagship. 2: Red Alien. 3: Purple alien. 4: Blue alien. 

* These 2 counters are used to determine when a flagship is permitted to attack.  See 0x156A.
FLAGSHIP_ATTACK_MASTER_COUNTER_1    EQU 0x4245          
FLAGSHIP_ATTACK_MASTER_COUNTER_2    EQU 0x4246          
...
* ALIEN_ATTACK_MASTER_COUNTER at 0x424A is the first element of the array. The secondary counters are stored in 0x424B to 0x425A. 
...
* If any of the secondary counters reach zero, an alien will attack the player. See 0x1532 for more info.
ALIEN_ATTACK_COUNTERS               EQU 0x424A  
ALIEN_ATTACK_MASTER_COUNTER         EQU 0x424A
ALIEN_ATTACK_SECONDARY_COUNTERS     EQU 0x425B         
ALIEN_ATTACK_SECONDARY_COUNTERS_END EQU 0x425A     


TIMING_VARIABLE                     EQU 0x425F         | Perpetually decremented by the NMI handler. Routines use this variable to determine when to execute.
                                                         

* ENEMY_BULLETS is an array of type ENEMY_BULLET. 
*
* The array occupies memory locations 0x4260 - 0x42A5| It is thus 70 bytes in size. 
...
struct ENEMY_BULLET
{
    ...
    BYTE YDelta|                                   // packed delta to add to YH *and* YL. Bit 7 = sign bit. Bits 0-6 = delta. See @0x0AA1.                                  
} - sizeof(ENEMY_BULLET) is 5 bytes

ENEMY_BULLETS                       EQU 0x4260
ENEMY_BULLETS_START                 EQU 0x4260                                                                        
ENEMY_BULLETS_END                   EQU 0x42A5
...
*
* The array occupies memory locations 0x42B0 - 0x43AF| It is thus 256 bytes in size. 
...
* explosion animation needs a free sprite to play. (See: 0x0B52 for an example of this)
*
...
* struct INFLIGHT_ALIEN
{
  ...
  2    BYTE StageOfLife                      // See 0x0CD6 for details. 
  3    BYTE X|                               // X coordinate
  4    BYTE Y|                               // Y coordinate. 
  5    BYTE AnimationFrame| 
  6    BYTE ArcClockwise                     // Set to 1 if the alien will rotate clockwise as it leaves the swarm or loops the loop. See 0x0D71 and 0x101F
  7    BYTE IndexInSwarm                     // index of alien within ALIEN_SWARM_FLAGS array
  8    BYTE ???                              // Unused
  9    BYTE PivotYValue                      // When alien is attacking, this value + 0x19 produces INFLIGHT_ALIEN.Y coordinate. See 0x0DF6
  A    BYTE ???                              // Unused
  B    BYTE ???                              // Unused
  C    BYTE ???                              // Unused 
  D    BYTE ???                              // Unused
  ...
  12   BYTE DeathAnimCode                    // when IsDying is set to 1, specifies the animation frame to display. See @0x0C9F
  13   BYTE ArcTableLsb                      // LSB of pointer into INFLIGHT_ALIEN_ARC_TABLE @0x1E00. See docs @0x0D71 and 0x1E00.
  14   BYTE ???                              // Unused  
  15   BYTE ???                              // Unused
  16   BYTE Colour
  17   BYTE SortieCount                      // Number of times the alien has reached the bottom of the screen then resumed attack on the player. Reset to 0 when rejoins swarm. See 0x0E9D.
  18   BYTE Speed                            // Value from 0..3. The higher the number the faster the alien moves. See 0x116B. 
  19   BYTE PivotYValueAdd                   // Signed number which is added to INFLIGHT_ALIEN.PivotYValue to produce INFLIGHT_ALIEN.Y. See 0x0DF6
  1A   BYTE ???                              
  1B   BYTE ???                                
  1C   BYTE ???
  1D   BYTE ???
...
INFLIGHT_ALIENS                     EQU 0x42B0
INFLIGHT_ALIENS_END                 EQU 0x43AF


0000: AF            xor  a
0001: 32 01 70      ld   (0x7001),a           | write to regen NMIon
...
0008: 3A 07 40      ld   a,(0x4007)           | read IS_GAME_OVER flag
...
0018: 77            ld   (hl),a
0019: 23            inc  hl
001A: 10 FC         djnz 0x0018
001C: 0D            dec  c
001D: 20 F9         jr   nz,0x0018
001F: C9            ret
...
002A: 5F            ld   e,a
002B: 16 00         ld   d,0x00               | extend A into DE. Now DE = offset into table
...
003C: 3A 1E 40      ld   a,(0x401E)          
003F: 47            ld   b,a
0040: 87            add  a,a
0041: 87            add  a,a
0042: 80            add  a,b
0043: 3C            inc  a
0044: 32 1E 40      ld   (0x401E),a
0047: C9            ret
...
*
* Thanks to Phil Murray (PhilMurr on UKVAC) for pointing out the significance of memory address 0x66 to me.
...
006C: FD E5         push iy
006E: AF            xor  a
006F: 32 01 70      ld   (0x7001),a           | Write to regen NMIon. This must disable further NMIs until the handler is done.
0072: 3A 1A 40      ld   a,(0x401A)           | read DIAGNOSTIC_MESSAGE_TYPE
0075: A7            and  a                   | test if zero
0076: C2 CD 1B      jp   nz,0x1BCD            | if not zero, goto 0x1BCD - perform a diagnostic test
...
008A: 32 16 40      ld   (0x4016),a           | and write to PREV_PREV_PREV_PORT_STATE_6000 
...
00AD: C2 00 00      jp   nz,0x0000            | if bit set, goto 0x0000 - redo test sequence

* if TEST not pressed
00B0: 21 5F 42      ld   hl,0x425F            | pointer to TIMING_VARIABLE
...
00BA: CD 7C 19      call 0x197C               | call HANDLE_COIN_LOCKOUT
...
00C6: 21 D8 00      ld   hl,0x00D8            | return address
00C9: E5            push hl                  | save it on the stack. RET will return to 0x00D8 
00CA: 3A 05 40      ld   a,(0x4005)           | read SCRIPT_NUMBER
00CD: EF            rst  0x28                 | jump to code @ 0x00CE + (A*2)

SCRIPT_TABLE:
00CE: 
     E6 00          | 0x00E6   (SCRIPT_ZERO)    
     56 01          | 0x0156   (SCRIPT_ONE)
     F2 03          | 0x03F2   (SCRIPT_TWO)
     36 05          | 0x0536   (SCRIPT_THREE)
     7B 07          | 0x077B   (SCRIPT_FOUR)
...
00DD: D1            pop  de
00DE: C1            pop  bc
00DF: 3E 01         ld   a,0x01
00E1: 32 01 70      ld   (0x7001),a           | Write to regen NMIon. I think this will re-enable NMIs.
00E4: F1            pop  af
00E5: C9            ret
...
00E6: 2A 0B 40      ld   hl,(0x400B)          | Read TEMP_CHAR_RAM_PTR. This holds character RAM to start clearing from
00E9: 06 20         ld   b,0x20               | #0x20 (32 decimal) bytes to fill in a row
00EB: 3E 10         ld   a,0x10               | ordinal of empty character
00ED: D7            rst  0x10                 | Clear entire row of characters
00EE: 22 0B 40      ld   (0x400B),hl          | save in TEMP_CHAR_RAM_PTR
00F1: 21 08 40      ld   hl,0x4008            | load HL with address of TEMP_COUNTER_1
00F4: 35            dec  (hl)                | decrement value
00F5: C0            ret  nz                  | if counter hasn't hit zero, return

00F6: 2D            dec  l                   | point HL to IS_GAME_OVER
00F7: 36 01         ld   (hl),0x01               
00F9: 2D            dec  l                   | point HL to IS_GAME_IN_PLAY
00FA: 36 00         ld   (hl),0x00             
00FC: 2D            dec  l                   | point HL to SCRIPT_NUMBER
00FD: 36 01         ld   (hl),0x01 

00FF: AF            xor  a
0100: 32 0A 40      ld   (0x400A),a           | reset SCRIPT_STAGE to 0
...
0117: 11 1B 05      ld   de,0x051B            | load DE with address of PACKED_DEFAULT_SWARM_DEFINITION
011A: CD 46 06      call 0x0646               | call UNPACK_ALIEN_SWARM
...
0134: CD 95 05      call 0x0595               | call SET_COLOUR_ATTRIBUTES_TABLE_1
0137: 3E 01         ld   a,0x01
0139: 32 40 53      ld   (0x5340),a           | poke "1" to character RAM
013C: 3E 25         ld   a,0x25
013E: 32 20 53      ld   (0x5320),a           | poke "U" to character RAM
0141: 3E 20         ld   a,0x20
0143: 32 00 53      ld   (0x5300),a           | poke "P" to character RAM - text "1UP" now drawn
...
SCRIPT_ONE:
0156: CD 0D 09      call 0x090D               | call HANDLE_SWARM_MOVEMENT  
0159: CD 8E 09      call 0x098E               | call SET_ALIEN_PRESENCE_FLAGS
015C: 21 D7 03      ld   hl,0x03D7            | return address 
015F: E5            push hl                  | push return address onto the stack
0160: 3A 0A 40      ld   a,(0x400A)           | read SCRIPT_STAGE       
0163: EF            rst  0x28                 | jump to code @ 0x0164 + (A*2)

0164: 
      8C 01         | 0x018C (DISPLAY_GAME_OVER_AND_REMAINING_CREDIT_1)
      BE 01         | 0x01BE (SET_PUSH_START_BUTTON_COUNTER)
      C6 01         | 0x01C6 (HIDE_SWARM_AND_PREPARE_TO_CLEAR_SCREEN) 
      E1 01         | 0x01E1 (CLEAR_SCREEN_BEFORE_WE_ARE_THE_GALAXIANS_INTRO)
      18 02         | 0x0218 (DISPLAY_WE_ARE_THE_GALAXIANS_INTRO)
      3F 02         | 0x023F (SCROLL_ON_CONVOY_CHARGER_POINTS)
      67 02         | 0x0267 (DISPLAY_NAMCO_LOGO) 
      8E 02         | 0x028E (BLINK_CONVOY_CHARGER_POINTS)
      C6 01         | 0x01C6 (HIDE_SWARM_AND_PREPARE_TO_CLEAR_SCREEN) 
      9D 02         | 0x029D (CLEAR_WE_ARE_GALAXIANS_SCREEN_AND_DISPLAY_GAME_OVER)
      D1 02         | 0x02D1 (DISPLAY_GAME_OVER_AND_REMAINING_CREDIT_2)
      2E 03         | 0x032E (WAIT_FOR_TEMP_COUNTER_2_THEN_ADVANCE_TO_NEXT_STAGE) 
      E8 02         | 0x02E8 (CLEAR_ALIEN_SWARM_AND_SUSPEND_SWARM_ANIMATION)
      FD 02         | 0x02FD (CREATE_ATTRACT_MODE_ALIEN_SWARM)
      14 06         | 0x0614 (HANDLE_SPAWN_PLAYER)
      61 06         | 0x0661 (HANDLE_MAIN_GAME_LOGIC)
      D8 06         | 0x06D8 (HANDLE_PLAYER_ONE_KILLED)
      2E 03         | 0x032E (WAIT_FOR_TEMP_COUNTER_2_THEN_ADVANCE_TO_NEXT_STAGE)
      22 03         | 0x0322 (SET_SCRIPT_STAGE_TO_1)
      00 00         
...
018C: 11 01 07      ld   de,0x0701            | command: BOTTOM_OF_SCREEN_INFO_COMMAND, parameter: 1 (invokes DISPLAY_AVAILABLE_CREDIT)
018F: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
0192: 11 00 06      ld   de,0x0600            | command: PRINT_TEXT, parameter: 0 (index of GAME OVER)
0195: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
0198: 3E 01         ld   a,0x01
019A: 32 07 40      ld   (0x4007),a           | set IS_GAME_OVER flag
019D: 32 04 70      ld   (0x7004),a           | enable stars
01A0: 32 02 70      ld   (0x7002),a           | Does nothing      
01A3: 32 03 70      ld   (0x7003),a           | Does nothing
01A6: 21 0A 40      ld   hl,0x400A            | pointer to SCRIPT_STAGE
01A9: 34            inc  (hl)                | advance to next stage
01AA: AF            xor  a
01AB: 32 19 40      ld   (0x4019),a           | clear PUSH_START_BUTTON_COUNTER
01AE: 32 0D 40      ld   (0x400D),a           | set CURRENT_PLAYER to 0 (player one)
01B1: 32 0E 40      ld   (0x400E),a           | clear IS_TWO_PLAYER_GAME
01B4: 32 06 40      ld   (0x4006),a           | clear IS_GAME_IN_PLAY
01B7: 21 60 10      ld   hl,0x1060
01BA: 22 08 40      ld   (0x4008),hl          | set TEMP_COUNTER_1 and TEMP_COUNTER_2 
01BD: C9            ret
...
01C0: 32 19 40      ld   (0x4019),a           | set PUSH_START_BUTTON_COUNTER
01C3: C3 36 03      jp   0x0336               | jump to WAIT_FOR_TEMP_COUNTERS
...
01C6: 21 00 41      ld   hl,0x4100            | load HL with address of ALIEN_SWARM_FLAGS
01C9: 06 80         ld   b,0x80               | sizeof(ALIEN_SWARM_FLAGS) array
01CB: AF            xor  a                   
01CC: D7            rst  0x10                 | Clear all alien swarm flags 
01CD: 32 5F 42      ld   (0x425F),a           | set TIMING_VARIABLE
01D0: 32 24 42      ld   (0x4224),a           | reset HAVE_AGGRESSIVE_ALIENS flag
01D3: 21 02 50      ld   hl,0x5002            | address of column 2 in character RAM
01D6: 22 0B 40      ld   (0x400B),hl          | write to TEMP_CHAR_RAM_PTR
01D9: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
01DC: 36 20         ld   (hl),0x20            | set counter 
01DE: 2C            inc  l                   | bump HL to 0x400A (SCRIPT_STAGE)
...
01E1: 2A 0B 40      ld   hl,(0x400B)          | load HL with contents of TEMP_CHAR_RAM_PTR
01E4: 06 1C         ld   b,0x1C               | We want to clear #0x1C (28 characters) on this row 
01E6: 3E 10         ld   a,0x10               | ordinal for empty character
01E8: D7            rst  0x10                 | Clear 28 characters from row
01E9: 11 04 00      ld   de,0x0004            | As a row is 32 characters wide, to get to start of next row...
01EC: 19            add  hl,de               | ... we need to add 4 characters.
01ED: 22 0B 40      ld   (0x400B),hl          | write to TEMP_CHAR_RAM_PTR
01F0: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2  
01F3: 35            dec  (hl)                | decrement value of counter
01F4: C0            ret  nz                  | if value is not zero then exit 

01F5: 2C            inc  l                   | bump HL to 0x400A (SCRIPT_STAGE)
01F6: 34            inc  (hl)                | advance to next part of script.
01F7: 21 40 04      ld   hl,0x0440
01FA: 22 08 40      ld   (0x4008),hl          | set TEMP_COUNTER_1 and TEMP_COUNTER_2 in one go

01FD: AF            xor  a
01FE: 06 30         ld   b,0x30
0200: 21 00 42      ld   hl,0x4200
0203: D7            rst  0x10                 | Clear from 0x4200-422F
0204: 32 06 70      ld   (0x7006),a           | reset regen hflip
0207: 32 07 70      ld   (0x7007),a           | reset regen vflip
020A: 32 18 40      ld   (0x4018),a           | reset DISPLAY_IS_COCKTAIL_P2
020D: 3E 01         ld   a,0x01
020F: 32 38 42      ld   (0x4238),a           | set DISABLE_SWARM_ANIMATION flag. 
0212: 21 B1 1D      ld   hl,0x1DB1            | pointer to COLOUR_ATTRIBUTE_TABLE_3
0215: C3 98 05      jp   0x0598               | jump to SET_COLOUR_ATTRIBUTES
...
0218: CD 63 03      call 0x0363               | call HANDLE_ALIEN_SWARM_SCROLL_RESET
021B: 21 08 40      ld   hl,0x4008            | load HL with address of TEMP_COUNTER_1
021E: 35            dec  (hl)
021F: C0            ret  nz
0220: 36 50         ld   (hl),0x50            | reset counter
0222: 2C            inc  l                   | bump HL to TEMP_COUNTER_2
0223: 16 06         ld   d,0x06               | Command: PRINT_TEXT
...
0228: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
022B: 35            dec  (hl)                | bump TEMP_COUNTER_2 to index of next string to print
022C: C0            ret  nz

022D: 2C            inc  l                   | bump HL to 0x400A (SCRIPT_STAGE)
022E: 34            inc  (hl)                | advance to next stage
022F: 21 20 04      ld   hl,0x0420
0232: 22 08 40      ld   (0x4008),hl          | set TEMP_COUNTER_1 and TEMP_COUNTER_2 in one go

* hide all sprites 
0235: 21 B0 42      ld   hl,0x42B0            | address of INFLIGHT_ALIENS
0238: AF            xor  a
0239: 47            ld   b,a
023A: D7            rst  0x10                 | Fill the entire INFLIGHT_ALIENS array with zero
023B: 32 41 42      ld   (0x4241),a           | set ATTRACT_MODE_SCROLL_ID
023E: C9            ret
...
023F: CD 63 03      call 0x0363               | call HANDLE_ALIEN_SWARM_SCROLL_RESET
0242: CD BE 0B      call 0x0BBE               | call HANDLE_INFLIGHT_ALIEN_SPRITE_UPDATE
0245: CD C3 0C      call 0x0CC3               | call HANDLE_INFLIGHT_ALIENS
0248: CD 67 03      call 0x0367               | call HANDLE_DRAW_CONVOY_CHARGER_POINTS
024B: 21 08 40      ld   hl,0x4008            | load HL with address of TEMP_COUNTER_1
024E: 35            dec  (hl)
024F: C0            ret  nz
0250: 36 D2         ld   (hl),0xD2
...
0253: CD 41 03      call 0x0341               | call INIT_CONVOY_CHARGER_SPRITE
0256: EB            ex   de,hl
0257: 21 41 42      ld   hl,0x4241            | pointer to ATTRACT_MODE_SCROLL_ID
025A: 34            inc  (hl)                | set id to next thing to scroll on
025B: EB            ex   de,hl
025C: 35            dec  (hl)                | dec TEMP_COUNTER_2
025D: C0            ret  nz                  | return if not zero

025E: 36 D2         ld   (hl),0xD2
0260: 2C            inc  l                   | point HL to SCRIPT_STAGE
0261: 34            inc  (hl)                | increment script stage 
0262: AF            xor  a
0263: 32 58 40      ld   (0x4058),a           | write to scroll offset in OBJRAM_BACK_BUF
0266: C9            ret



DISPLAY_NAMCO_LOGO:
0267: CD 63 03      call 0x0363               | call HANDLE_ALIEN_SWARM_SCROLL_RESET
026A: CD BE 0B      call 0x0BBE               | call HANDLE_INFLIGHT_ALIEN_SPRITE_UPDATE
026D: CD C3 0C      call 0x0CC3               | call HANDLE_INFLIGHT_ALIENS 
0270: CD 67 03      call 0x0367               | call HANDLE_DRAW_CONVOY_CHARGER_POINTS

* wait until TEMP_COUNTER_2 reaches 0
0273: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
0276: 35            dec  (hl)
0277: C0            ret  nz
0278: 2C            inc  l                   | bump HL to point to SCRIPT_STAGE
0279: 34            inc  (hl)                | advance to next stage
027A: AF            xor  a
027B: 32 58 40      ld   (0x4058),a           | set scroll offset in OBJRAM_BACK_BUF
027E: 21 40 11      ld   hl,0x1140
0281: 22 08 40      ld   (0x4008),hl          | set TEMP_COUNTER_1 and TEMP_COUNTER_2 in one go
0284: 21 41 42      ld   hl,0x4241            | pointer to ATTRACT_MODE_SCROLL_ID
0287: 34            inc  (hl)
0288: 11 0F 06      ld   de,0x060F            | command: PRINT_TEXT, parameter: #0x0F (Displays NAMCO logo)
028B: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND               
...
028E: CD 63 03      call 0x0363               | call HANDLE_ALIEN_SWARM_SCROLL_RESET
0291: CD BE 0B      call 0x0BBE               | call HANDLE_INFLIGHT_ALIEN_SPRITE_UPDATE
0294: CD C3 0C      call 0x0CC3               | call HANDLE_INFLIGHT_ALIENS
0297: CD 67 03      call 0x0367               | call HANDLE_DRAW_CONVOY_CHARGER_POINTS
029A: C3 36 03      jp   0x0336               | jump to WAIT_FOR_TEMP_COUNTERS
...
029D: CD 63 03      call 0x0363               | call HANDLE_ALIEN_SWARM_SCROLL_RESET
* clear everything except HUD
02A0: 2A 0B 40      ld   hl,(0x400B)          | read contents of TEMP_CHAR_RAM_PTR. Now HL = pointer to row to clear 
02A3: 06 1C         ld   b,0x1C               | we want to clear 0x1C (28 decimal) characters
02A5: 3E 10         ld   a,0x10               | ordinal of empty character
02A7: D7            rst  0x10                 | Clear 28 characters on column   
02A8: 11 04 00      ld   de,0x0004            | As the screen is 32 characters wide, we need to add 4 to get to...
02AB: 19            add  hl,de               | the start of the next column
02AC: 22 0B 40      ld   (0x400B),hl          | Update TEMP_CHAR_RAM_PTR 

* wait until TEMP_COUNTER_2 reaches 0
02AF: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
02B2: 35            dec  (hl)
02B3: C0            ret  nz
...
02B6: 21 B0 42      ld   hl,0x42B0            | load HL with address of INFLIGHT_ALIENS
02B9: AF            xor  a
02BA: 47            ld   b,a
02BB: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.  

* hide all sprites & bullets
02BC: 21 60 40      ld   hl,0x4060            | pointer to OBJRAM_BACK_BUF_SPRITES
02BF: 06 40         ld   b,0x40               | 
02C1: D7            rst  0x10                 | clear sprite and bullet information from back buffer  

* display GAME OVER
02C2: 21 40 04      ld   hl,0x0440 
02C5: 22 08 40      ld   (0x4008),hl          | set TEMP_COUNTER_1 and TEMP_COUNTER_2
02C8: CD 95 05      call 0x0595               | call SET_COLOUR_ATTRIBUTES_TABLE_1
02CB: 11 00 06      ld   de,0x0600            | command: PRINT_TEXT, parameter: 0 (index of "GAME OVER")
02CE: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND               
...
02D1: 11 01 07      ld   de,0x0701            | command: BOTTOM_OF_SCREEN_INFO_COMMAND , calls DISPLAY_AVAILABLE_CREDIT
02D4: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
02D7: 11 00 06      ld   de,0x0600            | command: PRINT_TEXT, parameter: 0 (index of "GAME OVER")
02DA: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
02DD: 21 0A 40      ld   hl,0x400A            | load HL with address of SCRIPT_STAGE
02E0: 34            inc  (hl)                | advance to next stage of script
02E1: 21 60 10      ld   hl,0x1060
02E4: 22 08 40      ld   (0x4008),hl          | set TEMP_COUNTER_1 and TEMP_COUNTER_2 
02E7: C9            ret
...
02E8: 21 00 41      ld   hl,0x4100            | load HL with address of ALIEN_SWARM_FLAGS
02EB: 06 80         ld   b,0x80               | sizeof(ALIEN_SWARM_FLAGS)
02ED: AF            xor  a
02EE: D7            rst  0x10                 | Reset all flags in ALIEN_SWARM_FLAGS array  
02EF: 32 5F 42      ld   (0x425F),a           | clear TIMING_VARIABLE
02F2: 32 38 42      ld   (0x4238),a           | clear DISABLE_SWARM_ANIMATION
02F5: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
02F8: 36 40         ld   (hl),0x40
02FA: C3 93 05      jp   0x0593               | advance to next stage of script
...
02FD: 11 1B 05      ld   de,0x051B            | load DE with address of PACKED_DEFAULT_SWARM_DEFINITION
0300: CD 46 06      call 0x0646               | call UNPACK_ALIEN_SWARM
0303: EB            ex   de,hl               | now HL = pointer to DEFAULT_PLAYER_STATE (0x052B)
0304: 11 18 42      ld   de,0x4218            | load DE with address of CURRENT_PLAYER_STATE 
0307: 01 08 00      ld   bc,0x0008            | sizeof (CURRENT_PLAYER_STATE)
030A: ED B0         ldir                     | reset current player state to default
030C: AF            xor  a
030D: 32 5F 42      ld   (0x425F),a           | reset TIMING_VARIABLE

* Give the demo player 1 life 
0310: 3C            inc  a
0311: 32 1D 42      ld   (0x421D),a           | set PLAYER_LIVES
0314: 21 0A 40      ld   hl,0x400A            | pointer to SCRIPT_STAGE
0317: 34            inc  (hl)                | advance to next stage of script            
0318: 2C            inc  l                   | bump HL to point to TEMP_CHAR_RAM_PTR
0319: 36 96         ld   (hl),0x96
031B: 21 40 06      ld   hl,0x0640
031E: 22 45 42      ld   (0x4245),hl          | set FLAGSHIP_ATTACK_MASTER_COUNTER_1 and FLAGSHIP_ATTACK_MASTER_COUNTER_2
0321: C9            ret
...
0322: 3E 01         ld   a,0x01
0324: 32 0A 40      ld   (0x400A),a           | set SCRIPT_STAGE to 1
0327: 21 03 03      ld   hl,0x0303            
032A: 22 08 40      ld   (0x4008),hl          | set TEMP_COUNTER_1 and TEMP_COUNTER_2
032D: C9            ret
...
032E: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
0331: 35            dec  (hl)                | decrement value of counter
0332: C0            ret  nz                  | return if counter is !=0
0333: 2C            inc  l                   | bump HL to point to 0x401A (SCRIPT_STAGE)
...
0336: 21 08 40      ld   hl,0x4008             | load HL with address of TEMP_COUNTER_1
0339: 35            dec  (hl)                 | decrement value of counter
033A: C0            ret  nz                   | return if counter is !=0
033B: 36 3C         ld   (hl),0x3C             | reset counter to 0x3C (50 decimal)
033D: 2C            inc  l                    | bump HL to point to TEMP_COUNTER_2
033E: C3 31 03      jp   0x0331                | and go check if that counter has counted down to 0 yet 
...
0348: 5F            ld   e,a                 |
0349: 16 00         ld   d,0x00               | Now DE = offset 
034B: 21 30 43      ld   hl,0x4330            | HL = address of INFLIGHT_ALIENS[3]
034E: 19            add  hl,de               | Add offset to HL. HL now points to INFLIGHT_ALIEN record we're using to scroll sprite on with
034F: 36 01         ld   (hl),0x01            | set INFLIGHT_ALIEN.IsActive to 1 
0351: 2C            inc  l                 
0352: 36 00         ld   (hl),0x00            | reset INFLIGHT_ALIEN.IsDying 
0354: 2C            inc  l
0355: 36 0D         ld   (hl),0x0D            | set INFLIGHT_ALIEN.StageOfLife to INFLIGHT_ALIEN_CONVOY_CHARGER_SET_COLOUR_POS_ANIM
0357: 2C            inc  l
0358: 2C            inc  l
0359: 36 00         ld   (hl),0x00            | set INFLIGHT_ALIEN.Y to position offscreen
035B: 2C            inc  l
035C: 36 0C         ld   (hl),0x0C            | set INFLIGHT_ALIEN.AnimationFrame
035E: 2C            inc  l
035F: 2C            inc  l
...
0364: C3 72 09      jp   0x0972               | call SET_SWARM_SCROLL_OFFSET
...
0367: 3A 41 42      ld   a,(0x4241)           | read ATTRACT_MODE_SCROLL_ID
...
036E: 47            ld   b,a
036F: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
0372: 4F            ld   c,a                 | save it in C 
0373: E6 3F         and  0x3F                 | mask in bits 0..5. Now A is a value from 0..63 decimal.
0375: 28 49         jr   z,0x03C0             | if bits 0..5 are not set, call CLEAR_DEMO_CONVOY_CHARGER_POINTS

0377: FE 20         cp   0x20                 | If A is not exactly 0x20..
0379: C0            ret  nz                  | .. exit
...
037B: 07            rlca                     
037C: 07            rlca               
037D: E6 03         and  0x03                 | now A is a value from 0..3. A is now an index into table @0x039A 
037F: 4F            ld   c,a                 | multiply A...
0380: 87            add  a,a                 |   
0381: 81            add  a,c                 | .. by 3
0382: 5F            ld   e,a 
0383: 16 00         ld   d,0x00               | extend A into DE. Now DE is an offset to add to HL

0385: 21 9A 03      ld   hl,0x039A            | pointer to Flagship score table
0388: 19            add  hl,de
0389: 11 93 51      ld   de,0x5193            | address in character RAM
038C: CD AF 03      call 0x03AF               | call DRAW_3_CHARACTERS
038F: 05            dec  b
0390: C8            ret  z

* The alien scores are static and don't cycle, so we just draw them straight from the table 
0391: 21 A6 03      ld   hl,0x03A6            | pointer to Alien score table
0394: CD AF 03      call 0x03AF               | call DRAW_3_CHARACTERS
0397: 10 FB         djnz 0x0394
0399: C9            ret
...
DRAW_3_CHARACTERS:
03AF: 0E 03         ld   c,0x03               | number of characters to draw       
...
03B5: D6 20         sub  0x20                 | subtract #0x20 (32 decimal) from it. Now DE points to character in same column, row above 
03B7: 5F            ld   e,a                 | 
03B8: 0D            dec  c                   | decrement counter of characters to draw. 
03B9: C2 B1 03      jp   nz,0x03B1            | if counter !=0, more characters are to be drawn, goto 0x03B1
03BC: C6 62         add  a,0x62               
03BE: 5F            ld   e,a                 | Add #0x62 (98 decimal) to DE. Now DE is back on row we started drawing from.
03BF: C9            ret
...
03C0: 21 93 51      ld   hl,0x5193            | address in character RAM
03C3: 11 E0 FF      ld   de,0xFFE0            | offset to add to character RAM address  (-32 decimal.)
03C6: 0E 03         ld   c,0x03               | 3 characters to erase
03C8: 3E 10         ld   a,0x10               | ordinal of empty character
...
03CD: C2 CA 03      jp   nz,0x03CA            | if not done goto 0x03CA
03D0: 7D            ld   a,l
03D1: C6 62         add  a,0x62
03D3: 6F            ld   l,a
03D4: 10 F0         djnz 0x03C6
03D6: C9            ret
...
03D7: 3A 02 40      ld   a,(0x4002)           | read NUM_CREDITS
03DA: A7            and  a                   | check if zero
03DB: C8            ret  z                   | return if no credits

03DC: 21 05 40      ld   hl,0x4005            | load HL with address of SCRIPT_NUMBER
03DF: 34            inc  (hl)                | advance to next script
03E0: 2C            inc  l              
03E1: 2C            inc  l                   | bump HL to point to IS_GAME_OVER
03E2: 36 00         ld   (hl),0x00            | set IS_GAME_OVER to 0
03E4: AF            xor  a
03E5: 32 0A 40      ld   (0x400A),a           | set SCRIPT_STAGE to 0
03E8: 32 C2 41      ld   (0x41C2),a           | set ENABLE_ALIEN_ATTACK_SOUND to 0
03EB: 32 DF 41      ld   (0x41DF),a           | set ALIEN_DEATH_SOUND to 0
03EE: 32 B0 40      ld   (0x40B0),a           | clear IS_COLUMN_SCROLLING flag
03F1: C9            ret
...
SCRIPT_TWO:
03F2: CD 0D 09      call 0x090D               | call HANDLE_SWARM_MOVEMENT
03F5: CD 8E 09      call 0x098E               | call SET_ALIEN_PRESENCE_FLAGS
03F8: 21 92 04      ld   hl,0x0492            | address of HANDLE_START_BUTTONS
03FB: E5            push hl
03FC: 3A 0A 40      ld   a,(0x400A)           | read SCRIPT_STAGE
03FF: EF            rst  0x28                 | jump to code @ 0x0400 + (A*2)

0400: 
    08 04           | 0x0408                  | INIT_0408
    30 04           | 0x0430                  | WAIT_BEFORE_DISPLAYING_PUSH_START_BUTTON
    43 04           | 0x0443                  | DISPLAY_PUSH_START_BUTTON_AND_BONUS_GALIXIP_FOR
    73 04           | 0x0473                  | BLINK_LAMPS_IF_CREDIT_INSERTED
...
INIT_0408:
0408: 21 91 1D      ld   hl,0x1D91            | pointer to COLOUR_ATTRIBUTE_TABLE_2
040B: CD 98 05      call 0x0598               | call SET_COLOUR_ATTRIBUTES

* hide all sprites
040E: 21 60 40      ld   hl,0x4060            | load HL with address of OBJRAM_BACK_BUF_SPRITES
0411: 06 40         ld   b,0x40               
0413: AF            xor  a
0414: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A. 
...
041C: 32 38 42      ld   (0x4238),a           | clear DISABLE_SWARM_ANIMATION     
041F: 32 B0 40      ld   (0x40B0),a           | clear IS_COLUMN_SCROLLING  
0422: 21 02 50      ld   hl,0x5002            | point to address in character RAM (3rd character of top row)
0425: 22 0B 40      ld   (0x400B),hl          | set TEMP_CHAR_RAM_PTR
0428: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
042B: 36 10         ld   (hl),0x10            | set countdown to 0x10 (16 decimal)
...
0430: 21 19 40      ld   hl,0x4019            | pointer to PUSH_START_BUTTON_COUNTER
0433: 35            dec  (hl)                | decrement counter
0434: C2 73 04      jp   nz,0x0473            | if non zero, goto BLINK_LAMPS_IF_CREDIT_INSERTED 

* counter's hit zero, go to next stage of script.
0437: 21 0A 40      ld   hl,0x400A            | pointer to SCRIPT_STAGE
043A: 34            inc  (hl)                | advance to next stage of script
043B: 21 00 41      ld   hl,0x4100            | load HL with address of ALIEN_SWARM_FLAGS
043E: 06 80         ld   b,0x80               | sizeof(ALIEN_SWARM_FLAGS)
0440: AF            xor  a                  
0441: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.  
0442: C9            ret
...
0443: 2A 0B 40      ld   hl,(0x400B)          | Get pointer to row to clear from TEMP_CHAR_RAM_PTR
0446: 06 1C         ld   b,0x1C               | #0x1C (28 decimal) characters to clear
0448: 3E 10         ld   a,0x10               | Ordinal for empty space character
044A: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A. 
044B: 11 04 00      ld   de,0x0004            | We've done 28 characters. Need 4 more to move to next row.
044E: 19            add  hl,de               | Adjust HL to point to start of next row
044F: 06 1C         ld   b,0x1C               | #0x1C (28 decimal) characters to clear
0451: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.  
0452: 19            add  hl,de               | Adjust HL to point to start of next row
0453: 22 0B 40      ld   (0x400B),hl          | And update TEMP_CHAR_RAM_PTR pointer 

* wait for TEMP_COUNTER_2 to reach 0
0456: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
0459: 35            dec  (hl)
045A: C0            ret  nz
...
045D: AF            xor  a
045E: 32 06 70      ld   (0x7006),a           | set regen hflip
0461: 32 07 70      ld   (0x7007),a           | set regen vflip
0464: 32 18 40      ld   (0x4018),a           | set DISPLAY_IS_COCKTAIL_P2

* display BONUS GALIXIP FOR and PUSH START BUTTON messages
0467: 11 02 07      ld   de,0x0702            | command: BOTTOM_OF_SCREEN_INFO_COMMAND , parameter: 2 (invokes DISPLAY_BONUS_GALIXIP_FOR)
046A: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
046D: 11 01 06      ld   de,0x0601            | command: PRINT_TEXT, parameter: 1 (index of "PUSH START BUTTON")
0470: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
...
0473: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
0476: E6 20         and  0x20                 | mask in bit 5
0478: 28 11         jr   z,0x048B             | if bit 5 is not set, turn off the lamps

* Does player have credit? If not, then don't turn the lamps on.
047A: 3A 02 40      ld   a,(0x4002)           | read NUM_CREDITS
047D: A7            and  a                   | test if credits is 0
047E: C8            ret  z                   | return if no credits 

* turn the lamps on to show you have credit
047F: 47            ld   b,a
0480: 3E 01         ld   a,0x01
0482: 32 00 60      ld   (0x6000),a           | turn on lamp 1         
0485: 05            dec  b
0486: C8            ret  z
0487: 32 01 60      ld   (0x6001),a           | turn on lamp 2
048A: C9            ret

* turn lamps off
048B: 32 00 60      ld   (0x6000),a           | turn on/off lamp 1
048E: 32 01 60      ld   (0x6001),a           | turn on/off lamp 2
0491: C9            ret
...
0492: 3A 11 40      ld   a,(0x4011)           | read PORT_STATE_6800
0495: CB 47         bit  0,a                 | test for 1P START button being hit
0497: 20 59         jr   nz,0x04F2            | if button is hit, goto 1P_START_BUTTON_HANDLER
...
049C: 3A 02 40      ld   a,(0x4002)           | read NUM_CREDITS
049F: FE 02         cp   0x02                 | do we have at least 2 credits for a 2 player game?
04A1: D8            ret  c                   | if credits < 2, then we don't have enough credit, return
04A2: D6 02         sub  0x02                 | otherwise, reduce credits by 2
...
04B2: 3A 1F 40      ld   a,(0x401F)           | read DIP_SWITCH_5_STATE
04B5: 0F            rrca                     | move bit 0 into carry
04B6: DC 0F 05      call c,0x050F             | if carry is set, call AWARD_PLAYER_TWO_THREE_LIVES
04B9: 21 00 01      ld   hl,0x0100            | CURRENT_PLAYER will be set to 0, IS_TWO_PLAYER_GAME set to 1
04BC: 22 0D 40      ld   (0x400D),hl          | set CURRENT_PLAYER, and IS_TWO_PLAYER_GAME 

* Create alien swarm and load default settings for player 1 
04BF: 21 1B 05      ld   hl,0x051B            | load HL with address of PACKED_DEFAULT_SWARM_DEFINITION
04C2: 11 80 41      ld   de,0x4180            | load DE with address of PLAYER_ONE_PACKED_SWARM_DEF
04C5: 01 20 00      ld   bc,0x0020            | sizeof (PLAYER_ONE_PACKED_SWARM_DEF)+sizeof(PLAYER_ONE_STATE)
04C8: ED B0         ldir

* if dip switch 5 is on, then player 1 gets 3 lives
04CA: 3A 1F 40      ld   a,(0x401F)           | read DIP_SWITCH_5_STATE
04CD: 0F            rrca                     | move bit 0 into carry
04CE: DC 15 05      call c,0x0515             | if carry is set, call AWARD_PLAYER_ONE_THREE_LIVES

04D1: AF            xor  a
04D2: 32 0A 40      ld   (0x400A),a           | set SCRIPT_STAGE to 0
04D5: 3E 03         ld   a,0x03
04D7: 32 05 40      ld   (0x4005),a           | set SCRIPT_NUMBER to 3
04DA: 3E 01         ld   a,0x01
04DC: 32 06 40      ld   (0x4006),a           | set IS_GAME_IN_PLAY
04DF: 32 D1 41      ld   (0x41D1),a

* Display high score, reset P1 and P2's score to 0
04E2: 11 04 06      ld   de,0x0604            | command: PRINT_TEXT, parameter: 4 (index of "HIGH SCORE")
04E5: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
04E8: 11 00 04      ld   de,0x0400            | command: RESET_SCORE_COMMAND, parameter: 0 (reset player 1's score and clear life awarded)
04EB: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
04EE: 1C            inc  e                   | command: RESET_SCORE_COMMAND, parameter: 1 (reset player 2's score and clear life awarded)
04EF: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND
...
04F2: 3A 02 40      ld   a,(0x4002)           | read number of credits
04F5: A7            and  a                   | test if number of credits is zero
04F6: 28 11         jr   z,0x0509             | if number of credits is zero, goto 0x0509
04F8: 3D            dec  a                   | otherwise reduce number of credits by 1
04F9: 32 02 40      ld   (0x4002),a           | and store back in credit counter
04FC: 21 A0 41      ld   hl,0x41A0            | load HL with address of PLAYER_TWO_PACKED_SWARM_DEF
04FF: 06 20         ld   b,0x20               | sizeof(PLAYER_TWO_PACKED_SWARM_DEF) + sizeof(PLAYER_TWO_STATE)
0501: AF            xor  a                   | 
0502: D7            rst  0x10                 | clear player 2 swarm & state as we're not using them
0503: 21 00 00      ld   hl,0x0000            | CURRENT_PLAYER will be set to 0, IS_TWO_PLAYER_GAME set to 0
0506: C3 BC 04      jp   0x04BC               | go set CURRENT_PLAYER, and IS_TWO_PLAYER_GAME.

0509: 3E 01         ld   a,0x01
050B: 32 05 40      ld   (0x4005),a           | set SCRIPT_NUMBER to 1
050E: C9            ret
...
050F: 3E 03         ld   a,0x03
0511: 32 B5 41      ld   (0x41B5),a           | Set number of lives for player 2
0514: C9            ret


* Only called if Dip Switch 5 is set to ON
AWARD_PLAYER_ONE_THREE_LIVES:
0515: 3E 03         ld   a,0x03
0517: 32 95 41      ld   (0x4195),a           | Set number of lives for player 1
051A: C9            ret
...
SCRIPT_THREE:
0536: CD 0D 09      call 0x090D               | call HANDLE_SWARM_MOVEMENT
0539: CD 8E 09      call 0x098E               | call SET_ALIEN_PRESENCE_FLAGS
053C: 3A 0A 40      ld   a,(0x400A)
053F: EF            rst  0x28                 | jump to code @ 0x0540 + (A*2)
0540: 
      50 05         | 0x0550   
      83 05         | 0x0583 (CLEAR_ROW_OF_SCREEN)
      A5 05         | 0x05A5 (PLAYER_ONE_INIT)
      05 06         | 0x0605 (CLEAR_PLAYER_TEXT)
      14 06         | 0x0614 (HANDLE_SPAWN_PLAYER)
      61 06         | 0x0661 (HANDLE_MAIN_GAME_LOGIC)                  
      D8 06         | 0x06D8 (HANDLE_PLAYER_ONE_KILLED)
      3D 07         | 0x073D (SWITCH_TO_PLAYER_TWO)


*
0550: 21 00 41      ld   hl,0x4100            | load HL with address of ALIEN_SWARM_FLAGS
0553: 06 80         ld   b,0x80               | sizeof (ALIEN_SWARM_FLAGS)
0555: AF            xor  a
0556: 32 00 60      ld   (0x6000),a           | clear !DRIVER lamp 1
0559: 32 01 60      ld   (0x6001),a           | clear !DRIVER lamp 2
055C: D7            rst  0x10                 | Clear the alien swarm 

055D: 32 5F 42      ld   (0x425F),a           | set TIMING_VARIABLE

0560: 21 00 42      ld   hl,0x4200            | pointer to HAS_PLAYER_SPAWNED. Also start of player state
0563: 06 17         ld   b,0x17               | 0x17 (23 decimal) bytes to clear 
0565: D7            rst  0x10                 | Clear all player state    
0566: 2C            inc  l
0567: 06 18         ld   b,0x18
0569: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
056A: 21 60 42      ld   hl,0x4260            | load HL with address of ENEMY_BULLETS_START
056D: 06 46         ld   b,0x46
056F: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
0570: 3E 01         ld   a,0x01
0572: 32 26 42      ld   (0x4226),a           | set HAVE_NO_INFLIGHT_ALIENS flag

0575: 21 0A 40      ld   hl,0x400A            | load HL with address of SCRIPT_STAGE
0578: 34            inc  (hl)                | advance to next stage of script
0579: 2D            dec  l                   | now HL points to TEMP_COUNTER_2
057A: 36 20         ld   (hl),0x20            | load counter with 0x20 (32 decimal)
057C: 21 00 50      ld   hl,0x5000            | pointer to start of character RAM
057F: 22 0B 40      ld   (0x400B),hl          | store in TEMP_CHAR_RAM_PTR
0582: C9            ret



CLEAR_ROW_OF_SCREEN:
0583: 2A 0B 40      ld   hl,(0x400B)          | read TEMP_CHAR_RAM_PTR
0586: 06 20         ld   b,0x20               | 0x20 (32 decimal) characters in a row
0588: 3E 10         ld   a,0x10               | ordinal of empty character
058A: D7            rst  0x10                 | Clear row of characters
058B: 22 0B 40      ld   (0x400B),hl          | update TEMP_CHAR_RAM_PTR
058E: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
0591: 35            dec  (hl)                | decrement counter
0592: C0            ret  nz                  | return if not zero
0593: 2C            inc  l                   | bump HL to point to 0x400A (SCRIPT_STAGE)
0594: 34            inc  (hl)                | advance to next stage of script

SET_COLOUR_ATTRIBUTES_TABLE_1:
0595: 21 71 1D      ld   hl,0x1D71            | pointer to COLOUR_ATTRIBUTE_TABLE_1
...
05A5: 11 80 41      ld   de,0x4180            | load DE with address of PLAYER_ONE_PACKED_SWARM_DEF
05A8: CD 46 06      call 0x0646               | call UNPACK_ALIEN_SWARM
* copy player 1's state (ie: game settings) to current player state
05AB: EB            ex   de,hl               | now HL = pointer to PLAYER_ONE_STATE
05AC: 11 18 42      ld   de,0x4218            | load DE with address of CURRENT_PLAYER_STATE
05AF: 01 08 00      ld   bc,0x0008            | sizeof(CURRENT_PLAYER_STATE)
05B2: ED B0         ldir                     | write P1 player state 
* reset any game settings 
05B4: AF            xor  a
05B5: 32 5F 42      ld   (0x425F),a           | set TIMING_VARIABLE
05B8: 32 20 42      ld   (0x4220),a           | clear HAVE_NO_ALIENS_IN_SWARM flag
05BB: 32 06 70      ld   (0x7006),a           | reset regen hflip
05BE: 32 07 70      ld   (0x7007),a           | reset regen vflip
05C1: 32 18 40      ld   (0x4018),a           | reset DISPLAY_IS_COCKTAIL_P2
05C4: 21 0A 40      ld   hl,0x400A            | load HL with address of SCRIPT_STAGE
05C7: 34            inc  (hl)                | advance to next stage in script
05C8: 2D            dec  l                   | bump HL to address of TEMP_COUNTER_2
05C9: 36 96         ld   (hl),0x96            | set counter value
05CB: 21 40 06      ld   hl,0x0640
05CE: 22 45 42      ld   (0x4245),hl          | set FLAGSHIP_ATTACK_MASTER_COUNTER_1 and FLAGSHIP_ATTACK_MASTER_COUNTER_2
* if its not demo mode, display the scores
05D1: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY
05D4: 0F            rrca                     | move flag into carry
05D5: D0            ret  nc                  | return if game is not in play
05D6: 3A 0E 40      ld   a,(0x400E)           | read IS_TWO_PLAYER_GAME
05D9: 0F            rrca                     | move bit 0 into carry
05DA: 38 20         jr   c,0x05FC             | if carry is set, we're in a 2 player game, goto 0x05FC
* the next 2 lines are only executed if its a one player game
05DC: 11 00 05      ld   de,0x0500            | command: DISPLAY_SCORE_COMMAND, parameter: 0  (Displays Player 1's score)
05DF: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
* the remaining lines are executed regardless of number of players 
05E2: 1E 02         ld   e,0x02               | command: DISPLAY_SCORE_COMMAND, parameter: 2  (invokes DISPLAY_HIGH_SCORE)
05E4: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
05E7: 14            inc  d                   | command: PRINT_TEXT, parameter: 2 (index of "PLAYER ONE") 
05E8: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
05EB: 1E 04         ld   e,0x04               | command: PRINT_TEXT, parameter: 4 (index of "HIGH SCORE")
05ED: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
05F0: 11 03 07      ld   de,0x0703            | command: BOTTOM_OF_SCREEN_INFO_COMMAND, calls DISPLAY_PLAYER_SHIPS_REMAINING
05F3: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
05F6: 11 00 07      ld   de,0x0700            | command: BOTTOM_OF_SCREEN_INFO_COMMAND, calls DISPLAY_LEVEL_FLAGS
05F9: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND

* Called when we're in a two player game. Displays player one's score, player two's score and high score.
05FC: 11 03 05      ld   de,0x0503            | command: DISPLAY_SCORE_COMMAND, parameter: 3 (invokes DISPLAY_ALL_SCORES)
05FF: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
0602: C3 E2 05      jp   0x05E2               | go display high score, ships remaining
...
CLEAR_PLAYER_TEXT:
0605: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
0608: 35            dec  (hl)
0609: C0            ret  nz
060A: 36 14         ld   (hl),0x14
060C: 2C            inc  l                   | bump HL to point to 0x400A (SCRIPT_STAGE)
060D: 34            inc  (hl)                | advance to next stage of script
* the text "PLAYER ONE" and "PLAYER TWO" are plotted to the same character cells  
060E: 11 82 06      ld   de,0x0682            | command: PRINT_TEXT, parameter: #0x82 - clear "PLAYER ONE" from screen 
0611: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND
...
0614: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
0617: 35            dec  (hl)               
0618: C0            ret  nz
0619: 36 0A         ld   (hl),0x0A            | reset TEMP_COUNTER_2
061B: 2C            inc  l                   | bump HL to point to 0x400A (SCRIPT_STAGE)
061C: 34            inc  (hl)                | advance to next stage of script

* Spawn the player
061D: 21 01 00      ld   hl,0x0001
0620: 22 00 42      ld   (0x4200),hl          | set HAS_PLAYER_SPAWNED to 1, IS_PLAYER_DYING to 0
0623: 3E 80         ld   a,0x80
0625: 32 02 42      ld   (0x4202),a           | set PLAYER_Y        
 
0628: 21 E3 15      ld   hl,0x15E3            | load HL with address of ALIEN_ATTACK_COUNTER_DEFAULT_VALUES
062B: 11 4A 42      ld   de,0x424A            | load DE with address of ALIEN_ATTACK_COUNTERS
062E: 01 10 00      ld   bc,0x0010            | sizeof(ALIEN_ATTACK_COUNTERS) array
0631: ED B0         ldir                     | reset all counters to their default values

0633: AF            xor  a
0634: 32 58 40      ld   (0x4058),a           | reset scroll offset in OBJRAM_BACK_BUF
0637: 32 5A 40      ld   (0x405A),a           | reset scroll offset in OBJRAM_BACK_BUF

* Draw lives left
063A: 11 03 07      ld   de,0x0703            | command: BOTTOM_OF_SCREEN_INFO_COMMAND, calls DISPLAY_PLAYER_SHIPS_REMAINING
063D: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
0640: 11 00 02      ld   de,0x0200            | command: DISPLAY_PLAYER_COMMAND, parameter: 0 (invokes DRAW_PLAYER_SHIP)
0643: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND
...
*
* Returns:
* HL = 0x4180  (pointer to PLAYER_ONE_STATE)
*


* Let's take a look at the default swarm definition located @0x051B. This is the "template" that is used to define the swarm when you
...
0661: CD 37 08      call 0x0837               | call HANDLE_PLAYER_MOVE
0664: CD 98 08      call 0x0898               | call HANDLE_PLAYER_BULLET
0667: CD 74 0A      call 0x0A74               | call HANDLE_ENEMY_BULLETS
066A: CD C3 0C      call 0x0CC3               | call HANDLE_INFLIGHT_ALIENS
066D: CD BE 0B      call 0x0BBE               | call HANDLE_INFLIGHT_ALIEN_SPRITE_UPDATE
0670: CD 32 0A      call 0x0A32               | call HANDLE_PLAYER_SHOOT
0673: CD 0B 0B      call 0x0B0B               | call HANDLE_SWARM_ALIEN_TO_PLAYER_BULLET_COLLISION_DETECTION
0676: CD 77 0B      call 0x0B77               | call HANDLE_PLAYER_TO_ENEMY_BULLET_COLLISION_DETECTION
0679: CD 27 12      call 0x1227               | call HANDLE_INFLIGHT_ALIEN_TO_PLAYER_BULLET_COLLISION_DETECTION
067C: CD 9E 12      call 0x129E               | call HANDLE_PLAYER_TO_INFLIGHT_ALIEN_COLLISION_DETECTION
067F: CD E5 08      call 0x08E5               | call HANDLE_PLAYER_BULLET_EXPIRED
0682: CD 0C 14      call 0x140C               | call HANDLE_FLAGSHIP_ATTACK
0685: CD 44 13      call 0x1344               | call HANDLE_SINGLE_ALIEN_ATTACK
0688: CD E1 13      call 0x13E1               | call SET_ALIEN_ATTACK_FLANK
...
06A0: CD F4 15      call 0x15F4               | call HANDLE_CALC_INFLIGHT_ALIEN_SHOOTING_DISTANCE
06A3: CD 21 16      call 0x1621               | call CHECK_IF_LEVEL_IS_COMPLETE
06A6: CD 37 16      call 0x1637               | call HANDLE_LEVEL_COMPLETE
06A9: CD B8 16      call 0x16B8               | call HANDLE_ALIEN_AGGRESSIVENESS
06AC: CD 88 16      call 0x1688               | call HANDLE_SHOCKED_SWARM
06AF: CD 8E 19      call 0x198E               | call HANDLE_SIMULATE_PLAYER_IN_ATTRACT_MODE
...
06B2: 3A 08 42      ld   a,(0x4208)           | read HAS_PLAYER_BULLET_BEEN_FIRED flag
06B5: 2A 00 42      ld   hl,(0x4200)          | read both HAS_PLAYER_SPAWNED and IS_PLAYER_DYING flags in one go              
...
06BC: 3A 25 42      ld   a,(0x4225)           | read HAVE_NO_INFLIGHT_OR_DYING_ALIENS
...
06C1: 21 60 42      ld   hl,0x4260            | load HL with address of ENEMY_BULLETS_START
06C4: 11 05 00      ld   de,0x0005            | sizeof (ENEMY_BULLET)
06C7: 06 0E         ld   b,0x0E               | #0x0E (14 decimal) bullets max to process
06C9: AF            xor  a                   | clear A
06CA: B6            or   (hl)                | read ENEMY_BULLET.IsActive flag. set A to 1 if bullet is active. 
06CB: 19            add  hl,de
06CC: 10 FC         djnz 0x06CA               | repeat until B==0
...
06D0: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
06D3: 35            dec  (hl)
06D4: C0            ret  nz
...
06D8: 21 0A 40      ld   hl,0x400A            | load HL with pointer to SCRIPT_STAGE

06DB: 3A 1D 42      ld   a,(0x421D)           | read PLAYER_LIVES 
06DE: A7            and  a                   | test if zero
06DF: 20 20         jr   nz,0x0701            | if player 1 has lives remaining, goto 0x0701

* OK, player 1 has no lives. What about player 2?
06E1: 3A B5 41      ld   a,(0x41B5)           | read PLAYER_TWO_LIVES
06E4: A7            and  a                   | test if zero
06E5: 28 3B         jr   z,0x0722             | if player 2 has no lives, goto GAME_OVER

* player 1 has no lives. If its not a 2 player game, then its GAME OVER.
06E7: 3A 0E 40      ld   a,(0x400E)           | read IS_TWO_PLAYER_GAME
06EA: A7            and  a                   | test if zero
06EB: 28 35         jr   z,0x0722             | if zero, it's not a 2 player game, goto GAME_OVER

* OK, its a 2 player game and player 2 has some lives. 
06ED: 34            inc  (hl)                | bump to next stage of the script
06EE: 2D            dec  l                   | now HL points to 0x4009 (TEMP_COUNTER_2)
06EF: 36 82         ld   (hl),0x82            | set counter 
                                                       
06F1: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY
06F4: 0F            rrca                     | move flag into carry
06F5: D0            ret  nc                  | return if game is not in play

* Display PLAYER ONE GAME OVER
06F6: 11 02 06      ld   de,0x0602            | command: PRINT_TEXT, parameter: 2 (index of "PLAYER ONE")
06F9: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
06FC: 1E 00         ld   e,0x00               | index of "GAME OVER"
06FE: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND

* Player one's been killed. Is it player two's turn now? 
0701: 3A B5 41      ld   a,(0x41B5)           | read PLAYER_TWO_LIVES
0704: A7            and  a                   | test if zero
0705: 28 0B         jr   z,0x0712             | 
0707: 3A 0E 40      ld   a,(0x400E)           | read IS_TWO_PLAYER_GAME
070A: A7            and  a                   | test if flag is set
070B: 28 05         jr   z,0x0712             | if its not a two player game goto 0x0712
070D: 34            inc  (hl)                | bump to next stage of the script
070E: 2D            dec  l                   | now HL points to 0x4009 (TEMP_COUNTER_2)
070F: 36 50         ld   (hl),0x50            | set counter
0711: C9            ret

* If we get here, either its a single player game or player two has lost all their lives.
0712: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY
0715: 0F            rrca                     | move bit 0 into carry
0716: 30 05         jr   nc,0x071D            | if carry is not set, game is not in play, goto 0x071D
0718: 36 04         ld   (hl),0x04            | set SCRIPT_STAGE to 4
071A: C3 0E 07      jp   0x070E

071D: 36 0E         ld   (hl),0x0E            |set SCRIPT_STAGE to 0x0E (14 decimal) 
071F: C3 0E 07      jp   0x070E
...
GAME_OVER:
0722: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY
0725: 0F            rrca                     | move bit 0 into carry
0726: 30 E5         jr   nc,0x070D            | if carry is not set, game is not in play, goto 0x070D
0728: 3E 01         ld   a,0x01
072A: 32 05 40      ld   (0x4005),a           | set SCRIPT_NUMBER to 1
072D: AF            xor  a
072E: 32 06 40      ld   (0x4006),a           | clear IS_GAME_IN_PLAY
0731: 32 0A 40      ld   (0x400A),a           | clear SCRIPT_STAGE
0734: CD B5 1C      call 0x1CB5               | call RESET_SOUND
0737: 11 00 06      ld   de,0x0600            | command: PRINT_TEXT, parameter: 0 (index of "GAME OVER")
073A: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND  
...
073D: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
0740: 35            dec  (hl)                | decrement counter
0741: C0            ret  nz                  | return if count !=0
0742: 2C            inc  l                   | bump HL to point to SCRIPT_STAGE
0743: AF            xor  a
0744: 77            ld   (hl),a              
0745: 32 22 42      ld   (0x4222),a           | reset LEVEL_COMPLETE flag
0748: 32 2B 42      ld   (0x422B),a           | reset IS_FLAGSHIP_HIT flag

* preserve state of the swarm so that it can be restored for player one's next turn. 
074B: 11 80 41      ld   de,0x4180            | load DE with pointer to PLAYER_ONE_PACKED_SWARM_DEF
074E: CD 64 07      call 0x0764               | call PACK_ALIEN_SWARM

* save rest of player state so it can be restored too.
0751: 21 18 42      ld   hl,0x4218            | load HL with address of CURRENT_PLAYER_STATE
0754: 01 08 00      ld   bc,0x0008
0757: ED B0         ldir                     | update player one's state

* OK, now execute script to handle player two.
0759: 3E 01         ld   a,0x01
075B: 32 0D 40      ld   (0x400D),a           | set CURRENT_PLAYER to 1 (Player TWO)
075E: 3E 04         ld   a,0x04
0760: 32 05 40      ld   (0x4005),a           | set SCRIPT_NUMBER to 4
0763: C9            ret
...
PACK_ALIEN_SWARM:
0764: 21 00 41      ld   hl,0x4100            | load HL with address of ALIEN_SWARM_FLAGS
0767: 06 10         ld   b,0x10               | buffer is 0x10 (16 decimal) bytes in length
0769: 0E 01         ld   c,0x01               
076B: AF            xor  a                   
076C: CB 46         bit  0,(hl)              
076E: 28 01         jr   z,0x0771             
0770: B1            or   c                   
0771: 23            inc  hl                  
0772: CB 01         rlc  c                   
0774: 30 F6         jr   nc,0x076C            
0776: 12            ld   (de),a        
0777: 13            inc  de            
0778: 10 F1         djnz 0x076B
077A: C9            ret
...
SCRIPT_FOUR:
077B: CD 0D 09      call 0x090D               | call HANDLE_SWARM_MOVEMENT
077E: CD 8E 09      call 0x098E               | call SET_ALIEN_PRESENCE_FLAGS
0781: 3A 0A 40      ld   a,(0x400A)
0784: EF            rst  0x28                 | jump to code @ 0x0785 + (A*2)

0785: 
      50 05         | 0x0550   
      83 05         | 0x0583 (CLEAR_ROW_OF_SCREEN)
      95 07         | 0x0795 (PLAYER_TWO_INIT)
      05 06         | 0x0605 (CLEAR_PLAYER_TEXT)
      14 06         | 0x0614 (HANDLE_SPAWN_PLAYER)
      61 06         | 0x0661 (HANDLE_MAIN_GAME_LOGIC)
      E8 07         | 0x07E8 (HANDLE_PLAYER_TWO_KILLED)
      18 08         | 0x0818 (SWITCH_TO_PLAYER_ONE)
...
PLAYER_TWO_INIT:
* restore alien swarm to what it was last turn 
0795: 11 A0 41      ld   de,0x41A0            | address of PLAYER_TWO_PACKED_SWARM_DEF
0798: CD 46 06      call 0x0646               | call UNPACK_ALIEN_SWARM
* copy player two's state (ie: game settings) to current player state
079B: EB            ex   de,hl               | now HL = pointer to PLAYER_TWO_STATE
079C: 11 18 42      ld   de,0x4218            | load DE with address of CURRENT_PLAYER_STATE 
079F: 01 08 00      ld   bc,0x0008
07A2: ED B0         ldir                     | move player 2's state to CURRENT_PLAYER_STATE 

* reset any game settings 
07A4: AF            xor  a
07A5: 32 5F 42      ld   (0x425F),a           | set TIMING_VARIABLE
07A8: 32 20 42      ld   (0x4220),a           | clear HAVE_NO_ALIENS_IN_SWARM flag.

* if the cabinet is cocktail, flip the screen for player two
07AB: 3A 0F 40      ld   a,(0x400F)           | read IS_COCKTAIL
07AE: A7            and  a                   | test if its zero (meaning UPRIGHT)
07AF: 28 09         jr   z,0x07BA             | if its upright, goto 0x07BA          
07B1: 32 18 40      ld   (0x4018),a           | set DISPLAY_IS_COCKTAIL_P2 to true
07B4: 32 06 70      ld   (0x7006),a           | enable "regen hflip" which hardware flips the screen horizontally 
07B7: 32 07 70      ld   (0x7007),a           | enable "regen vflip" which hardware flips the screen vertically

07BA: 21 0A 40      ld   hl,0x400A            | load HL with address of SCRIPT_STAGE
07BD: 34            inc  (hl)                | advance to next stage
07BE: 2D            dec  l                   | bump HL to point to TEMP_COUNTER_2
07BF: 36 96         ld   (hl),0x96            | set counter
07C1: 21 30 08      ld   hl,0x0830
07C4: 22 45 42      ld   (0x4245),hl          | set FLAGSHIP_ATTACK_MASTER_COUNTER_1 and FLAGSHIP_ATTACK_MASTER_COUNTER_2

* if its not demo mode, display the scores
07C7: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY
07CA: 0F            rrca                     | move bit 0 into carry
07CB: D0            ret  nc                  | return if game is not in play
07CC: 11 03 05      ld   de,0x0503            | command: DISPLAY_SCORE_COMMAND, parameter: 3 (invokes DISPLAY_ALL_SCORES)
07CF: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
07D2: 11 03 06      ld   de,0x0603            | command: PRINT_TEXT, parameter: 3 (index of "PLAYER TWO")
07D5: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
07D8: 1C            inc  e                   | index of "HIGH SCORE" 
07D9: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
07DC: 11 03 07      ld   de,0x0703            | command: BOTTOM_OF_SCREEN_INFO_COMMAND, parameter: 3 (invokes DISPLAY_PLAYER_SHIPS_REMAINING)
07DF: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
07E2: 11 00 07      ld   de,0x0700            | command: BOTTOM_OF_SCREEN_INFO_COMMAND, parameter: 0 (invokes DISPLAY_LEVEL_FLAGS) 
07E5: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND
...
07E8: 21 0A 40      ld   hl,0x400A            | load HL with address of SCRIPT_STAGE
07EB: 3A 1D 42      ld   a,(0x421D)           | read PLAYER_LIVES
07EE: A7            and  a                   | test if zero 
07EF: 20 1B         jr   nz,0x080C            | if player 2 has lives remaining, goto 0x080C

* OK, player 1 has no lives. What about player 1?
07F1: 3A 95 41      ld   a,(0x4195)           | read PLAYER_ONE_LIVES 
07F4: A7            and  a                   | test if zero
07F5: CA 22 07      jp   z,0x0722             | if player one is out of lives then that means GAME OVER for both players, goto 0x0722

* player 1 has some lives left
07F8: 34            inc  (hl)                | increment SCRIPT_STAGE
07F9: 2D            dec  l                   | bump HL to point to TEMP_COUNTER_2
07FA: 36 82         ld   (hl),0x82            | set value of counter

* are we in demo mode?
07FC: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY flag 
07FF: 0F            rrca                     | move flag into carry
0800: D0            ret  nc                  | return if game is not in play

* Display PLAYER TWO GAME OVER
0801: 11 03 06      ld   de,0x0603            | command: PRINT_TEXT, parameter: 3 (index of "PLAYER TWO") 
0804: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
0807: 1E 00         ld   e,0x00               | index of text string "GAME OVER"
0809: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND

* Player two's been killed. Is it player one's turn now? 
080C: 3A 95 41      ld   a,(0x4195)           | read PLAYER_ONE_LIVES        
080F: A7            and  a                   | test if zero
0810: CA 12 07      jp   z,0x0712             | if zero, goto 0x0712
0813: 34            inc  (hl)                | increment SCRIPT_STAGE
0814: 2D            dec  l                   | bump HL to point to TEMP_COUNTER_2  
0815: 36 50         ld   (hl),0x50
0817: C9            ret
...
0818: 21 09 40      ld   hl,0x4009            | load HL with address of TEMP_COUNTER_2
081B: 35            dec  (hl)
081C: C0            ret  nz
...
0820: 32 0D 40      ld   (0x400D),a           | set CURRENT_PLAYER to 0 (player one)
0823: 3E 03         ld   a,0x03
0825: 32 05 40      ld   (0x4005),a           | set SCRIPT_NUMBER

* preserve state of the swarm so that it can be restored for player two's next turn. 
0828: 11 A0 41      ld   de,0x41A0            | Address of PLAYER_TWO_PACKED_SWARM_DEF
082B: CD 64 07      call 0x0764               | call PACK_ALIEN_SWARM to convert the swarm into bit flags and write to DE

* save rest of player state so it can be restored too.
082E: 21 18 42      ld   hl,0x4218            | load HL with address of CURRENT_PLAYER_STATE
0831: 01 08 00      ld   bc,0x0008
...
HANDLE_PLAYER_MOVE:
0837: 21 00 42      ld   hl,0x4200            | load HL with address of HAS_PLAYER_SPAWNED flag
083A: CB 46         bit  0,(hl)              | has player spawned?
083C: 28 39         jr   z,0x0877             | no, goto SPAWN_PLAYER_OR_DIE
083E: 2C            inc  l
083F: 2C            inc  l                   | now HL points to PLAYER_Y

* are we in demo mode?
0840: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY      
0843: 0F            rrca                     | move bit 0 into carry. If carry is now set, the game is in play.
0844: D2 92 08      jp   nc,0x0892            | carry not set, game is not in play, goto 0x0892 to simulate moving player ship in attract mode
0847: 3A 18 40      ld   a,(0x4018)           | read DISPLAY_IS_COCKTAIL_P2 flag
084A: 0F            rrca                     | move bit 0 into carry
084B: 38 3F         jr   c,0x088C             | if carry is set, we're in cocktail mode and it's player 2 in control. Goto 0x088C

* read movement controls
084D: 3A 10 40      ld   a,(0x4010)           | read PORT_STATE_6000
0850: 47            ld   b,a                 | save a copy in B
0851: CB 5F         bit  3,a                 | has MOVE RIGHT been pushed?
0853: 28 06         jr   z,0x085B             | no, goto TEST_JOYSTICK_PUSHED_LEFT

* player pushed stick right
0855: 7E            ld   a,(hl)              | load PLAYER_Y into A           
0856: FE 17         cp   0x17                 | compare to #0x17 (23 decimal) 
0858: 38 01         jr   c,0x085B             | if A < #0x17 then player is at far right screen edge and can go no further, goto 0x085B
...
085D: 28 06         jr   z,0x0865             | No, goto 0x0865

* player pushed stick left
085F: 7E            ld   a,(hl)              | get Y coordinate of player ship into A
0860: FE E9         cp   0xE9                 | compare to #0xE9 (233 decimal)
0862: 30 01         jr   nc,0x0865            | if A>= #0xE9 then player is at far left screen edge and can go no further, goto 0x0865
...
0867: C6 80         add  a,0x80               | add an offset. Now we have a scroll value in A.
0869: 0E 06         ld   c,0x06               | set colour of player ship
086B: 21 54 40      ld   hl,0x4054            | set hl to point to first column containing player ship pseudosprite. 

* the players ship is 2x2 characters when alive, 4x4 when its exploding. We need to set the scroll offset for the 4 columns.
086E: 06 04         ld   b,0x04               | we're doing 4 columns
...
0874: 10 FA         djnz 0x0870               | do until b==0
0876: C9            ret
...
087A: 20 06         jr   nz,0x0882            | if flag is set, player will die, goto PLAYER_EXPLOSION_INIT

* spawn player
087C: 2C            inc  l                   | otherwise, point HL to PLAYER_Y
087D: 36 00         ld   (hl),0x00            | set value of PLAYER_Y to 0
087F: C3 65 08      jp   0x0865               | and set the scroll for the player ship
...
0884: 2F            cpl                        
0885: C6 80         add  a,0x80               | add an offset. Now we have a scroll value in A.
0887: 0E 07         ld   c,0x07               | colour value (see 0x0872)
0889: C3 6B 08      jp   0x086B               | and set the scroll offsets and colour values for the exploding ship


088C: 3A 11 40      ld   a,(0x4011)           | read PORT_STATE_6800
088F: C3 50 08      jp   0x0850               | now test player 2's movement stick
...
0892: 3A 3F 42      ld   a,(0x423F)           | read bit flags from ATTRACT_MODE_FAKE_CONTROLLER.
0895: C3 50 08      jp   0x0850
...
0898: CD BC 08      call 0x08BC               | call POSITION_PLAYER_BULLET
089B: 2A 09 42      ld   hl,(0x4209)          | load value of PLAYER_BULLET_Y  into H, PLAYER_BULLET_X into L        
089E: 3A 18 40      ld   a,(0x4018)           | read DISPLAY_IS_COCKTAIL_P2
08A1: 0F            rrca                     | move bit 0 into carry. 
08A2: 38 0D         jr   c,0x08B1             | If carry is set, then player 2 is playing and its a cocktail cab: goto 0x08B1.
...
08A8: 32 9F 40      ld   (0x409F),a           | update OBJRAM_BUF_PLAYER_BULLET_X 
08AB: 7C            ld   a,h                 | load A with PLAYER_BULLET_Y
08AC: 2F            cpl
08AD: 32 9D 40      ld   (0x409D),a           | update OBJRAM_BUF_PLAYER_BULLET_Y
08B0: C9            ret
...
08B1: 7D            ld   a,l
08B2: 3D            dec  a
08B3: 32 9F 40      ld   (0x409F),a           | update OBJRAM_BUF_PLAYER_BULLET_X
08B6: 7C            ld   a,h
08B7: 2F            cpl
08B8: 32 9D 40      ld   (0x409D),a           | update OBJRAM_BUF_PLAYER_BULLET_Y
08BB: C9            ret
...
08BC: 21 08 42      ld   hl,0x4208            | pointer to HAS_PLAYER_BULLET_BEEN_FIRED flag 
08BF: CB 46         bit  0,(hl)              | test if flag is set
08C1: 23            inc  hl                  | HL now points to PLAYER_BULLET_X
08C2: 28 0F         jr   z,0x08D3             | if player has not fired, goto POSITION_PLAYER_BULLET_ABOVE_SHIP
...
08C5: D6 04         sub  0x04                 | subtract 4, moving bullet UP the screen
08C7: 77            ld   (hl),a              | update X coordinate of bullet
08C8: D6 0E         sub  0x0E                 | 
08CA: D6 04         sub  0x04h                | subtract 18 (decimal) total from X coordinate
08CC: D0            ret  nc                  | if there's no carry, then the players bullet has not reached its limit

* player bullet has gone offscreen
08CD: 3E 01         ld   a,0x01               
08CF: 32 0B 42      ld   (0x420B),a           | set IS_PLAYER_BULLET_DONE flag
08D2: C9            ret

* the player bullet isn't fired, so place it above the player ship.
POSITION_PLAYER_BULLET_ABOVE_SHIP:
08D3: 36 DC         ld   (hl),0xDC            | set PLAYER_BULLET_X
08D5: 2C            inc  l                   | point HL to PLAYER_BULLET_Y
08D6: 3A 00 42      ld   a,(0x4200)           | read HAS_PLAYER_SPAWNED flag
08D9: CB 47         bit  0,a                 | test if player has spawned
08DB: 28 05         jr   z,0x08E2             | no, player has not spawned, goto 0x08E2
08DD: 3A 02 42      ld   a,(0x4202)           | read PLAYER_Y
08E0: 77            ld   (hl),a              | and write to PLAYER_BULLET_Y
08E1: C9            ret

* player hasn't spawned. Hide the bullet!
08E2: 36 00         ld   (hl),0x00            | write 0 to PLAYER_BULLET_Y
08E4: C9            ret
...
08E5: 3A 0B 42      ld   a,(0x420B)           | read IS_PLAYER_BULLET_DONE flag
...
08EA: AF            xor  a
08EB: 32 0B 42      ld   (0x420B),a           | reset the IS_PLAYER_BULLET_DONE flag
08EE: 32 08 42      ld   (0x4208),a           | reset HAS_PLAYER_BULLET_BEEN_FIRED flag. Player can shoot again.
08F1: C9            ret
...
QUEUE_COMMAND:
08F2: E5            push hl
08F3: 26 40         ld   h,0x40               | set high byte of address
08F5: 3A A0 40      ld   a,(0x40A0)           | read CIRC_CMD_QUEUE_PTR_LO          
...
08FB: 28 0E         jr   z,0x090B             | if bit 7 not set, this entry cannot be used, goto 0x090B and exit
08FD: 72            ld   (hl),d              | write DE...
08FE: 2C            inc  l
08FF: 73            ld   (hl),e
0900: 2C            inc  l                   | to (HL)
0901: 7D            ld   a,l                 | 
0902: FE C0         cp   0xC0                 | compare low byte of address in HL to #0xC0. 
0904: 30 02         jr   nc,0x0908            | if A > #0xC0 (192 decimal) then we've not hit the end of the circular queue, goto 0x0908
0906: 3E C0         ld   a,0xC0               | otherwise, we're past the end of the queue, reset queue pointer high byte to #0xC0 (192 decimal)
0908: 32 A0 40      ld   (0x40A0),a           | update CIRC_CMD_QUEUE_PTR_LO to point to next queue entry
090B: E1            pop  hl
090C: C9            ret
...
0972: 21 28 40      ld   hl,0x4028            | pointer to attribute and column scroll data held in OBJRAM_BACK_BUF
0975: 06 09         ld   b,0x09               | we're doing 9 columns.
...
097A: 10 FB         djnz 0x0977
097C: C9            ret


MAKE_SWARM_MOVE_RIGHT:
097D: 3E 01         ld   a,0x01
097F: 32 0D 42      ld   (0x420D),a           | set SWARM_DIRECTION to 1. Swarm now moves right.
0982: C9            ret

MAKE_SWARM_MOVE_LEFT:
0983: AF            xor  a
0984: 32 0D 42      ld   (0x420D),a           | set SWARM_DIRECTION to 0. Swarm now moves left.
0987: C9            ret
...
0B13: FE 68         cp   0x68                 | 
0B15: D0            ret  nc                  | if X coordinate >= #0x68 (104 decimal) then bullet is not near swarm yet, so exit
...
0BBE: 3A 18 40      ld   a,(0x4018)           | read DISPLAY_IS_COCKTAIL_P2
0BC1: 0F            rrca                     | move flag into carry
0BC2: 38 2E         jr   c,0x0BF2             | if flag is set, goto 0x0BF2

0BC4: DD 21 B0 42   ld   ix,0x42B0            | load IX with address of INFLIGHT_ALIENS
0BC8: FD 21 60 40   ld   iy,0x4060            | load IY with address of OBJRAM_BACK_BUF_SPRITES
...
0BCC: 06 03         ld   b,0x03               | number of sprites to set sprite state for
0BCE: 0E 07         ld   c,0x07               | set pixel offset to 7
0BD0: CD 20 0C      call 0x0C20               | call SET_SPRITE_STATE               
0BD3: 11 20 00      ld   de,0x0020            | sizeof (INFLIGHT_ALIEN)
0BD6: DD 19         add  ix,de               | bump IX to point to next alien entry
0BD8: 11 04 00      ld   de,0x0004            | sizeof (INFLIGHT_ALIEN_SPRITE)
0BDB: FD 19         add  iy,de               | bump IY to point to next sprite entry in OBJRAM_BACK_BUF_SPRITES
0BDD: 10 F1         djnz 0x0BD0

* for the next 5 alien sprites, their Y coordinates need to be offset 8 pixels vertically.
0BDF: 06 05         ld   b,0x05               | number of sprites to set sprite state for
0BE1: 0C            inc  c                   | adjust pixel offset to be 8
0BE2: CD 20 0C      call 0x0C20               | call SET_SPRITE_STATE
0BE5: 11 20 00      ld   de,0x0020            | sizeof (INFLIGHT_ALIEN)
0BE8: DD 19         add  ix,de               | bump IX to point to next alien entry
0BEA: 11 04 00      ld   de,0x0004            | sizeof (INFLIGHT_ALIEN_SPRITE)
0BED: FD 19         add  iy,de               | bump IY to point to next sprite entry in OBJRAM_BACK_BUF_SPRITES
0BEF: 10 F1         djnz 0x0BE2
0BF1: C9            ret

* called when display is cocktail. I'm not going to look at this routine until everything else is done...
0BF2: DD 21 B0 42   ld   ix,0x42B0            | load IX with address of INFLIGHT_ALIENS
0BF6: FD 21 60 40   ld   iy,0x4060            | load IY with address of OBJRAM_BACK_BUF_SPRITES
0BFA: 06 03         ld   b,0x03
0BFC: 0E 09         ld   c,0x09
0BFE: CD 20 0C      call 0x0C20               | call SET_SPRITE_STATE
0C01: 11 20 00      ld   de,0x0020
0C04: DD 19         add  ix,de
0C06: 11 04 00      ld   de,0x0004
0C09: FD 19         add  iy,de
0C0B: 10 F1         djnz 0x0BFE
0C0D: 06 05         ld   b,0x05
0C0F: 0D            dec  c
0C10: CD 20 0C      call 0x0C20               | call SET_SPRITE_STATE
0C13: 11 20 00      ld   de,0x0020
0C16: DD 19         add  ix,de
0C18: 11 04 00      ld   de,0x0004
0C1B: FD 19         add  iy,de
0C1D: 10 F1         djnz 0x0C10
0C1F: C9            ret
...
SET_SPRITE_STATE:
0C20: DD CB 00 46   bit  0,(ix+0x00)          | test INFLIGHT_ALIEN.IsActive
0C24: CA 98 0C      jp   z,0x0C98             | if alien is not active, goto SET_INACTIVE_OR_DYING_SPRITE_STATE
0C27: DD 7E 16      ld   a,(ix+0x16)          | read INFLIGHT_ALIEN.Colour
0C2A: FD 77 02      ld   (iy+0x02),a          | write to INFLIGHT_ALIEN_SPRITE.Colour
0C2D: DD 7E 03      ld   a,(ix+0x03)          | read INFLIGHT_ALIEN.X
0C30: D6 08         sub  0x08
0C32: FD 77 03      ld   (iy+0x03),a          | write to INFLIGHT_ALIEN_SPRITE.X
0C35: DD 7E 04      ld   a,(ix+0x04)          | read INFLIGHT_ALIEN.Y 
0C38: 2F            cpl                      | flip bits  
0C39: 91            sub  c                   | subtract pixel adjustment
0C3A: FD 77 00      ld   (iy+0x00),a          | write to INFLIGHT_ALIEN_SPRITE.Y 
...
0C3D: DD 7E 05      ld   a,(ix+0x05)          | read INFLIGHT_ALIEN.AnimationFrame
0C40: A7            and  a                   | set flags 
0C41: F2 58 0C      jp   p,0x0C58

0C44: FE FA         cp   0xFA                 | compare to -6 
0C46: FA 82 0C      jp   m,0x0C82

* alien is between an angle of 90 and 180 degrees (as player sees it)
0C49: 2F            cpl
0C4A: C6 12         add  a,0x12
0C4C: F6 40         or   0x40                 | set X-Flip bit for sprite
0C4E: DD 86 0F      add  a,(ix+0x0f)          | add in INFLIGHT_ALIEN.AnimFrameStartCode
0C51: FD 77 01      ld   (iy+0x01),a          | write to INFLIGHT_ALIEN_SPRITE.Code
0C54: FD 34 03      inc  (iy+0x03)            | increment INFLIGHT_ALIEN_SPRITE.X
0C57: C9            ret

0C58: FE 06         cp   0x06
0C5A: F2 6E 0C      jp   p,0x0C6E

* alien is between an angle of 180 and 270 degrees (as player sees it)
0C5D: C6 11         add  a,0x11
0C5F: F6 C0         or   0xC0                 | set X-Flip and Y-Flip bits for sprite
0C61: DD 86 0F      add  a,(ix+0x0f)          | add in INFLIGHT_ALIEN.AnimFrameStartCode
0C64: FD 77 01      ld   (iy+0x01),a          | write to INFLIGHT_ALIEN_SPRITE.Code
0C67: FD 34 03      inc  (iy+0x03)            | increment INFLIGHT_ALIEN_SPRITE.X
0C6A: FD 34 00      inc  (iy+0x00)            | increment INFLIGHT_ALIEN_SPRITE.Y 
0C6D: C9            ret

0C6E: FE 0C         cp   0x0C
0C70: F2 90 0C      jp   p,0x0C90

* alien is between an angle of 270-360 degrees (as player sees it)
0C73: 2F            cpl
0C74: C6 1E         add  a,0x1E
0C76: F6 80         or   0x80                  | set Y-Flip bit for sprite
0C78: DD 86 0F      add  a,(ix+0x0f)           | add in INFLIGHT_ALIEN.AnimFrameStartCode
0C7B: FD 77 01      ld   (iy+0x01),a           | write to INFLIGHT_ALIEN_SPRITE.Code
0C7E: FD 34 00      inc  (iy+0x00)             | increment INFLIGHT_ALIEN_SPRITE.Y 
0C81: C9            ret

0C82: FE F4         cp   0xF4
0C84: FA 94 0C      jp   m,0x0C94

* alien is between an angle of 0-90 degrees (as player sees it)
0C87: C6 1D         add  a,0x1D
0C89: DD 86 0F      add  a,(ix+0x0f)           | add in INFLIGHT_ALIEN.AnimFrameStartCode
0C8C: FD 77 01      ld   (iy+0x01),a           | write to INFLIGHT_ALIEN_SPRITE.Code
0C8F: C9            ret

0C90: D6 18         sub  0x18
0C92: 18 AC         jr   0x0C40

0C94: C6 18         add  a,0x18
0C96: 18 A8         jr   0x0C40
...
0C98: DD CB 01 46   bit  0,(ix+0x01)          | test INFLIGHT_ALIEN.IsDying flag
0C9C: CA BA 0C      jp   z,0x0CBA             | if the alien has finally expired, goto 0x0CBA

* alien is dying
0C9F: FD 36 02 07   ld   (iy+0x02),0x07        | set INFLIGHT_ALIEN_SPRITE.Colour
0CA3: DD 7E 03      ld   a,(ix+0x03)          | read INFLIGHT_ALIEN.X
0CA6: D6 08         sub  0x08
0CA8: FD 77 03      ld   (iy+0x03),a          | set INFLIGHT_ALIEN_SPRITE.X
0CAB: DD 7E 04      ld   a,(ix+0x04)          | read INFLIGHT_ALIEN.Y 
0CAE: 2F            cpl
0CAF: 91            sub  c                   | subtract pixel adjustment value
0CB0: FD 77 00      ld   (iy+0x00),a          | set INFLIGHT_ALIEN_SPRITE.Y 
0CB3: DD 7E 12      ld   a,(ix+0x12)          | read INFLIGHT_ALIEN.DyingAnimFrameCode
0CB6: FD 77 01      ld   (iy+0x01),a          | set INFLIGHT_ALIEN_SPRITE.Code 
0CB9: C9            ret

* This alien has died. Move sprite off-screen
0CBA: FD 36 03 F8   ld   (iy+0x03),0xF8        | set INFLIGHT_ALIEN_SPRITE.X to value offscreen
0CBE: FD 36 00 F8   ld   (iy+0x00),0xF8        | set INFLIGHT_ALIEN_SPRITE.Y to value offscreen
0CC2: C9            ret
...
0CC3: DD 21 B0 42   ld   ix,0x42B0            | load IX with address of INFLIGHT_ALIENS
0CC7: 11 20 00      ld   de,0x0020            | sizeof(INFLIGHT_ALIEN)
0CCA: 06 08         ld   b,0x08               | 1 misc + 7 attacking aliens to process
0CCC: D9            exx
0CCD: CD D6 0C      call 0x0CD6               | call HANDLE_INFLIGHT_ALIEN_STAGE_OF_LIFE
0CD0: D9            exx
0CD1: DD 19         add  ix,de               | bump IX to point to next INFLIGHT_ALIEN structure
0CD3: 10 F7         djnz 0x0CCC               | do while b!=0
0CD5: C9            ret
...
0CD6: DD CB 01 46   bit  0,(ix+0x01)          | test INFLIGHT_ALIEN.IsDying flag
0CDA: C2 E4 10      jp   nz,0x10E4            | if alien is dying, goto HANDLE_INFLIGHT_ALIEN_DYING
0CDD: DD CB 00 46   bit  0,(ix+0x00)          | test INFLIGHT_ALIEN.IsActive flag 
...
0CE2: DD 7E 02      ld   a,(ix+0x02)          | read INFLIGHT_ALIEN.StageOfLife 
0CE5: EF            rst  0x28                 | jump to code @ 0x0CE6 + (A*2)
0CE6: 
      06 0D         | 0x0D06                  | INFLIGHT_ALIEN_PACKS_BAGS
      71 0D         | 0x0D71                  | INFLIGHT_ALIEN_FLIES_IN_ARC
      D1 0D         | 0x0DD1                  | INFLIGHT_ALIEN_READY_TO_ATTACK
      2B 0E         | 0x0E2B                  | INFLIGHT_ALIEN_ATTACKING_PLAYER
      6B 0E         | 0x0E6B                  | INFLIGHT_ALIEN_NEAR_BOTTOM_OF_SCREEN
      99 0E         | 0x0E99                  | INFLIGHT_ALIEN_REACHED_BOTTOM_OF_SCREEN
      07 0F         | 0x0F07                  | INFLIGHT_ALIEN_RETURNING_TO_SWARM
      3C 0F         | 0x0F3C                  | INFLIGHT_ALIEN_CONTINUING_ATTACK_RUN_FROM_TOP_OF_SCREEN 
      66 0F         | 0x0F66                  | INFLIGHT_ALIEN_FULL_SPEED_CHARGE 
      AF 0F         | 0x0FAF                  | INFLIGHT_ALIEN_ATTACKING_PLAYER_AGGRESSIVELY
      1F 10         | 0x101F                  | INFLIGHT_ALIEN_LOOP_THE_LOOP
      8E 10         | 0x108E                  | INFLIGHT_ALIEN_COMPLETE_LOOP
      91 10         | 0x1091                  | INFLIGHT_ALIEN_UNKNOWN_1091
      9B 10         | 0x109B                  | INFLIGHT_ALIEN_CONVOY_CHARGER_SET_COLOUR_POS_ANIM
      C2 10         | 0x10C2                  | INFLIGHT_ALIEN_CONVOY_CHARGER_START_SCROLL  
      D8 10         | 0x10D8                  | INFLIGHT_ALIEN_CONVOY_CHARGER_DO_SCROLL
...
0D06: DD 36 17 00   ld   (ix+0x17),0x00        | clear INFLIGHT_ALIEN.SortieCount 
0D0A: 3E 01         ld   a,0x01
0D0C: 32 C2 41      ld   (0x41C2),a           | set ENABLE_ALIEN_ATTACK_SOUND to 1.
0D0F: CD 47 11      call 0x1147               | call SET_INFLIGHT_ALIEN_START_POSITION
0D12: DD 5E 07      ld   e,(ix+0x07)          | set command parameter to INFLIGHT_ALIEN.IndexInSwarm
0D15: 16 01         ld   d,0x01               | command: DELETE_ALIEN_COMMAND
0D17: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
0D1A: 7B            ld   a,e                 | load A with INFLIGHT_ALIEN.IndexInSwarm
0D1B: E6 70         and  0x70                 | keep the row start, remove the column number
0D1D: 21 D1 1D      ld   hl,0x1DD1
0D20: 0F            rrca                     | divide the row offset...
0D21: 0F            rrca
0D22: 0F            rrca                     | .. by 8.
0D23: 5F            ld   e,a
0D24: 16 00         ld   d,0x00               | Extend A into DE
0D26: 19            add  hl,de               | HL = 0x1DD1 + (row number of alien /8)
0D27: 7E            ld   a,(hl)
0D28: DD 77 16      ld   (ix+0x16),a          | set INFLIGHT_ALIEN.Colour

0D2B: 23            inc  hl
0D2C: 7E            ld   a,(hl)
0D2D: DD 77 18      ld   (ix+0x18),a          | set INFLIGHT_ALIEN.Speed

0D30: 7B            ld   a,e
0D31: FE 0E         cp   0x0E                 | flagship?
0D33: 28 23         jr   z,0x0D58             | yes, goto 0x0D58

0D35: DD 36 0F 00   ld   (ix+0x0f),0x00        | set INFLIGHT_ALIEN.AnimFrameStartCode
0D39: DD 36 10 03   ld   (ix+0x10),0x03        | set INFLIGHT_ALIEN.TempCounter1 to speed of animation (higher number = slower)
0D3D: DD 36 11 0C   ld   (ix+0x11),0x0C        | set INFLIGHT_ALIEN.TempCounter2 to total number of animation frames 
0D41: DD 36 13 00   ld   (ix+0x13),0x00        | set INFLIGHT_ALIEN.ArcTableLsb
0D45: DD 34 02      inc  (ix+0x02)            | set INFLIGHT_ALIEN.StageOfLife to INFLIGHT_ALIEN_FLIES_IN_ARC

0D48: DD CB 06 46   bit  0,(ix+0x06)          | test INFLIGHT_ALIEN.ArcClockwise
0D4C: 20 05         jr   nz,0x0D53            | if alien will be facing right when it breaks away from swarm, goto 0x0D53
0D4E: DD 36 05 0C   ld   (ix+0x05),0x0C        | set INFLIGHT_ALIEN.AnimationFrame
0D52: C9            ret

0D53: DD 36 05 F4   ld   (ix+0x05),0xF4        | set INFLIGHT_ALIEN.AnimationFrame
0D57: C9            ret

* This code is called for flagships. We need to count how many escorts we have.
0D58: DD 36 0F 18   ld   (ix+0x0f),0x18        | set INFLIGHT_ALIEN.AnimFrameStartCode
0D5C: AF            xor  a
0D5D: DD CB 20 46   bit  0,(ix+0x20)          | test if we have an escort
0D61: 28 01         jr   z,0x0D64             | no, goto 0x0D64
0D63: 3C            inc  a                   | increment escort count 
0D64: DD CB 40 46   bit  0,(ix+0x40)          | test if we have an escort
0D68: 28 01         jr   z,0x0D6B             | no, goto 0x0D6B
0D6A: 3C            inc  a                   | increment escort count
0D6B: 32 2A 42      ld   (0x422A),a           | set FLAGSHIP_ESCORT_COUNT
0D6E: C3 39 0D      jp   0x0D39               | finalise setting up flagship
...
0D71: DD 6E 13      ld   l,(ix+0x13)          | read INFLIGHT_ALIEN.ArcTableLsb
0D74: 26 1E         ld   h,0x1E               | Now HL points to an entry in INFLIGHT_ALIEN_ARC_TABLE at 0x1E00. 
0D76: DD 7E 03      ld   a,(ix+0x03)          | read INFLIGHT_ALIEN.X 
0D79: 86            add  a,(hl)              | add in X delta from table
0D7A: DD 77 03      ld   (ix+0x03),a          | update INFLIGHT_ALIEN.X
0D7D: 2C            inc  l                   | bump HL to Y coordinate in table 
0D7E: DD CB 06 46   bit  0,(ix+0x06)          | test INFLIGHT_ALIEN.ArcClockwise
0D82: 20 24         jr   nz,0x0DA8            | if alien is facing right, goto 0x0DA8
...
0D84: DD 7E 04      ld   a,(ix+0x04)          | load A with INFLIGHT_ALIEN.Y 
0D87: 86            add  a,(hl)              | add in Y delta from table                       
0D88: DD 77 04      ld   (ix+0x04),a          | update INFLIGHT_ALIEN.Y 
0D8B: C6 07         add  a,0x07
0D8D: FE 0E         cp   0x0E                 | is the alien off-screen?
0D8F: 38 3B         jr   c,0x0DCC             | if A< #0x0E, its gone off screen, so make alien return to swarm from top of screen.
0D91: 2C            inc  l                   | bump HL to point to next X,Y coordinate pair in table
0D92: DD 75 13      ld   (ix+0x13),l          | and update INFLIGHT_ALIEN.ArcTableLsb
* Tempcounter1 = delay before changing animation frame
* Tempcounter2 = number of animation frames left to do 
0D95: DD 35 10      dec  (ix+0x10)            | decrement INFLIGHT_ALIEN.TempCounter1
0D98: C0            ret  nz
0D99: DD 36 10 04   ld   (ix+0x10),0x04        | reset INFLIGHT_ALIEN.TempCounter1
0D9D: DD 35 05      dec  (ix+0x05)            | update INFLIGHT_ALIEN.AnimationFrame to rotate the alien left
0DA0: DD 35 11      dec  (ix+0x11)            | decrement INFLIGHT_ALIEN.TempCounter2 
...
0DA4: DD 34 02      inc  (ix+0x02)            | set stage of alien's life to INFLIGHT_ALIEN_READY_TO_ATTACK
0DA7: C9            ret

* alien that is leaving swarm to attack player is arcing up and right
* HL = pointer to table defining arc
* IX = pointer to INFLIGHT_ALIEN structure
0DA8: DD 7E 04      ld   a,(ix+0x04)          | read INFLIGHT_ALIEN.Y 
0DAB: 96            sub  (hl)                | read Y delta from table and subtract from INFLIGHT_ALIEN.Y 
0DAC: DD 77 04      ld   (ix+0x04),a          | update INFLIGHT_ALIEN.Y 
0DAF: C6 07         add  a,0x07
0DB1: FE 0E         cp   0x0E                 | is the alien off-screen?
0DB3: 38 17         jr   c,0x0DCC             | if A < #0x0E, its gone off screen, so make alien return to swarm from top of screen. 
0DB5: 2C            inc  l                   | bump HL to point to next X,Y coordinate pair in table
0DB6: DD 75 13      ld   (ix+0x13),l          | and update INFLIGHT_ALIEN.ArcTableLsb
* Tempcounter1 = delay before changing animation frame
* Tempcounter2 = number of animation frames left to do 
0DB9: DD 35 10      dec  (ix+0x10)            | decrement INFLIGHT_ALIEN.TempCounter1
0DBC: C0            ret  nz
0DBD: DD 36 10 04   ld   (ix+0x10),0x04        | reset INFLIGHT_ALIEN.TempCounter1
0DC1: DD 34 05      inc  (ix+0x05)            | update INFLIGHT_ALIEN.AnimationFrame to rotate the alien right
0DC4: DD 35 11      dec  (ix+0x11)            | decrement INFLIGHT_ALIEN.TempCounter2
0DC7: C0            ret  nz                  | if we've not done all of the animation frames, exit

* OK, we've done all of the animation frames. The alien's ready to attack the player.
0DC8: DD 34 02      inc  (ix+0x02)            | move to next stage of alien's life
0DCB: C9            ret                 

* if we get here, an alien leaving the swarm has gone offscreen. It will return to the swarm from the top of the screen.
0DCC: DD 36 02 05   ld   (ix+0x02),0x05        | set INFLIGHT_ALIEN.StageOfLife 
0DD0: C9            ret
...
INFLIGHT_ALIEN_READY_TO_ATTACK:
0DD1: DD 34 03      inc  (ix+0x03)            | increment INFLIGHT_ALIEN.X
0DD4: DD 7E 07      ld   a,(ix+0x07)          | read INFLIGHT_ALIEN.IndexInSwarm
0DD7: E6 70         and  0x70                 | keep the row, remove the column
0DD9: FE 60         cp   0x60                 | is this a red alien?
0DDB: 28 43         jr   z,0x0E20             | yes, goto 0x0E20

INFLIGHT_ALIEN_DEFINE_FLIGHTPATH:
0DDD: 3A 02 42      ld   a,(0x4202)           | read PLAYER_Y 
0DE0: 47            ld   b,a
0DE1: DD 7E 04      ld   a,(ix+0x04)          | read INFLIGHT_ALIEN.Y 
0DE4: 90            sub  b                   | A = INFLIGHT_ALIEN.Y  - PLAYER_Y
0DE5: 38 28         jr   c,0x0E0F             | if alien is to right of player, goto 0x0E0F
...
0DE8: C6 10         add  a,0x10               | add 0x10 (16 decimal) to product
* clamp A between 0x30 and 0x70
0DEA: FE 30         cp   0x30                 | compare to 48 (decimal)
0DEC: 30 02         jr   nc,0x0DF0            | if A>=48 goto 0x0DF0
0DEE: 3E 30         ld   a,0x30
0DF0: FE 70         cp   0x70                 | compare to 112 (decimal)  NB: 112 is half the screen height in pixels
0DF2: 38 02         jr   c,0x0DF6
0DF4: 3E 70         ld   a,0x70
...
0DF6: DD 77 19      ld   (ix+0x19),a          | set INFLIGHT_ALIEN.PivotYValueAdd
0DF9: DD 96 04      sub  (ix+0x04)            | subtract INFLIGHT_ALIEN.Y 
0DFC: ED 44         neg
0DFE: DD 77 09      ld   (ix+0x09),a          | set INFLIGHT_ALIEN.PivotYValue. Now PivotYValue + PivotYValueAdd = INFLIGHT_ALIEN.Y
0E01: AF            xor  a
0E02: DD 77 1A      ld   (ix+0x1a),a
0E05: DD 77 1B      ld   (ix+0x1b),a
0E08: DD 77 1C      ld   (ix+0x1c),a

0E0B: DD 34 02      inc  (ix+0x02)             | set stage of life to INFLIGHT_ALIEN_ATTACKING_PLAYER or INFLIGHT_ALIEN_ATTACKING_PLAYER_AGGRESSIVELY
0E0E: C9            ret
...
0E12: FE D0         cp   0xD0                  | compare to -48 (decimal)
0E14: 38 02         jr   c,0x0E18
0E16: 3E D0         ld   a,0xD0             
0E18: FE 90         cp   0x90                  | compare to -112 (decimal)  NB: 112 is half the screen height in pixels
0E1A: 30 DA         jr   nc,0x0DF6
0E1C: 3E 90         ld   a,0x90
0E1E: 18 D6         jr   0x0DF6


0E20: 3A D0 42      ld   a,(0x42D0)           | address of INFLIGHT_ALIENS[1].IsActive
0E23: 0F            rrca                     | move flag into carry
0E24: 30 B7         jr   nc,0x0DDD            | if not set then we are not part of a convoy, goto INFLIGHT_ALIEN_DEFINE_FLIGHTPATH
...
0E26: 3A E9 42      ld   a,(0x42E9)           | read flagship INFLIGHT_ALIENS[1].PivotYValueAdd  
0E29: 18 CB         jr   0x0DF6
...
0E2B: DD 34 03      inc  (ix+0x03)            | increment INFLIGHT_ALIEN.X 
0E2E: CD 6B 11      call 0x116B               | call UPDATE_INFLIGHT_ALIEN_YADD
0E31: DD 7E 09      ld   a,(ix+0x09)          | load A with INFLIGHT_ALIEN.PivotYValue
0E34: DD 86 19      add  a,(ix+0x19)          | add in INFLIGHT_ALIEN.PivotYValueAdd to produce a Y coordinate                                                   
0E37: DD 77 04      ld   (ix+0x04),a          | write to INFLIGHT_ALIEN.Y 
0E3A: C6 07         add  a,0x07
0E3C: FE 0E         cp   0x0E
0E3E: 38 24         jr   c,0x0E64             | if the alien has gone off the side of the screen, return to swarm
0E40: DD 7E 03      ld   a,(ix+0x03)          | load A with INFLIGHT_ALIEN.X
0E43: C6 48         add  a,0x48
0E45: 38 20         jr   c,0x0E67             | if the alien is nearing the bottom of the screen, speed it up!
0E47: 3A 00 42      ld   a,(0x4200)           | read HAS_PLAYER_SPAWNED
0E4A: 0F            rrca                     | move flag into carry
0E4B: D0            ret  nc                  | return if player has not spawned
0E4C: CD B0 11      call 0x11B0               | call CALCULATE_INFLIGHT_ALIEN_LOOKAT_ANIM_FRAME

* alien won't shoot at you if a flagship has been hit
0E4F: 3A 2B 42      ld   a,(0x422B)           | read IS_FLAGSHIP_HIT
...
0E54: 2A 13 42      ld   hl,(0x4213)          | get INFLIGHT_ALIEN_SHOOT_EXACT_X into H and INFLIGHT_ALIEN_SHOOT_RANGE_MUL into L
0E57: DD 7E 03      ld   a,(ix+0x03)          | read INFLIGHT_ALIEN.X

0E5A: BC            cp   h                   | compare A to INFLIGHT_ALIEN_SHOOT_EXACT_X 
0E5B: CA E0 11      jp   z,0x11E0             | if equal, jump to TRY_SPAWN_ENEMY_BULLET
0E5E: C6 19         add  a,0x19               | add 0x19 (25 decimal) to A
0E60: 2D            dec  l                   | and try again...
0E61: 20 F7         jr   nz,0x0E5A            | until L is 0.
0E63: C9            ret
...
0E64: DD 34 02      inc  (ix+0x02)      
0E67: DD 34 02      inc  (ix+0x02)
0E6A: C9            ret
...
0E6B: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
0E6E: E6 01         and  0x01                 | ..now A is either 0 or 1.
0E70: 3C            inc  a                   | ..now A is either 1 or 2. 
0E71: DD 86 03      add  a,(ix+0x03)          | Add either 1 or 2 pixels to INFLIGHT_ALIEN.X
0E74: DD 77 03      ld   (ix+0x03),a          | and update INFLIGHT_ALIEN.X
0E77: D6 06         sub  0x06
0E79: FE 03         cp   0x03                 | has alien gone off the bottom of the screen?
0E7B: 38 18         jr   c,0x0E95             | yes, goto 0x0E95

0E7D: CD 6B 11      call 0x116B               | call UPDATE_INFLIGHT_ALIEN_YADD 
0E80: DD 7E 19      ld   a,(ix+0x19)          | read INFLIGHT_ALIEN.PivotYValueAdd        
0E83: A7            and  a                   | set flags - we are interested if its a minus value
0E84: FA 90 0E      jp   m,0x0E90             | if the PivotYValueAdd is a negative value, goto 0x0E90

0E87: DD 86 09      add  a,(ix+0x09)          | add INFLIGHT_ALIEN.PivotYValue 
0E8A: 38 09         jr   c,0x0E95             | carry flag set if alien has gone off side of screen,  goto 0x0E95

0E8C: DD 77 04      ld   (ix+0x04),a          | set INFLIGHT_ALIEN.Y 
0E8F: C9            ret

0E90: DD 86 09      add  a,(ix+0x09)          | add INFLIGHT_ALIEN.PivotYValue
0E93: 38 F7         jr   c,0x0E8C

* alien's went off the bottom or the side of the screen. 
0E95: DD 34 02      inc  (ix+0x02)            | now call INFLIGHT_ALIEN_REACHED_BOTTOM_OF_SCREEN stage of life.
0E98: C9            ret
...
0E99: DD 36 03 08   ld   (ix+0x03),0x08        | set INFLIGHT_ALIEN.X to position at very top of screen
0E9D: DD 34 17      inc  (ix+0x17)            | increment INFLIGHT_ALIEN.SortieCount
0EA0: DD 36 05 00   ld   (ix+0x05),0x00        | clear INFLIGHT_ALIEN.AnimationFrame

* what type of alien are we dealing with?
0EA4: DD 7E 07      ld   a,(ix+0x07)          | read INFLIGHT_ALIEN.IndexInSwarm  
0EA7: E6 70         and  0x70                 | remove the column number from the index, keep the row
0EA9: FE 70         cp   0x70                 | is this alien a flagship?
0EAB: 28 2D         jr   z,0x0EDA             | yes, goto INFLIGHT_ALIEN_FLAGSHIP_REACHED_BOTTOM_OF_SCREEN

*if the player has not spawned, the alien will return to the swarm.
0EAD: 3A 00 42      ld   a,(0x4200)           | read HAS_PLAYER_SPAWNED
0EB0: 0F            rrca                     | move flag into carry
0EB1: 30 23         jr   nc,0x0ED6            | if player has not spawned yet, goto 0x0ED6 - aliens return to swarm
...
0EB3: 3A 24 42      ld   a,(0x4224)           | read HAVE_AGGRESSIVE_ALIENS flag
0EB6: A7            and  a                   | test flag
0EB7: 20 06         jr   nz,0x0EBF            | if aliens are aggressive, make alien reappear at top of screen, keep attacking 
0EB9: 3A 21 42      ld   a,(0x4221)           | read HAVE_NO_BLUE_OR_PURPLE_ALIENS
0EBC: A7            and  a                   | test flag             
0EBD: 28 17         jr   z,0x0ED6             | if we do have any blue or purple aliens, goto 0x0ED6 - aliens return to swarm
...
0EBF: DD 7E 04      ld   a,(ix+0x04)          | read INFLIGHT_ALIEN.Y 
0EC2: 1F            rra                      | divide by 2 (don't worry about carry, it was cleared by AND above)
0EC3: 4F            ld   c,a                 | preserve in C              
0EC4: CD 3C 00      call 0x003C               | call GENERATE_RANDOM_NUMBER
0EC7: E6 1F         and  0x1F                 | ensure random number is between 0..31 decimal
0EC9: 81            add  a,c                 
0ECA: C6 20         add  a,0x20               
0ECC: DD 77 04      ld   (ix+0x04),a          | set INFLIGHT_ALIEN.Y   
0ECF: DD 36 10 28   ld   (ix+0x10),0x28        | set INFLIGHT_ALIEN.TempCounter1 for INFLIGHT_ALIEN_UNKNOWN_OF3C to use.
...
0ED3: DD 34 02      inc  (ix+0x02)            | increment INFLIGHT_ALIEN.StageOfLife
0ED6: DD 34 02      inc  (ix+0x02)            | increment INFLIGHT_ALIEN.StageOfLife
0ED9: C9            ret
...
0EDA: 3A 2A 42      ld   a,(0x422A)           | read FLAGSHIP_ESCORT_COUNT
0EDD: A7            and  a                   | test if flagship actually had any escort!
0EDE: 20 12         jr   nz,0x0EF2            | if flagship has escort, goto INFLIGHT_ALIEN_COUNT_FLAGSHIP_ESCORTS
...
0EE0: DD 36 00 00   ld   (ix+0x00),0x00        | reset INFLIGHT_ALIEN.IsActive
0EE4: 3A 1E 42      ld   a,(0x421E)           | read FLAGSHIP_SURVIVOR_COUNT
0EE7: 3C            inc  a                   | add another one to the survivor count!
0EE8: FE 03         cp   0x03                 | have we got 3 surviving flagships?
0EEA: 38 02         jr   c,0x0EEE             | if we have less than 3, that's OK, goto 0x0EEE

* We seem to have 3 flagships but only 2 flagships are allowed to be carried over...
0EEC: 3E 02         ld   a,0x02               | clamp surviving flagship count to 2.

0EEE: 32 1E 42      ld   (0x421E),a           | set FLAGSHIP_SURVIVOR_COUNT
0EF1: C9            ret

* count how many aliens were escorting the flagship. 
INFLIGHT_ALIEN_COUNT_FLAGSHIP_ESCORTS:
0EF2: AF            xor  a
0EF3: DD CB 20 46   bit  0,(ix+0x20)          | test IsActive flag of first escort  
0EF7: 28 01         jr   z,0x0EFA             | 
0EF9: 3C            inc  a                   | increment escort count
0EFA: DD CB 40 46   bit  0,(ix+0x40)          | test IsActive flag of second escort
0EFE: 28 01         jr   z,0x0F01
0F00: 3C            inc  a                   | increment escort count
0F01: 32 2A 42      ld   (0x422A),a           | set FLAGSHIP_ESCORT_COUNT
0F04: C3 AD 0E      jp   0x0EAD               | make flagship reappear at top of screen
...
0F07: DD 46 03      ld   b,(ix+0x03)          | keep copy of INFLIGHT_ALIEN.X in B as SET_INFLIGHT_ALIEN_START_POSITION changes it 
0F0A: 04            inc  b                   
0F0B: CD 47 11      call 0x1147               | call SET_INFLIGHT_ALIEN_START_POSITION to determine where alien needs to go  

* INFLIGHT_ALIEN.Y  and INFLIGHT_ALIEN.X have been changed by SET_INFLIGHT_ALIEN_START_POSITION
0F0E: DD 7E 03      ld   a,(ix+0x03)          | A = destination INFLIGHT_ALIEN.X
0F11: DD 70 03      ld   (ix+0x03),b          | restore INFLIGHT_ALIEN.X back to what it was before
0F14: 90            sub  b                   | OK, how far away is this alien from where it wants to be?
0F15: 28 14         jr   z,0x0F2B             | distance is zero, it's got where it wants to be, goto INFLIGHT_ALIEN_BACK_IN_SWARM
0F17: FE 19         cp   0x19                 | 25 pixels away?
0F19: D0            ret  nc                  | if distance is more than 0x19 (25 decimal), not near enough to destination, so exit
0F1A: E6 01         and  0x01                 | is distance an odd number?
...
0F1D: DD CB 06 46   bit  0,(ix+0x06)          | read INFLIGHT_ALIEN.ArcClockwise
0F21: 20 04         jr   nz,0x0F27             

0F23: DD 34 05      inc  (ix+0x05)            | update INFLIGHT_ALIEN.AnimationFrame to rotate the alien right
0F26: C9            ret

0F27: DD 35 05      dec  (ix+0x05)            | update INFLIGHT_ALIEN.AnimationFrame to rotate the alien left
0F2A: C9            ret

* alien has returned to swarm. Remove sprite and substitute sprite with characters.
INFLIGHT_ALIEN_BACK_IN_SWARM:
0F2B: DD 36 00 00   ld   (ix+0x00),0x00        | set INFLIGHT_ALIEN.IsActive to 0 - will hide sprite (see 0x0C98)
0F2F: 26 41         ld   h,0x41               | MSB of ALIEN_SWARM_FLAGS address 
0F31: DD 6E 07      ld   l,(ix+0x07)          | Now HL = pointer to address in ALIEN_SWARM_FLAGS where alien belongs
0F34: 16 00         ld   d,0x00               | command: DRAW_ALIEN_COMMAND
0F36: 36 01         ld   (hl),0x01            | mark flag in ALIEN_SWARM_FLAGS as "occupied". Our alien's back in the swarm!
0F38: 5D            ld   e,l                 | E = index of alien in swarm
0F39: C3 F2 08      jp   0x08F2               | jump to QUEUE COMMAND. Alien will be drawn in its place in the swarm.
...
0F3C: DD 34 03      inc  (ix+0x03)            | increment INFLIGHT_ALIEN.X

* 
0F3F: 3A 02 42      ld   a,(0x4202)           | read PLAYER_Y      
0F42: DD 96 04      sub  (ix+0x04)            | subtract INFLIGHT_ALIEN.Y 
...
0F4F: DD 66 04      ld   h,(ix+0x04)
0F52: DD 6E 09      ld   l,(ix+0x09)          | INFLIGHT_ALIEN.PivotYValue
...
0F58: DD 74 04      ld   (ix+0x04),h          | update INFLIGHT_ALIEN.Y 
0F5B: DD 75 09      ld   (ix+0x09),l          | update INFLIGHT_ALIEN.PivotYValue
0F5E: DD 35 10      dec  (ix+0x10)            | counter was set @ 0x0ECF
0F61: C0            ret  nz

0F62: DD 34 02      inc  (ix+0x02)            | set INFLIGHT_ALIEN.StageOfLife to INFLIGHT_ALIEN_FULL_SPEED_CHARGE
0F65: C9            ret
...
0F66: DD 34 03      inc  (ix+0x03)            | increment INFLIGHT_ALIEN.X

* first check the X coordinate to see if the alien is in the centre 
0F69: DD 7E 03      ld   a,(ix+0x03)          | read INFLIGHT_ALIEN.X 
0F6C: D6 60         sub  0x60                                         
0F6E: FE 40         cp   0x40
0F70: 30 09         jr   nc,0x0F7B            | if INFLIGHT_ALIEN.X-0x60 >= 0x40, we're not centre horizontally   

* next thing we need to do is check if we have enough space for a loop.
0F72: DD 7E 04      ld   a,(ix+0x04)          | read INFLIGHT_ALIEN.Y 
0F75: D6 60         sub  0x60
0F77: FE 40         cp   0x40
0F79: 38 0C         jr   c,0x0F87             | yes, we have space, make alien loop the loop

* otherwise, make the alien veer erratically. 
0F7B: CD DD 0D      call 0x0DDD               | call INFLIGHT_ALIEN_DEFINE_FLIGHTPATH
0F7E: DD 36 18 03   ld   (ix+0x18),0x03        | set INFLIGHT_ALIEN.Speed to maximum!
0F82: DD 36 10 64   ld   (ix+0x10),0x64        | set INFLIGHT_ALIEN.TempCounter1
0F86: C9            ret
                     
 
0F87: DD 34 02      inc  (ix+0x02)
0F8A: DD 34 02      inc  (ix+0x02)            | set INFLIGHT_ALIEN.StageOfLife to INFLIGHT_ALIEN_LOOP_THE_LOOP 
0F8D: DD 36 10 03   ld   (ix+0x10),0x03        | set INFLIGHT_ALIEN.TempCounter1 to delay before changing animation frame
0F91: DD 36 11 0C   ld   (ix+0x11),0x0C        | set INFLIGHT_ALIEN.TempCounter2 to number of animation frames in total
0F95: DD 36 05 00   ld   (ix+0x05),0x00        | set INFLIGHT_ALIEN.AnimationFrame
0F99: DD 36 13 00   ld   (ix+0x13),0x00        | set INFLIGHT_ALIEN.ArcTableLsb 
0F9D: 3A 02 42      ld   a,(0x4202)           | read PLAYER_Y 
0FA0: DD 96 04      sub  (ix+0x04)            | subtract INFLIGHT_ALIEN.Y 
0FA3: 38 05         jr   c,0x0FAA             | if player to right of alien, make alien loop the loop clockwise

* alien will perform an anti-clockwise loop
0FA5: DD 36 06 00   ld   (ix+0x06),0x00        | reset INFLIGHT_ALIEN.ArcClockwise
0FA9: C9            ret

0FAA: DD 36 06 01   ld   (ix+0x06),0x01        | set INFLIGHT_ALIEN.ArcClockwise 
0FAE: C9            ret
...
0FAF: DD 34 03      inc  (ix+0x03)            | increment INFLIGHT_ALIEN.X
0FB2: CD 6B 11      call 0x116B               | call UPDATE_INFLIGHT_ALIEN_YADD
0FB5: DD 7E 17      ld   a,(ix+0x17)          | read INFLIGHT_ALIEN.SortieCount           
0FB8: FE 04         cp   0x04                 | has the alien made it past the player 4 times?
0FBA: 28 48         jr   z,0x1004             | yes, exactly 4 times, *maybe* make alien closer to player                                                                 
0FBC: 30 4D         jr   nc,0x100B            | more than 4 times, make aliens hug player closer 

0FBE: DD 7E 09      ld   a,(ix+0x09)          | INFLIGHT_ALIEN.PivotYValue
0FC1: DD 86 19      add  a,(ix+0x19)          | Add INFLIGHT_ALIEN.PivotYValueAdd 
0FC4: DD 77 04      ld   (ix+0x04),a          | set INFLIGHT_ALIEN.Y 

* has alien wandered off left or right side of screen as player sees it?
0FC7: C6 07         add  a,0x07
0FC9: FE 0E         cp   0x0E
0FCB: 38 29         jr   c,0x0FF6             | alien has gone off side of screen, goto 0x0FF6

* is alien near bottom of the screen?
0FCD: DD 7E 03      ld   a,(ix+0x03)          | read INFLIGHT_ALIEN.X
0FD0: C6 40         add  a,0x40               | 
0FD2: 38 27         jr   c,0x0FFB             | if adding 0x40 pixels to X gives a result >255, then alien is near bottom of screen, goto 0x0FFB
0FD4: DD 35 10      dec  (ix+0x10)
0FD7: 28 27         jr   z,0x1000
0FD9: 3A 00 42      ld   a,(0x4200)           | read HAS_PLAYER_SPAWNED
0FDC: 0F            rrca                     | move flag into carry
0FDD: D0            ret  nc                  | return if player has not spawned
0FDE: CD B0 11      call 0x11B0               | call CALCULATE_INFLIGHT_ALIEN_LOOKAT_ANIM_FRAME
0FE1: 3A 2B 42      ld   a,(0x422B)           | read IS_FLAGSHIP_HIT
...
0FE6: 2A 13 42      ld   hl,(0x4213)          | get INFLIGHT_ALIEN_SHOOT_EXACT_X into H and INFLIGHT_ALIEN_SHOOT_RANGE_MUL into L
0FE9: DD 7E 03      ld   a,(ix+0x03)          
0FEC: BC            cp   h
0FED: CA E0 11      jp   z,0x11E0             | jump to TRY_SPAWN_ENEMY_BULLET
0FF0: C6 19         add  a,0x19
0FF2: 2D            dec  l                   
0FF3: 20 F7         jr   nz,0x0FEC
0FF5: C9            ret

0FF6: DD 36 02 05   ld   (ix+0x02),0x05        | set INFLIGHT_ALIEN.StageOfLife to INFLIGHT_ALIEN_REACHED_BOTTOM_OF_SCREEN
0FFA: C9            ret

0FFB: DD 36 02 04   ld   (ix+0x02),0x04        | set INFLIGHT_ALIEN.StageOfLife to INFLIGHT_ALIEN_NEAR_BOTTOM_OF_SCREEN
0FFF: C9            ret

1000: DD 35 02      dec  (ix+0x02)
1003: C9            ret


* If we get here, the alien has survived exactly 4 continuous sorties.     
1004: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
1007: E6 01         and  0x01                 | is the number odd?
1009: 28 B3         jr   z,0x0FBE             | no, the number's even, it's business as usual, goto 0x0FBE
...
100B: 3A 02 42      ld   a,(0x4202)           | read PLAYER_Y 
100E: DD 96 09      sub  (ix+0x09)            | subtract INFLIGHT_ALIEN.PivotYValue
1011: 38 06         jr   c,0x1019             if a carry occurred, alien is, as player sees it, to left of player ship

* Make the alien's pivot Y coordinate a bit closer to the player...
1013: DD 34 09      inc  (ix+0x09)            | Update INFLIGHT_ALIEN.PivotYValue
1016: C3 BE 0F      jp   0x0FBE

1019: DD 35 09      dec  (ix+0x09)            | Update INFLIGHT_ALIEN.PivotYValue
101C: C3 BE 0F      jp   0x0FBE
...
101F: DD 6E 13      ld   l,(ix+0x13)          | load L with INFLIGHT_ALIEN.ArcTableLsb
1022: 26 1E         ld   h,0x1E               | MSB of INFLIGHT_ALIEN_ARC_TABLE                 

* Now HL is a pointer to an entry in the INFLIGHT_ALIEN_ARC_TABLE (see docs @ 0x1E00)
1024: DD 7E 03      ld   a,(ix+0x03)          | load INFLIGHT_ALIEN.X          
1027: 96            sub  (hl)                | subtract X component from table  
1028: DD 77 03      ld   (ix+0x03),a

102B: 2C            inc  l

102C: DD CB 06 46   bit  0,(ix+0x06)          | is this alien going to do a clockwise loop?
1030: 20 2E         jr   nz,0x1060            | yes, goto INFLIGHT_ALIEN_LOOPING_CLOCKWISE

* Alien is performing a counter-clockwise loop-the-loop maneuvre
1032: DD 7E 04      ld   a,(ix+0x04)          | read INFLIGHT_ALIEN.Y
1035: 96            sub  (hl)                | subtract Y component from INFLIGHT_ALIEN_ARC_TABLE
1036: DD 77 04      ld   (ix+0x04),a          | set INFLIGHT_ALIEN.Y 
1039: 2C            inc  l                   | bump HL to point to next X,Y pair in INFLIGHT_ALIEN_ARC_TABLE
103A: DD 75 13      ld   (ix+0x13),l          | set INFLIGHT_ALIEN.ArcTableLsb
103D: DD 35 10      dec  (ix+0x10)            | decrement INFLIGHT_ALIEN.TempCounter1 
1040: C0            ret  nz

* When INFLIGHT_ALIEN.TempCounter1 counts down to zero, its time to change the animation frame
1041: DD 36 10 04   ld   (ix+0x10),0x04        | reset INFLIGHT_ALIEN.TempCounter1 
1045: DD 35 05      dec  (ix+0x05)            | change sprite frame to appear to rotate LEFT

* INFLIGHT_ALIEN.TempCounter2 is used to count down number of animation frames left
1048: DD 35 11      dec  (ix+0x11)            | decrement INFLIGHT_ALIEN.TempCounter2
...
104C: DD 34 02      inc  (ix+0x02)            | bump INFLIGHT_ALIEN.StageOfLife to INFLIGHT_ALIEN_COMPLETE_LOOP
104F: DD 36 10 03   ld   (ix+0x10),0x03        | set INFLIGHT_ALIEN.TempCounter1 to delay before changing animation frame
1053: DD 36 11 0C   ld   (ix+0x11),0x0C        | set INFLIGHT_ALIEN.TempCounter2 to number of animation frames
1057: DD 36 05 0C   ld   (ix+0x05),0x0C        | set INFLIGHT_ALIEN.AnimationFrame
105B: DD 36 13 00   ld   (ix+0x13),0x00        | set INFLIGHT_ALIEN.ArcTableLsb
105F: C9            ret


* Alien is performing a clockwise loop-the-loop maneuvre
INFLIGHT_ALIEN_LOOPING_CLOCKWISE:
1060: DD 7E 04      ld   a,(ix+0x04)          | read INFLIGHT_ALIEN.Y
1063: 86            add  a,(hl)              | add Y component from INFLIGHT_ALIEN_ARC_TABLE
1064: DD 77 04      ld   (ix+0x04),a          | set INFLIGHT_ALIEN.Y
1067: 2C            inc  l                   | bump HL to point to next X,Y pair in INFLIGHT_ALIEN_ARC_TABLE
1068: DD 75 13      ld   (ix+0x13),l          | set INFLIGHT_ALIEN.ArcTableLsb
106B: DD 35 10      dec  (ix+0x10)
106E: C0            ret  nz

* When INFLIGHT_ALIEN.TempCounter1 counts down to zero, its time to change the animation frame
106F: DD 36 10 04   ld   (ix+0x10),0x04        reset INFLIGHT_ALIEN.TempCounter1 
1073: DD 34 05      inc  (ix+0x05)            | change sprite frame to appear to rotate RIGHT
1076: DD 35 11      dec  (ix+0x11)
1079: C0            ret  nz

* we've done 270 degrees rotation, hand off the remaining 90 to the INFLIGHT_ALIEN_FLIES_IN_ARC
107A: DD 34 02      inc  (ix+0x02)            | bump INFLIGHT_ALIEN.StageOfLife to INFLIGHT_ALIEN_COMPLETE_LOOP
107D: DD 36 10 03   ld   (ix+0x10),0x03        | set INFLIGHT_ALIEN.TempCounter1 to delay before changing animation frame
1081: DD 36 11 0C   ld   (ix+0x11),0x0C        | set INFLIGHT_ALIEN.TempCounter2 to number of animation frames
1085: DD 36 05 F4   ld   (ix+0x05),0xF4        | set INFLIGHT_ALIEN.AnimationFrame
1089: DD 36 13 00   ld   (ix+0x13),0x00        | set INFLIGHT_ALIEN.ArcTableLsb
108D: C9            ret


INFLIGHT_ALIEN_COMPLETE_LOOP:
108E: C3 71 0D      jp   0x0D71               | jump to INFLIGHT_ALIEN_FLIES_IN_ARC


INFLIGHT_ALIEN_UNKNOWN_1091:
1091: DD 34 03      inc  (ix+0x03)            | update INFLIGHT_ALIEN.X
1094: DD 36 02 08   ld   (ix+0x02),0x08        | set INFLIGHT_ALIEN.StageOfLife to INFLIGHT_ALIEN_FULL_SPEED_CHARGE
1098: C3 7B 0F      jp   0x0F7B               
...
109B: DD 7E 07      ld   a,(ix+0x07)          | read INFLIGHT_ALIEN.IndexInSwarm
...
10A3: DD 77 16      ld   (ix+0x16),a          | set INFLIGHT_ALIEN.Colour
10A6: 07            rlca                     | multiply A..
10A7: 07            rlca
10A8: 07            rlca
10A9: 07            rlca                     | .. by 16
10AA: C6 8C         add  a,0x8C               | X coord = 0x8C + (A * 16)
10AC: DD 77 03      ld   (ix+0x03),a          | set INFLIGHT_ALIEN.X
10AF: DD 36 10 18   ld   (ix+0x10),0x18        | set INFLIGHT_ALIEN.TempCounter1
10B3: DD 34 02      inc  (ix+0x02)            | advance alien to its next stage of life
10B6: DD 36 0F 00   ld   (ix+0x0f),0x00        | set INFLIGHT_ALIEN.AnimFrameStartCode
10BA: 78            ld   a,b
...
10BD: DD 36 0F 18   ld   (ix+0x0f),0x18        | set INFLIGHT_ALIEN.AnimFrameStartCode
10C1: C9            ret
...
10C2: DD 34 04      inc  (ix+0x04)            | keep incrementing INFLIGHT_ALIEN.Y..            
10C5: DD 35 10      dec  (ix+0x10)            | ..until this counter hits zero.   
10C8: C0            ret  nz

10C9: DD 7E 07      ld   a,(ix+0x07)          | read INFLIGHT_ALIEN.IndexInSwarm
10CC: C6 4B         add  a,0x4B               | set bit 6 to indicate the text needs to be scrolled on
10CE: 5F            ld   e,a
10CF: 16 06         ld   d,0x06               | command is PRINT_TEXT
10D1: CD F2 08      call 0x08F2               | call QUEUE_COMMAND
10D4: DD 34 02      inc  (ix+0x02)            | advance to next stage of alien's life
10D7: C9            ret
...
10D8: DD 7E 04      ld   a,(ix+0x04)          | read INFLIGHT_ALIEN.Y
10DB: D6 C8         sub  0xC8
10DD: FE 05         cp   0x05                  
10DF: D8            ret  c
10E0: DD 34 04      inc  (ix+0x04)            | update INFLIGHT_ALIEN.Y
10E3: C9            ret
...
10E4: DD 7E 02      ld   a,(ix+0x02)
10E7: EF            rst  0x28                 | jump to code @ 0x10E8 + (A*2)
10E8: 

      F0 10         | 0x10F0                  | INFLIGHT_ALIEN_DYING_SETUP_ANIM_AND_SOUND    
      12 11         | 0x1112                  | INFLIGHT_ALIEN_DYING_DISPLAY_EXPLOSION
      3D 11         | 0x113D                  | INFLIGHT_ALIEN_DYING_FINALLY_BUYS_FARM
      46 11         | 0x1146                  | just a pointer to a RET command
...
10F0: DD 36 10 04   ld   (ix+0x10),0x04        | set INFLIGHT_ALIEN.TempCounter1 to speed of death animation (lower = faster)
10F4: DD 36 11 04   ld   (ix+0x11),0x04        | set INFLIGHT_ALIEN.TempCounter2 to number of times to repeat death animation
10F8: DD 36 12 1C   ld   (ix+0x12),0x1C        | set INFLIGHT_ALIEN.DyingAnimFrameCode
10FC: DD 34 02      inc  (ix+0x02)            | bump INFLIGHT_ALIEN.StageOfLife to next stage
10FF: DD 7E 07      ld   a,(ix+0x07)          | read INFLIGHT_ALIEN.IndexInSwarm
1102: FE 70         cp   0x70                 | is this a flagship?
1104: 30 06         jr   nc,0x110C            | yes, goto 0x110C to play flagship hit sound
1106: 3E 07         ld   a,0x07
1108: 32 DF 41      ld   (0x41DF),a           | play sound effect of alien hit
110B: C9            ret

* play flagship hit sound
110C: 3E 17         ld   a,0x17
110E: 32 DF 41      ld   (0x41DF),a           | play sound effect of flagship hit
1111: C9            ret
...
1112: DD 35 10      dec  (ix+0x10)            | decrement INFLIGHT_ALIEN.TempCounter1 
1115: C0            ret  nz                  | if counter hasn't reached zero, not time to update explosion animation frame yet
1116: DD 36 10 04   ld   (ix+0x10),0x04        | reset animation counter. Higher number = slower explosion animation speed
111A: DD 34 12      inc  (ix+0x12)            | bump INFLIGHT_ALIEN.DyingAnimFrameCode to next frame
111D: DD 35 11      dec  (ix+0x11)            | decrement INFLIGHT_ALIEN.TempCounter2 which holds count of frames left to show 
1120: C0            ret  nz                  | if we've not shown all the explosion frames, exit

1121: DD 7E 07      ld   a,(ix+0x07)          | read INFLIGHT_ALIEN.IndexInSwarm
1124: FE 70         cp   0x70                 | is this a flagship?
1126: 30 05         jr   nc,0x112D            | yes, goto DISPLAY_FLAGSHIP_POINTS_VALUE
1128: DD 36 01 00   ld   (ix+0x01),0x00        | alien's not a flagship, so just clear INFLIGHT_ALIEN.IsDying to say "we've finished dying, thanks". 
112C: C9            ret
...
112D: DD 36 10 32   ld   (ix+0x10),0x32        | set INFLIGHT_ALIEN.TempCounter1 to length of time to keep points on screen
1131: 3A 2D 42      ld   a,(0x422D)           | read FLAGSHIP_SCORE_FACTOR. 3 = full (800) points
1134: C6 20         add  a,0x20
1136: DD 77 12      ld   (ix+0x12),a          | set INFLIGHT_ALIEN.DyingAnimFrameCode 
1139: DD 34 02      inc  (ix+0x02)            | set INFLIGHT_ALIEN.StageOfLife (or death, should I say.)
113C: C9            ret
...
113D: DD 35 10      dec  (ix+0x10)            | decrement INFLIGHT_ALIEN.TempCounter1 
1140: C0            ret  nz                  | if points value countdown isn't zero, exit
1141: DD 36 01 00   ld   (ix+0x01),0x00        | clear INFLIGHT_ALIEN.IsDying flag.   
1145: C9            ret
...
1147: DD 7E 07      ld   a,(ix+0x07)          | read INFLIGHT_ALIEN.IndexInSwarm
114A: E6 70         and  0x70                 | compute row that alien is in
114C: 0F            rrca
114D: 4F            ld   c,a
114E: 0F            rrca
114F: 81            add  a,c
1150: ED 44         neg
1152: C6 7C         add  a,0x7C
1154: DD 77 03      ld   (ix+0x03),a          | set INFLIGHT_ALIEN.X

1157: DD 7E 07      ld   a,(ix+0x07)          | read INFLIGHT_ALIEN.IndexInSwarm
115A: E6 0F         and  0x0F                 | compute column that alien is in
115C: 07            rlca
115D: 07            rlca
115E: 07            rlca
115F: 07            rlca                     | multiply by 16..
1160: C6 07         add  a,0x07
1162: 4F            ld   c,a
1163: 3A 0E 42      ld   a,(0x420E)           | read SWARM_SCROLL_VALUE   
1166: 81            add  a,c                    
1167: DD 77 04      ld   (ix+0x04),a          | set INFLIGHT_ALIEN.Y 
116A: C9            ret
...
*
* The key is in the mutation of IX+0x19, INFLIGHT_ALIEN.PivotYValueAdd 
* 
* Expects:
* IX = pointer to INFLIGHT_ALIEN structure
* 
...
116B: DD 7E 18      ld   a,(ix+0x18)          | read INFLIGHT_ALIEN.Speed
116E: E6 03         and  0x03                 
1170: 3C            inc  a                   | now A is between 1 and 4.
1171: 47            ld   b,a

1172: DD 66 19      ld   h,(ix+0x19)          | read INFLIGHT_ALIEN.PivotYValueAdd
1175: DD 6E 1A      ld   l,(ix+0x1a)
1178: DD 56 1B      ld   d,(ix+0x1b)
117B: DD 5E 1C      ld   e,(ix+0x1c)
...
1180: 87            add  a,a           
1181: 30 01         jr   nc,0x1184

1183: 25            dec  h
1184: 82            add  a,d
1185: 57            ld   d,a
1186: 3E 00         ld   a,0x00
1188: 8C            adc  a,h
...
1190: ED 44         neg
1192: 87            add  a,a
1193: 30 01         jr   nc,0x1196

1195: 2D            dec  l

1196: 83            add  a,e
1197: 5F            ld   e,a
1198: 3E 00         ld   a,0x00
119A: 8D            adc  a,l
119B: FE 80         cp   0x80
119D: 20 01         jr   nz,0x11A0

119F: 79            ld   a,c

11A0: 6F            ld   l,a                 | restore L from C

11A1: 10 DB         djnz 0x117E

11A3: DD 74 19      ld   (ix+0x19),h
11A6: DD 75 1A      ld   (ix+0x1a),l
11A9: DD 72 1B      ld   (ix+0x1b),d
11AC: DD 73 1C      ld   (ix+0x1c),e
11AF: C9            ret
...
11D0: CD 48 00      call 0x0048               | call CALCULATE_TANGENT
11D3: 79            ld   a,c
11D4: A7            and  a
...
122C: DD 21 D0 42   ld   ix,0x42D0            | pointer to INFLIGHT_ALIENS_START+sizeof(INFLIGHT_ALIEN) (JOTD: comment was wrong)
...
HANDLE_SOUND:
16F5: AF            xor  a
16F6: 32 C0 41      ld   (0x41C0),a           | clear SOUND_VOL
16F9: 3D            dec  a
16FA: 32 C1 41      ld   (0x41C1),a           | set PITCH_SOUND_FX_BASE_FREQ value
16FD: CD 47 17      call 0x1747               | call HANDLE_GAME_START_MELODY
1700: CD D0 17      call 0x17D0               | call HANDLE_ALIEN_ATTACK_SOUND
1703: CD 19 18      call 0x1819               | call HANDLE_ALIEN_DEATH_SOUND
1706: CD 5D 17      call 0x175D               | call HANDLE_COMPLEX_SOUNDS
1709: CD 4F 18      call 0x184F               | call HANDLE_EXTRA_LIFE_SOUND
170C: CD 76 18      call 0x1876               | call HANDLE_COIN_INSERT_SOUND
170F: CD 23 17      call 0x1723               | call HANDLE_PLAYER_SHOOTING_SOUND
1712: 3A C0 41      ld   a,(0x41C0)           | load A with value of SOUND_VOL
1715: 32 06 68      ld   (0x6806),a           | Write to !SOUND Vol of F1
1718: 0F            rrca
1719: 32 07 68      ld   (0x6807),a           | Write to !SOUND Vol of f2
171C: 3A C1 41      ld   a,(0x41C1)           | read PITCH_SOUND_FX_BASE_FREQ value
171F: 32 00 78      ld   (0x7800),a           | Write to !Pitch Sound FX base frequency
1722: C9            ret



HANDLE_PLAYER_SHOOTING_SOUND:
1723: 3A CC 41      ld   a,(0x41CC)           | read PLAY_PLAYER_SHOOT_SOUND flag 
1726: 3D            dec  a                   | decrement value
1727: C2 33 17      jp   nz,0x1733            | if the value is nonzero now then it wasn't set to 1 before, so don't play the sound. Goto 0x1733
172A: 32 CC 41      ld   (0x41CC),a           | a is zero, so this sets the shoot flag to false. 
172D: 3E 08         ld   a,0x08               | this value here appears to affect the length of the shoot sound. Higher value = longer
172F: 32 CE 41      ld   (0x41CE),a           | set PLAYER_SHOOT_SOUND_COUNTER
1732: C9            ret

1733: 3A CE 41      ld   a,(0x41CE)           | read PLAYER_SHOOT_SOUND_COUNTER 
1736: A7            and  a                   | test if zero
1737: CA 43 17      jp   z,0x1743             | if it is zero, goto 0x1743, which will turn the player shoot sound off
173A: 3D            dec  a                   | reduce counter value by 1.
173B: 32 CE 41      ld   (0x41CE),a           | and write updated count back.
173E: 3A 07 40      ld   a,(0x4007)           | read IS_GAME_OVER flag
1741: EE 01         xor  0x01
1743: 32 05 68      ld   (0x6805),a           | !SOUND shoot on/off
1746: C9            ret
...
1747: 3A D1 41      ld   a,(0x41D1)           | read PLAY_GAME_START_MELODY flag
174A: 3D            dec  a                   | if was set to 1, this dec will set zero flag
174B: C0            ret  nz                  | return if PLAY_GAME_START_MELODY wasn't set
174C: 32 D1 41      ld   (0x41D1),a           | clear PLAY_GAME_START_MELODY flag
174F: 3C            inc  a
1750: 32 D2 41      ld   (0x41D2),a
1753: 32 D6 41      ld   (0x41D6),a
1756: 21 68 1E      ld   hl,0x1E68
1759: 22 D3 41      ld   (0x41D3),hl          | set COMPLEX_SOUND_POINTER
175C: C9            ret
...
1769: 21 CD 41      ld   hl,0x41CD            | pointer to address of IS_COMPLEX_SOUND_PLAYING
176C: 7E            ld   a,(hl)              | read flag
176D: A7            and  a                   | is flag set?
176E: C8            ret  z                   | No, a complex sound isn't playing, so return
176F: EB            ex   de,hl               | OK, now DE = 0x41CD
1770: 3E 02         ld   a,0x02
1772: 32 C0 41      ld   (0x41C0),a           | Set value of SOUND_VOL
1775: 3A D5 41      ld   a,(0x41D5)
1778: 32 C1 41      ld   (0x41C1),a           | Set PITCH_SOUND_FX_BASE_FREQ

* wait until counter has hit 0 before getting next musical note or sound effect to play.
177B: 3A D6 41      ld   a,(0x41D6)           | read DELAY_BEFORE_NEXT_SOUND
177E: 3D            dec  a                   | decrement countdown
177F: C2 A2 17      jp   nz,0x17A2            | if count hasn't hit zero, then goto 0x17A2

* OK, counter is zero, we get musical note or sound effect to play.
1782: 2A D3 41      ld   hl,(0x41D3)          | read COMPLEX_SOUND_POINTER
1785: 7E            ld   a,(hl)              | read sound to play
1786: FE E0         cp   0xE0                 | is this the end of sound marker?
1788: 28 1C         jr   z,0x17A6             | if so, then we've finished playing our sounds, goto 0x17A6
178A: 23            inc  hl                  | bump HL to point to next sound
178B: 22 D3 41      ld   (0x41D3),hl          | update COMPLEX_SOUND_POINTER
178E: 47            ld   b,a
178F: E6 1F         and  0x1F
1791: 21 A9 17      ld   hl,0x17A9
1794: E7            rst  0x20                 | call routine to fetch value @ HL + A
1795: 32 D5 41      ld   (0x41D5),a
1798: 78            ld   a,b
1799: E6 E0         and  0xE0
179B: 07            rlca
179C: 07            rlca
179D: 07            rlca
179E: 21 C8 17      ld   hl,0x17C8
17A1: E7            rst  0x20                 | call routine to fetch value @ HL + A
17A2: 32 D6 41      ld   (0x41D6),a           | set DELAY_BEFORE_NEXT_SOUND
17A5: C9            ret
...
17B7: C0            ret  nz
17B8: C7            rst  0x00
17B9: CD D0 D5      call 0xD5D0
17BC: DA DC E0      jp   c,0xE0DC
17BF: 1C            inc  e
17C0: 35            dec  (hl)
17C1: 87            add  a,a
17C2: A5            and  l
17C3: C4 D3 CA      call nz,0xCAD3
17C6: E3            ex   (sp),hl
17C7: E6 
...
17D0: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY
...
17D5: 21 C2 41      ld   hl,0x41C2
17D8: 7E            ld   a,(hl)
17D9: 3D            dec  a
17DA: C2 E5 17      jp   nz,0x17E5
17DD: 77            ld   (hl),a
17DE: 21 02 A0      ld   hl,0xA002
17E1: 22 C3 41      ld   (0x41C3),hl
17E4: C9            ret

17E5: 3A 26 42      ld   a,(0x4226)           | read HAVE_NO_INFLIGHT_ALIENS
...
17EB: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
17EE: 0F            rrca                     | move bit 0 into carry
17EF: 38 10         jr   c,0x1801             
17F1: 3A C4 41      ld   a,(0x41C4)
17F4: FE 60         cp   0x60
17F6: 30 01         jr   nc,0x17F9
17F8: 34            inc  (hl)
17F9: A7            and  a
17FA: CA 01 18      jp   z,0x1801
17FD: 3D            dec  a
17FE: 32 C4 41      ld   (0x41C4),a
1801: 7E            ld   a,(hl)
1802: E6 03         and  0x03
1804: C2 0C 18      jp   nz,0x180C
1807: 3E 60         ld   a,0x60
1809: C3 15 18      jp   0x1815
180C: 0F            rrca
180D: 3A C4 41      ld   a,(0x41C4)
1810: 30 03         jr   nc,0x1815
1812: C6 60         add  a,0x60
1814: 1F            rra
1815: 32 C1 41      ld   (0x41C1),a           | Set PITCH_SOUND_FX_BASE_FREQ
1818: C9            ret
...
1819: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY
181C: 0F            rrca                     | move flag into carry                     
181D: D0            ret  nc                  | if game is not in play, we won't play sound.                

181E: 3A DF 41      ld   a,(0x41DF)
1821: FE 06         cp   0x06                 | Do we want to play the ALIEN DEATH sound?       
1823: C2 3A 18      jp   nz,0x183A            | no, goto 0x183A

1826: 3A CD 41      ld   a,(0x41CD)           | read IS_COMPLEX_SOUND_PLAYING flag    
1829: 0F            rrca                     | move                                                          
182A: D8            ret  c
...
183A: FE 16         cp   0x16                 | Do we want to play the FLAGSHIP DEATH sound?                 
183C: C0            ret  nz                  | if not, return
183D: AF            xor  a
183E: 32 CF 41      ld   (0x41CF),a
1841: 3C            inc  a
1842: 32 CD 41      ld   (0x41CD),a
1845: 32 D6 41      ld   (0x41D6),a
1848: 21 DF 1E      ld   hl,0x1EDF
184B: 22 D3 41      ld   (0x41D3),hl
184E: C9            ret
...
1854: CA 5E 18      jp   z,0x185E             | if flag is not set, goto 0x185E.
1857: 21 00 80      ld   hl,0x8000            | store 0 in PLAY_EXTRA_LIFE_SOUND and 0x80 in EXTRA_LIFE_SOUND_COUNTER
...
1862: 32 C8 41      ld   (0x41C8),a           | update EXTRA_LIFE_SOUND_COUNTER
1865: E6 04         and  0x04             
1867: CA 6C 18      jp   z,0x186C
186A: 3E 81         ld   a,0x81
186C: 3D            dec  a
186D: 32 C1 41      ld   (0x41C1),a           | Set PITCH_SOUND_FX_BASE_FREQ
1870: 3E 01         ld   a,0x01
1872: 32 C0 41      ld   (0x41C0),a           | set SOUND_VOL
1875: C9            ret
...
1876: 21 C9 41      ld   hl,0x41C9            | address of PLAY_PLAYER_CREDIT_SOUND flag
1879: 7E            ld   a,(hl)              | read flag
187A: 3D            dec  a                   
187B: C2 86 18      jp   nz,0x1886            | if PLAY_PLAYER_CREDIT_SOUND was not set to 1, goto 0x1886
187E: 77            ld   (hl),a              | clear flag. 
187F: 21 20 00      ld   hl,0x0020            | duration to play sound
1882: 22 CA 41      ld   (0x41CA),hl
1885: C9            ret

* play the credit sound
1886: 23            inc  hl                  | bump HL to 0x41CA, which is address of PLAYER_CREDIT_SOUND_COUNTER               
...
188B: 23            inc  hl                  | bump HL to 0x41CB
188C: 7E            ld   a,(hl)
188D: C6 04         add  a,0x04             
188F: 77            ld   (hl),a
1890: 32 C1 41      ld   (0x41C1),a           | Set PITCH_SOUND_FX_BASE_FREQ
1893: AF            xor  a
1894: 32 C0 41      ld   (0x41C0),a           | set SOUND_VOL
1897: C9            ret
...
HANDLE_SWARM_SOUND:
1898: 3A D0 41      ld   a,(0x41D0)           | read RESET_SWARM_SOUND_TEMPO flag
189B: A7            and  a                   | test if zero
189C: 28 08         jr   z,0x18A6             | if zero, goto 0x18A6
189E: AF            xor  a                   | clear A
189F: 32 D0 41      ld   (0x41D0),a           | reset RESET_SWARM_SOUND_TEMPO flag
18A2: 3E 0F         ld   a,0x0F               | Maximum (slowest) Tempo setting
18A4: 18 0C         jr   0x18B2

18A6: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
18A9: C6 01         add  a,0x01               | add 1 to it
18AB: D0            ret  nc                  | return if no carry. This means the following code will only fire when A == #0xFF (255 decimal)
18AC: 3A 1F 42      ld   a,(0x421F)           | read LFO_FREQ_BITS
18AF: A7            and  a                   | test if zero
18B0: C8            ret  z                   | return if zero
18B1: 3D            dec  a                   | otherwise reduce by 1
18B2: 32 1F 42      ld   (0x421F),a           | and update LFO_FREQ_BITS 
18B5: 06 04         ld   b,0x04               | We're writing to 4 ports
18B7: 21 04 60      ld   hl,0x6004            | !DRIVER Background lfo freq bit0
...
HANDLE_TEXT_SCROLL:
18C0: 3A B0 40      ld   a,(0x40B0)           | read IS_COLUMN_SCROLLING flag
18C3: 0F            rrca                     | move flag value into carry
18C4: D0            ret  nc                  | return if flag was not set
18C5: 2A B1 40      ld   hl,(0x40B1)          | Load HL with pointer to scroll attribute data in OBJRAM_BACK_BUF.  
...
18C9: E6 07         and  0x07                 | mask in bits 0..2. We now have a value from 0..7 in A.
18CB: 20 1B         jr   nz,0x18E8            | if A is not zero, we don't scroll on a new character yet, goto 0x18E8.  

18CD: EB            ex   de,hl               | swap HL and DE. This preserves value of HL elsewhere without requiring a PUSH or a store
18CE: 2A B3 40      ld   hl,(0x40B3)          | HL now is pointer to a character to be scrolled onto screen      
18D1: 7E            ld   a,(hl)              | read character to scroll on
18D2: FE 3F         cp   0x3F                 | is this a terminating byte marking the end of the characters to scroll on?
18D4: 28 11         jr   z,0x18E7             | if so, goto 0x18E7

18D6: 23            inc  hl                  | bump pointer to next character 
18D7: 22 B3 40      ld   (0x40B3),hl          | and update COLUMN_SCROLL_NEXT_CHAR_PTR pointer 
18DA: D6 30         sub  0x30
18DC: 2A B5 40      ld   hl,(0x40B5)          | get character RAM address to plot character at 
18DF: 77            ld   (hl),a              | store character into character RAM

18E0: 01 E0 FF      ld   bc,0xFFE0            | load BC with -32 decimal 
18E3: 09            add  hl,bc               | Add offset to HL. HL now points to character in row above, same column. 
18E4: 22 B5 40      ld   (0x40B5),hl          | And update pointer 
...
18EB: 32 B0 40      ld   (0x40B0),a           | clear IS_COLUMN_SCROLLING flag
18EE: C9            ret



CHECK_IF_COIN_INSERTED:
18EF: 3A 00 40      ld   a,(0x4000)           | read stored state of dip switch 1 & 2
18F2: FE 03         cp   0x03                 | are dip switches set to FREE PLAY?
18F4: 28 21         jr   z,0x1917             | yes, free play enabled, so goto 0x1917: we don't need to check if coins are inserted
18F6: 21 10 40      ld   hl,0x4010            | pointer to PORT_STATE_6000 value
18F9: 7E            ld   a,(hl)              | read value
18FA: 2C            inc  l 
18FB: 2C            inc  l
18FC: 2C            inc  l                   | bump HL to 0x4013, which is PREV_PORT_STATE_6000 value
18FD: B6            or   (hl)                | combine bits set for current state of port 6000 with bits set from previous state
18FE: 2C            inc  l
18FF: 2C            inc  l                   | bump HL to 0x4015, which is PREV_PREV_PORT_STATE_6000 value
1900: 2F            cpl                      | flip bits
1901: A6            and  (hl)
1902: 2C            inc  l                   | bump HL to 0x4016, which is PREV_PREV_PREV_PORT_STATE_6000 value.
1903: A6            and  (hl)
1904: CB 7F         bit  7,a                 | read SERVICE state
1906: 20 16         jr   nz,0x191E            | if SERVICE is pressed, goto 0x191E
1908: E6 03         and  0x03                 | mask in COIN 1 & COIN 2 bits, discard rest
190A: C8            ret  z                   | if neither bits are set, meaning no coins inserted, return
190B: 21 04 40      ld   hl,0x4004            | address of UNPROCESSED_COINS counter
...
1912: E6 02         and  0x02                 | test COIN 2 state
...
191A: 22 01 40      ld   (0x4001),hl          | set COIN_COUNT to 1 and NUM_CREDITS to 9.
191D: C9            ret
...
191E: 21 02 40      ld   hl,0x4002            | address of NUM_CREDITS
1921: 7E            ld   a,(hl)              | read number of credits
1922: FE 63         cp   0x63                 | compare to 99 (decimal)
1924: D0            ret  nc                  | if A < 99 decimal, return
1925: 34            inc  (hl)                | otherwise, increment number of credits
1926: 3E 01         ld   a,0x01
1928: 32 C9 41      ld   (0x41C9),a           | Play sound of credit being added
192B: 11 01 07      ld   de,0x0701            | command: BOTTOM_OF_SCREEN_INFO_COMMAND, parameter: 1 (invokes DISPLAY_AVAILABLE_CREDIT)
192E: C3 F2 08      jp   0x08F2               | jump to QUEUE_COMMAND



HANDLE_UNPROCESSED_COINS:
1931: 21 03 40      ld   hl,0x4003            | COIN_CONTROL
1934: 7E            ld   a,(hl)              | Read value
1935: A7            and  a                   | test if zero. Zero means we process unprocessed coins
1936: 20 3C         jr   nz,0x1974            | if non-zero goto 0x1974 - don't bother processing coins.
...
193C: 2D            dec  l                   | point HL to 0x4003, COIN_CONTROL
193D: 36 0F         ld   (hl),0x0F            | reset COIN_CONTROL
193F: 3A 00 40      ld   a,(0x4000)           | read stored state of dip switches 1 & 2
1942: FE 03         cp   0x03                 | are both switches set, meaning FREE PLAY?
...
1946: 28 1C         jr   z,0x1964             | yes, goto 0x1964 to handle TWO COINS 1 PLAY

* Nice piece of code @0x194C to add two credits if required with only one CALL. Spot it?   
1948: 21 02 40      ld   hl,0x4002            | point HL to NUM_CREDITS
194B: 3D            dec  a                   | have we set dip switches to ONE COIN = 2 PLAYS? If so, A will be 0  
194C: CC 4F 19      call z,0x194F             | call code immediately after to add 1 credit, then return from CALL to add another credit!
194F: 7E            ld   a,(hl)              | read number of credits              
1950: FE 63         cp   0x63                 | have we reached 99 credits (decimal)?
1952: C8            ret  z                   | yes, so exit, no more credits allowed
1953: 30 0C         jr   nc,0x1961            | if we have more than 99 credits (decimal) then goto 0x1961 - clamp credits to 99.
1955: 34            inc  (hl)                | increment credit count
1956: 3E 01         ld   a,0x01
1958: 32 C9 41      ld   (0x41C9),a           | Set PLAY_PLAYER_CREDIT_SOUND flag to 1. I think you can guess what this does |)
195B: 11 01 07      ld   de,0x0701            | command: BOTTOM_OF_SCREEN_INFO_COMMAND, parameter: 1 (invokes DISPLAY_AVAILABLE_CREDIT)
195E: C3 F2 08      jp   0x08F2               | jump to QUEUE_COMMAND

1961: 36 63         ld   (hl),0x63            | set NUM_CREDITS to 99 decimal.
1963: C9            ret

* Called when dip switches are set to TWO COINS ONE PLAY
1964: 21 01 40      ld   hl,0x4001            | load HL with address of COIN_COUNT
1967: CB 46         bit  0,(hl)              | Have we only inserted one coin? 
1969: 28 06         jr   z,0x1971             | if bit test fails, we've not inserted any coins yet, goto 0x1971
196B: 36 00         ld   (hl),0x00            | reset COIN_COUNT to say we've acknowledged coins and awarded extra credit
196D: 2C            inc  l                   | bump HL to point to NUM_CREDITS      
196E: C3 4F 19      jp   0x194F               | call routine to add a credit
...
1975: 0F            rrca
1976: 0F            rrca
1977: 32 03 60      ld   (0x6003),a        | write to DRIVER|COIN CONTROL
197A: 35            dec  (hl)
197B: C9            ret
...
HANDLE_COIN_LOCKOUT:
197C: 3A 02 40      ld   a,(0x4002)           | read NUM_CREDITS
197F: FE 09         cp   0x09                 | compare to 9
1981: 30 06         jr   nc,0x1989            | if we have >= 9 credits, goto 0x1989
1983: 3E 01         ld   a,0x01
1985: 32 02 60      ld   (0x6002),a           | write to !DRIVER | COIN LOCKOUT
1988: C9            ret
1989: AF            xor  a
198A: 32 02 60      ld   (0x6002),a           | write to !DRIVER | COIN LOCKOUT
198D: C9            ret
...
198E: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
1991: C6 09         add  a,0x09
1993: E6 1F         and  0x1F
1995: C0            ret  nz
1996: 3A 07 40      ld   a,(0x4007)           | read IS_GAME_OVER flag
1999: 0F            rrca                     | move flag into carry
199A: D0            ret  nc                  | return if its not GAME OVER
199B: 3A 00 42      ld   a,(0x4200)           | read HAS_PLAYER_SPAWNED
199E: 0F            rrca                     | move flag into carry
199F: D0            ret  nc                  | return if player has not spawned yet

19A0: DD 21 D0 42   ld   ix,0x42D0            | pointer to INFLIGHT_ALIENS_START + sizeof(INFLIGHT_ALIEN)
19A4: 06 00         ld   b,0x00
19A6: D9            exx
19A7: 11 20 00      ld   de,0x0020            | sizeof(INFLIGHT_ALIEN)
19AA: 06 07         ld   b,0x07               | 7 aliens to process
19AC: D9            exx
19AD: DD 66 03      ld   h,(ix+0x03)          | H = INFLIGHT_ALIEN.X
19B0: DD 6E 04      ld   l,(ix+0x04)          | L = INFLIGHT_ALIEN.Y 
19B3: DD 4E 1A      ld   c,(ix+0x1a)
19B6: CD 12 1A      call 0x1A12
19B9: D9            exx
19BA: DD 19         add  ix,de               | bump IX to next INFLIGHT_ALIEN 
19BC: 10 EE         djnz 0x19AC


19BE: DD 21 60 42   ld   ix,0x4260            | load IX with address of ENEMY_BULLETS_START
19C2: 11 05 00      ld   de,0x0005            | sizeof(ENEMY_BULLET)
19C5: 06 07         ld   b,0x07               | number of enemy bullets
19C7: D9            exx
19C8: DD 66 01      ld   h,(ix+0x01)          | H = ENEMY_BULLET.X
19CB: DD 6E 03      ld   l,(ix+0x03)          | L = ENEMY_BULLET.YH
19CE: DD 4E 04      ld   c,(ix+0x04)          | C = ENEMY_BULLET.YDelta
19D1: CD 12 1A      call 0x1A12
19D4: D9            exx
19D5: DD 19         add  ix,de               | bump IX to next ENEMY_BULLET
19D7: 10 EE         djnz 0x19C7

19D9: D9            exx
19DA: 3A 02 42      ld   a,(0x4202)           | read PLAYER_Y
19DD: 4F            ld   c,a
19DE: 3A 0E 42      ld   a,(0x420E)           | read SWARM_SCROLL_VALUE
19E1: C6 80         add  a,0x80
...
19EF: CB 2F         sra  a
19F1: 4F            ld   c,a
19F2: CD 3C 00      call 0x003C               | call GENERATE_RANDOM_NUMBER. Now A = pseudorandom number
19F5: 41            ld   b,c
19F6: 87            add  a,a
19F7: 9F            sbc  a,a
19F8: 20 01         jr   nz,0x19FB
19FA: 3C            inc  a
19FB: 80            add  a,b
19FC: C6 01         add  a,0x01
19FE: FA 0E 1A      jp   m,0x1A0E
1A01: FE 02         cp   0x02
1A03: 30 05         jr   nc,0x1A0A            
1A05: AF            xor  a
1A06: 32 3F 42      ld   (0x423F),a           | set ATTRACT_MODE_FAKE_CONTROLLER value. Ship will move under AI control
1A09: C9            ret

1A0A: 3E 04         ld   a,0x04               | simulates player moving LEFT (bit maps to !SW0   p1 left)
1A0C: 18 F8         jr   0x1A06               | go set ATTRACT_MODE_FAKE_CONTROLLER 

1A0E: 3E 08         ld   a,0x08               | simulates player moving RIGHT (bit maps to !SW0    p1 right)
1A10: 18 F4         jr   0x1A06               | go set ATTRACT_MODE_FAKE_CONTROLLER 
...
1A1B: 1E 00         ld   e,0x00
1A1D: D6 34         sub  0x34                 | subtract 0x34 from X coordinate
1A1F: 38 04         jr   c,0x1A25
1A21: 1C            inc  e
1A22: D6 34         sub  0x34
1A24: D0            ret  nc

1A25: 3A 02 42      ld   a,(0x4202)           | read PLAYER_Y 
1A28: 95            sub  l                   | subtract Y coordinate of alien/bullet
1A29: D6 40         sub  0x40
1A2B: FE 80         cp   0x80
1A2D: D0            ret  nc                  | return if A >= 0x80
...
1A39: B3            or   e
1A3A: 5F            ld   e,a
1A3B: 16 00         ld   d,0x00
1A3D: 21 45 1A      ld   hl,0x1A45
1A40: 19            add  hl,de
1A41: 7E            ld   a,(hl)
1A42: 80            add  a,b
1A43: 47            ld   b,a
...
* Clear screen 
1A55: 21 00 50      ld   hl,0x5000            | address of character RAM
1A58: 06 04         ld   b,0x04               | the number of bytes we need to write to clear the screen, divided by 256
1A5A: 3E 10         ld   a,0x10               | ordinal of empty character 
1A5C: 77            ld   (hl),a              | write character to screen
1A5D: 2C            inc  l                   | increment low byte of character RAM address to write to
1A5E: C2 5C 1A      jp   nz,0x1A5C            | do while low byte is !=0 
1A61: 24            inc  h                   | increment high byte of screen address to write to
1A62: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog
1A65: 10 F3         djnz 0x1A5A               | decrement b and if !=0 goto 0x1A5A

* clear attributes and reset scroll values
1A67: 21 00 58      ld   hl,0x5800            | start of screen attribute RAM
...
1A6D: C2 6B 1A      jp   nz,0x1A6B            | if low byte of pointer has not wrapped to 0, goto 0x1A6B

* Disable lamps and reset coin control
1A70: AF            xor  a                   | clear a
1A71: 21 00 60      ld   hl,0x6000            | pointer to !DRIVER lamp 1
1A74: 06 04         ld   b,0x04               | 4 ports to write to, from 0x6000 - 0x6003
1A76: 77            ld   (hl),a              | write 0 to port. 
1A77: 23            inc  hl                  | bump HL to point to next port
1A78: 10 FC         djnz 0x1A76               | do until all ports written to

* Set !DRIVER Background lfo freq bits 0-3 to 1
* HL currently points to 0x6004, which is !DRIVER Background lfo freq bit0
1A7A: 3C            inc  a                   | set A to 1
1A7B: 06 04         ld   b,0x04               | 4 ports to write to, from 0x6004 - 0x6007
1A7D: 77            ld   (hl),a              | write to port
1A7E: 23            inc  hl                  | bump HL to point to next port
1A7F: 10 FC         djnz 0x1A7D               | do until all ports written to

* writes zero to all !SOUND ports
1A81: AF            xor  a                   | clear a
1A82: 06 08         ld   b,0x08               | 8 ports to write to, from 0x6800 - 0x6807
1A84: 21 00 68      ld   hl,0x6800            | pointer to !SOUND reset background F1   
1A87: 77            ld   (hl),a              | write to port
1A88: 23            inc  hl                  | bump HL to point to next port
1A89: 10 FC         djnz 0x1A87               | do until all ports written to

* writes zero to 9Nregen NMIon, 9Nregen stars on, 9Nregen hflip, 9Nregen vflip
1A8B: 06 08         ld   b,0x08
1A8D: 21 01 70      ld   hl,0x7001            | pointer to 9Nregen NMIon
1A90: 77            ld   (hl),a              | write to port
1A91: 23            inc  hl                  | bump HL to point to next port
1A92: 10 FC         djnz 0x1A90

1A94: 3D            dec  a                   | set a to #0xFF (255 decimal)
1A95: 32 00 78      ld   (0x7800),a           | write to !pitch  Sound Fx base frequency

* Write a batch of bytes with calculated values to an area of working RAM (0x4000 - 0x43FF).
...
* do the write phase
1A98: 0E 20         ld   c,0x20               | number of times to repeat RAM tests
1A9A: 21 00 40      ld   hl,0x4000            | pointer to start of RAM
1A9D: 06 04         ld   b,0x04               | total number of bytes to write, divided by 256
1A9F: 79            ld   a,c                 | initial seed value 
1AA0: C6 2F         add  a,0x2F               | Add #0x2F (47 decimal) to seed value

1AA2: 77            ld   (hl),a              | write byte to working RAM
1AA3: 2C            inc  l                   | increment low byte of RAM pointer
1AA4: C2 A0 1A      jp   nz,0x1AA0            | if low byte of pointer has not wrapped to 0, goto 0x1AA0
1AA7: 3C            inc  a                   | increment value of byte to write to RAM
1AA8: 24            inc  h                   | increment high byte of RAM pointer 
1AA9: 10 F5         djnz 0x1AA0               | do until B==0

* now do the read (verify) phase
1AAB: 21 00 40      ld   hl,0x4000            | pointer to start of RAM
1AAE: 06 04         ld   b,0x04               | total number of bytes to verify, divided by 256
1AB0: 79            ld   a,c                 | initial seed value 
1AB1: C6 2F         add  a,0x2F               | add #0x2F (47 decimal) to A, as write phase did
1AB3: BE            cp   (hl)                | compare byte to test RAM integrity
1AB4: 20 45         jr   nz,0x1AFB            | if they don't match, there's a RAM error, goto 0x1AFB
1AB6: 2C            inc  l                   | increment low byte of RAM pointer
1AB7: C2 B1 1A      jp   nz,0x1AB1            | if low byte of pointer has not wrapped to 0, goto 0x1AA0
1ABA: 3C            inc  a                   | increment value of byte expected to be read from RAM
1ABB: 24            inc  h                   | increment high byte of RAM pointer 
1ABC: 10 F3         djnz 0x1AB1               | do until B==0

1ABE: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog
1AC1: 0D            dec  c                   | decrement counter of how many times to repeat RAM write and test phases
1AC2: C2 9A 1A      jp   nz,0x1A9A            | if the counter !=0, then goto 0x1A9A

* Write a batch of bytes with calculated values to character RAM (0x5000 - 0x53FF) 
* When the batch has been written, read each byte back and compare to calculated values. 
...
1AC5: 31 00 44      ld   sp,0x4400
1AC8: 0E 20         ld   c,0x20               | number of times to repeat RAM tests
1ACA: 21 00 50      ld   hl,0x5000            | pointer to character RAM
1ACD: 06 04         ld   b,0x04               | total number of bytes to write, divided by 256
1ACF: 79            ld   a,c                 | initial seed value 
1AD0: C6 2F         add  a,0x2F               | Add #0x2F (47 decimal) to seed value
1AD2: 77            ld   (hl),a              | write byte to character RAM
1AD3: 2C            inc  l                   | increment low byte of RAM pointer
1AD4: C2 D0 1A      jp   nz,0x1AD0            | if low byte of pointer has not wrapped to 0, goto 0x1AD0
1AD7: 3C            inc  a                   | increment value of byte to write to character RAM
1AD8: 24            inc  h                   | increment high byte of RAM pointer 
1AD9: 10 F5         djnz 0x1AD0               | do until B==0
1ADB: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog

* now do the character RAM read (verify) phase
1ADE: 21 00 50      ld   hl,0x5000            | pointer to character RAM
1AE1: 06 04         ld   b,0x04               | total number of bytes to verify, divided by 256
1AE3: 79            ld   a,c                 | initial seed value 
1AE4: C6 2F         add  a,0x2F               | add #0x2F (47 decimal) to A, as write phase did 
1AE6: BE            cp   (hl)                | compare byte to test character RAM integrity
1AE7: 20 16         jr   nz,0x1AFF            | if they don't match, there's a character RAM error, goto 0x1AFB
1AE9: 2C            inc  l                   | increment low byte of character RAM pointer
1AEA: C2 E4 1A      jp   nz,0x1AE4            | if low byte of pointer has not wrapped to 0, goto 0x1AE4
1AED: 3C            inc  a                   | increment value of byte expected to be read from RAM
1AEE: 24            inc  h                   | increment high byte of RAM pointer 
1AEF: 10 F3         djnz 0x1AE4               | do until B==0
1AF1: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog
1AF4: 0D            dec  c
1AF5: C2 CA 1A      jp   nz,0x1ACA
1AF8: C3 70 1B      jp   0x1B70
...
DISPLAY_BAD_RAM_1:
1AFB: 3E 01         ld   a,0x01               | Error code for BAD RAM 1
1AFD: 18 05         jr   0x1B04               | jump to DISPLAY_BAD_RAM_MESSAGE
...
BAD_RAM_2:
1AFF: CD 5D 1B      call 0x1B5D               | call CLEAR_SCREEN
1B02: 3E 02         ld   a,0x02               | Error code for BAD RAM 2
...
1B04: 32 F3 51      ld   (0x51F3),a           | write number stored in A to screen 
1B07: 11 2D 1B      ld   de,0x1B2D            | address of text string "BAD RAM" in reverse

* Called to display "BAD RAM [n]" or "BAD ROM [n]""
DISPLAY_BAD_RAM_OR_ROM_MESSAGE:
1B0A: 21 33 52      ld   hl,0x5233            | character RAM address to print text at
1B0D: 01 20 00      ld   bc,0x0020            | offset to add to character RAM address after every character drawn
1B10: D9            exx                      | swap to external registers. We want to use B in the other set for a counter
1B11: 06 07         ld   b,0x07               | there's 7 characters in "BAD RAM" (including the space between words)
...
1B19: 10 F8         djnz 0x1B13               | Decrement B and if not 0, goto 0x1B13 to plot remaining characters.
1B1B: AF            xor  a
1B1C: 32 01 70      ld   (0x7001),a           | Set flag for NMI to read
1B1F: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog
1B22: 3A 00 60      ld   a,(0x6000)           | read coin, p1 control, test & service state
1B25: E6 40         and  0x40                 | mask in !SW0 TEST button bit
1B27: C2 1B 1B      jp   nz,0x1B1B            | if TEST button is pressed, goto 0x1B1B. I think this is a bug. Should be jp Z.
1B2A: C3 00 00      jp   0x0000               | ... do self test again. Doesn't give user long to see the text!!!
...
1B35: 3A 00 60      ld   a,(0x6000)           | read coin, p1 control, test & service state 
1B38: 47            ld   b,a
1B39: 3A 00 68      ld   a,(0x6800)           | read start button, p2 control, dipsw 1/2 state 
1B3C: A0            and  b
1B3D: E6 04         and  0x04
1B3F: 28 10         jr   z,0x1B51
1B41: 79            ld   a,c                 | load A with incorrect checksum
1B42: E6 0F         and  0x0F                 | mask in lower nibble
1B44: 32 D3 51      ld   (0x51D3),a           | write to character RAM
1B47: 79            ld   a,c
1B48: 0F            rrca
1B49: 0F            rrca
1B4A: 0F            rrca
1B4B: 0F            rrca
1B4C: E6 0F         and  0x0F
1B4E: 32 F3 51      ld   (0x51F3),a           | write to character RAM
1B51: 11 56 1B      ld   de,0x1B56            | address of text "BAD ROM" in reverse
1B54: 18 B4         jr   0x1B0A               | call DISPLAY_BAD_RAM_OR_ROM_MESSAGE
...
CLEAR_SCREEN:
1B5D: 21 00 50      ld   hl,0x5000            | start address of character RAM 
1B60: 06 04         ld   b,0x04               | total number of bytes in character RAM, divided by 256
1B62: 3E 10         ld   a,0x10               | ordinal for empty character 
1B64: 77            ld   (hl),a              | write character
1B65: 2C            inc  l
1B66: C2 64 1B      jp   nz,0x1B64
1B69: 24            inc  h
1B6A: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog
1B6D: 10 F3         djnz 0x1B62
1B6F: C9            ret
...
ROM_CHECKSUM:
1B70: CD 5D 1B      call 0x1B5D               | call CLEAR_SCREEN
1B73: 21 00 00      ld   hl,0x0000            | point HL to start of ROM
1B76: 06 28         ld   b,0x28               | B holds number of bytes we need to read  
...
1B7B: C2 79 1B      jp   nz,0x1B79            | if low byte has not wrapped to 0, goto 0x1B79
1B7E: 24            inc  h                   | Otherwise increment high byte of ROM pointer
1B7F: 4F            ld   c,a                 | Save current checksum value in C
1B80: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog
1B83: 79            ld   a,c                 | Restore current checksum value from C
1B84: 10 F3         djnz 0x1B79               | Loop until B == 0
1B86: A7            and  a                   | Test if checksum total is 0
1B87: C2 34 1B      jp   nz,0x1B34            | If checksum total is not 0, then display BAD ROM message
...
1B8A: 21 00 40      ld   hl,0x4000            | Start of working RAM
1B8D: 06 C0         ld   b,0xC0
1B8F: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
1B90: 3D            dec  a
1B91: 06 40         ld   b,0x40
1B93: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
1B94: AF            xor  a
1B95: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
1B96: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
1B97: 06 A0         ld   b,0xA0
1B99: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
1B9A: 32 01 70      ld   (0x7001),a           | clear 9Nregen NMIon
1B9D: 32 05 70      ld   (0x7005),a
1BA0: 32 06 70      ld   (0x7006),a           | set "regen hflip" to 0
1BA3: 32 07 70      ld   (0x7007),a           | set "regen vflip" to 0
1BA6: 32 18 40      ld   (0x4018),a           | set DISPLAY_IS_COCKTAIL_P2 to 0
1BA9: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog
1BAC: 3E 20         ld   a,0x20
1BAE: 32 08 40      ld   (0x4008),a           | set TEMP_COUNTER_1
1BB1: 3E 03         ld   a,0x03
1BB3: 32 1A 40      ld   (0x401A),a           | set DIAGNOSTIC_MESSAGE_TYPE to 1.
1BB6: 21 C0 C0      ld   hl,0xC0C0            
1BB9: 22 A0 40      ld   (0x40A0),hl          | reset CIRC_CMD_QUEUE_PTR_LO and CIRC_CMD_QUEUE_PROC_LO
1BBC: 3E 01         ld   a,0x01
1BBE: 32 04 70      ld   (0x7004),a           | Set 9Nregen stars on. Starry background now appears 
1BC1: 32 02 70      ld   (0x7002),a           | Does nothing
1BC4: 32 03 70      ld   (0x7003),a           | Does nothing
1BC7: 32 01 70      ld   (0x7001),a           | Enable 9Nregen NMIon
1BCA: C3 00 20      jp   0x2000


*
*
* This is invoked by the NMI when the contents of 0x401A (DIAGNOSTIC_MESSAGE_TYPE) are non-zero.
*
...
* 3                                 Invokes OBJRAM_TEST: test attributes & sprites for (value held in 0x4008) times
* Any other value                   Back to self-test
*
*

HANDLE_DIAGNOSTICS:
1BCD: 21 D8 00      ld   hl,0x00D8            | address of exit code in NMI handler
1BD0: E5            push hl                  | push return address onto stack. When RET hit, this will be jumped to
1BD1: 3D            dec  a
1BD2: CA 3A 1C      jp   z,0x1C3A             | if A was 1 on entry, goto 0x1C3A (HANDLE_SERVICE_MODE)
1BD5: 3D            dec  a
1BD6: CA 28 1D      jp   z,0x1D28             | if A was 2 on entry, goto 0x1D28
1BD9: 3D            dec  a
1BDA: C2 00 00      jp   nz,0x0000            | if value is !=0 then back to self-test


* Write a batch of bytes with calculated values to an area of OBJRAM (0x5800 - 0x58FF).
* When the batch has been written, read each byte back and compare to calculated values.
...
1BDD: 21 00 58      ld   hl,0x5800            | Start of OBJRAM
1BE0: 3A 1E 40      ld   a,(0x401E)           | use RAND_NUMBER as our seed number.
1BE3: 77            ld   (hl),a              | write to OBJRAM
1BE4: C6 2F         add  a,0x2F               | add #0x2F (47 decimal) to number just written
1BE6: 2C            inc  l                   | increment low byte of OBJRAM pointer
1BE7: C2 E3 1B      jp   nz,0x1BE3            | if low byte has not wrapped to 0, goto 0x1BE3

* Second, read values from OBJRAM and verify.
1BEA: 3A 1E 40      ld   a,(0x401E)           | use RAND_NUMBER as our seed number.            
1BED: BE            cp   (hl)                | read from OBJRAM and compare 
1BEE: 20 3C         jr   nz,0x1C2C            | if the value read from OBJRAM doesn't match what we expect, goto DISPLAY_BAD_RAM_3
1BF0: C6 2F         add  a,0x2F               | add #0x2F (47 decimal) to number just written
1BF2: 2C            inc  l                   | increment low byte of OBJRAM pointer
1BF3: C2 ED 1B      jp   nz,0x1BED            | if low byte has not wrapped to 0, goto 0x1BED

1BF6: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog
1BF9: CD 3C 00      call 0x003C               | call GENERATE_RANDOM_NUMBER
1BFC: 21 08 40      ld   hl,0x4008            | pointer to count of how many times to repeat test
1BFF: 35            dec  (hl)                | decrement value 
1C00: C0            ret  nz                  | if value is 0, return, no more tests to do
1C01: AF            xor  a
1C02: 21 00 58      ld   hl,0x5800            | Start of OBJRAM
1C05: 47            ld   b,a                 | 
1C06: D7            rst  0x10                 | Fill 0x5800 to 0x5900 inclusive with value in A.
1C07: 3E 01         ld   a,0x01
1C09: 32 06 40      ld   (0x4006),a           | set IS_GAME_IN_PLAY
1C0C: 32 1A 40      ld   (0x401A),a
...
1C28: 32 F3 51      ld   (0x51F3),a           | write "K" to screen
1C2B: C9            ret



DISPLAY_BAD_RAM_3:
1C2C: 21 00 58      ld   hl,0x5800            | Start of OBJRAM
1C2F: AF            xor  a                   | value to write
1C30: 77            ld   (hl),a              | write to OBJRAM
1C31: 2C            inc  l                   | increment low byte of pointer
1C32: C2 30 1C      jp   nz,0x1C30            | if low byte has not wrapped to 0, goto 0x1C30
1C35: 3E 03         ld   a,0x03               | error number
1C37: C3 04 1B      jp   0x1B04               | Jump to DISPLAY_BAD_RAM_MESSAGE, to display BAD RAM 3




HANDLE_SERVICE_MODE:
1C3A: CD F5 16      call 0x16F5               | call HANDLE_SOUND
1C3D: CD A6 16      call 0x16A6               | does nothing - must be legacy.
1C40: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog 
1C43: 3A 00 60      ld   a,(0x6000)           | read coin, p1 control, test & service state
1C46: 47            ld   b,a                 | save bits in B as A will be corrupted by succeeding operations
1C47: E6 83         and  0x83                 | mask in coin1, coin2 and service bits
1C49: 28 05         jr   z,0x1C50             | if coin1 or coin2 are not held down, goto 0x1C50
1C4B: 3E 01         ld   a,0x01
1C4D: 32 C9 41      ld   (0x41C9),a           | set PLAY_PLAYER_CREDIT_SOUND
1C50: 3A 00 68      ld   a,(0x6800)           | read start button, p2 control, dipsw 1/2 state 
1C53: 4F            ld   c,a                 | save state in C
1C54: E6 03         and  0x03                 | mask in the 1p START and 2p START button states
1C56: 28 05         jr   z,0x1C5D             | if no buttons held down, goto 0x1C5D
1C58: 3E 16         ld   a,0x16
1C5A: 32 DF 41      ld   (0x41DF),a           | play the sound made when you shoot a Flagship in flight
...
1C5F: E6 0C         and  0x0C                 | mask in the controller bits. Tests if P1 or P2's controllers have been moved left or right
1C61: 28 05         jr   z,0x1C68             | if no controllers have been moved, goto 0x1C68
1C63: 3E 06         ld   a,0x06               | otherwise... 
1C65: 32 DF 41      ld   (0x41DF),a           | play the sound made when you shoot an alien
...
1C6A: E6 10         and  0x10                 | mask in the bit for SHOOT 
1C6C: 28 05         jr   z,0x1C73             | if no shoot button is pressed, goto 0x1C73
1C6E: 3E 01         ld   a,0x01               | 
1C70: 32 CC 41      ld   (0x41CC),a           | set PLAY_PLAYER_SHOOT_SOUND flag

* handle METHOD OF PLAY
1C73: 3A 00 68      ld   a,(0x6800)           | read start button, p2 control, dipsw 1/2 state 
1C76: 07            rlca                     | move bits 6 & 7 (the state of dip switches 1 & 2)..   
1C77: 07            rlca                     | .. to bits 0 & 1.
1C78: E6 03         and  0x03                 | mask in bits 0 & 1, discard everything else.
1C7A: CD CF 1C      call 0x1CCF               | call DISPLAY_DIP_SWITCH_SETTINGS

* handle BONUS GALIXIP
1C7D: 3A 00 70      ld   a,(0x7000)           | read state of dip switch 3,4,5,6  
1C80: E6 03         and  0x03                 | mask in state of dip switches 3 & 4
1C82: C6 04         add  a,0x04
1C84: CD CF 1C      call 0x1CCF               | call DISPLAY_DIP_SWITCH_SETTINGS

* handle NUMBER OF GALIXIP PER GAME
1C87: 3A 00 70      ld   a,(0x7000)           | read state of dip switch 3,4,5,6
1C8A: 0F            rrca                     | move bit 2 (the state of dip switch 5)..   
1C8B: 0F            rrca                     | .. to bit 0
1C8C: E6 01         and  0x01                 | mask in bit 0, discard everything else
1C8E: C6 08         add  a,0x08
1C90: CD CF 1C      call 0x1CCF               | call DISPLAY_DIP_SWITCH_SETTINGS

1C93: 3A 00 60      ld   a,(0x6000)           | read coin, p1 control, test & service state   
1C96: E6 40         and  0x40                 | read TEST bit
1C98: C0            ret  nz                  | if bit set, return
1C99: AF            xor  a
1C9A: 32 06 40      ld   (0x4006),a           | clear IS_GAME_IN_PLAY
1C9D: 3E 02         ld   a,0x02
1C9F: 32 1A 40      ld   (0x401A),a           | set SCRIPT_STAGE
1CA2: 21 10 30      ld   hl,0x3010
1CA5: 22 08 40      ld   (0x4008),hl          | set TEMP_COUNTER_1 and TEMP_COUNTER_2 in one go
1CA8: 21 00 50      ld   hl,0x5000            | HL = pointer to character RAM
1CAB: 22 0B 40      ld   (0x400B),hl          | set TEMP_CHAR_RAM_PTR
1CAE: AF            xor  a
1CAF: 21 00 60      ld   hl,0x6000            | point HL to !SW0 
1CB2: 06 04         ld   b,0x04
1CB4: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.    

RESET_SOUND:
1CB5: 3E 01         ld   a,0x01
1CB7: 21 04 60      ld   hl,0x6004            | Address of !DRIVER Background lfo freq bit0
1CBA: 06 04         ld   b,0x04
1CBC: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
1CBD: AF            xor  a
1CBE: 06 08         ld   b,0x08
1CC0: 21 00 68      ld   hl,0x6800
1CC3: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
1CC4: 06 05         ld   b,0x05
1CC6: 21 01 70      ld   hl,0x7001
1CC9: D7            rst  0x10                 | Fill B bytes of memory from HL with value in A.
1CCA: 3D            dec  a
1CCB: 32 00 78      ld   (0x7800),a
1CCE: C9            ret
...
1CD3: 5F            ld   e,a
1CD4: 16 00         ld   d,0x00               | DE is now offset into DIP_SWITCH_SETTINGS_TEXT table.
1CD6: 21 F6 1C      ld   hl,0x1CF6            | pointer to DIP_SWITCH_SETTINGS_TEXT table
1CD9: 19            add  hl,de               | hl+=de. Now HL points to an entry in the table @ 1CF6.

1CDA: 06 02         ld   b,0x02               | we want to push 2 values onto stack
...
1CE1: 10 F9         djnz 0x1CDC               | do until values pushed on stack
...
1CE7: 01 E0 FF      ld   bc,0xFFE0            | BC = -32 decimal
...
*
* Value in 0x4008: represents how many columns of grids you want to display.

CHARACTER_RAM_COLOUR_TEST: 
1D28: 21 08 40      ld   hl,0x4008            | load HL with address of TEMP_COUNTER_1
1D2B: 3A 00 78      ld   a,(0x7800)           | Kick the watchdog
1D2E: 7E            ld   a,(hl)             
1D2F: A7            and  a                   | test if zero
1D30: CA 51 1D      jp   z,0x1D51    
1D33: D9            exx
1D34: 2A 0B 40      ld   hl,(0x400B)          | 
1D37: 06 10         ld   b,0x10               | Render 16 pairs of characters on a row..
1D39: 36 30         ld   (hl),0x30            | plot top right of square character
1D3B: 23            inc  hl
1D3C: 36 32         ld   (hl),0x32            | plot bottom right of square character
1D3E: 23            inc  hl
1D3F: 10 F8         djnz 0x1D39

1D41: 06 10         ld   b,0x10
1D43: 36 34         ld   (hl),0x34            | plot top left of square character
1D45: 23            inc  hl
1D46: 36 36         ld   (hl),0x36            | plot bottom left of square character
1D48: 23            inc  hl
1D49: 10 F8         djnz 0x1D43

1D4B: 22 0B 40      ld   (0x400B),hl          | set TEMP_CHAR_RAM_PTR
1D4E: D9            exx
...
1D54: 28 02         jr   z,0x1D58             | if it's zero, goto 0x1D58
1D56: 35            dec  (hl)                | otherwise decrement value of TEMP_COUNTER_2
1D57: C0            ret  nz                  | return if value isn't zero.

1D58: 3A 00 60      ld   a,(0x6000)           | read coin, p1 control, test & service state
1D5B: E6 40         and  0x40                 | mask in TEST bit
1D5D: C0            ret  nz                  | return if TEST is on
1D5E: 21 00 50      ld   hl,0x5000            | pointer to start of character RAM 
1D61: 22 0B 40      ld   (0x400B),hl          | set TEMP_CHAR_RAM_PTR
1D64: 3E 20         ld   a,0x20
1D66: 32 08 40      ld   (0x4008),a           | set TEMP_COUNTER_1
1D69: AF            xor  a
1D6A: 32 1A 40      ld   (0x401A),a           | set DIAGNOSTIC_MESSAGE_TYPE
1D6D: 32 05 40      ld   (0x4005),a           | set SCRIPT_NUMBER
1D70: C9            ret
...
*
* GAME START tune. Referenced by 0x1756
*
...
*
* ALIEN DEATH sound effect. Referenced by 0x1833
*
...
*
* FLAGSHIP DEATH sound effect. Referenced by 0x1848
*
...
1F82:  58 49 50 40 32 47 41 4C 41 58 49 50 40 33        XIP@2GALIXIP@3
...
2000: 21 A2 40      ld   hl,0x40A2            | pointer to player 1 state
2003: 06 1E         ld   b,0x1E               | number of bytes to write
2005: 36 00         ld   (hl),0x00            | write 0 
2007: 23            inc  hl
2008: 10 FB         djnz 0x2005               | repeat until b==0
...
200A: 26 40         ld   h,0x40               | high byte of pointer to queue entry be processed
200C: 3A A1 40      ld   a,(0x40A1)           | read CIRC_CMD_QUEUE_PROC_LO
...
2011: 87            add  a,a                 | multiply A by 2 to form an offset into jump table @0x203D
2012: 30 05         jr   nc,0x2019            | if no carry, then we have a valid command number, goto 0x2019
2014: CD 67 20      call 0x2067               | call HANDLE_SWARM_ANIMATION
2017: 18 F1         jr   0x200A               | process next entry in circular queue

2019: E6 0F         and  0x0F                 | mask in lower nibble
201B: 4F            ld   c,a
201C: 06 00         ld   b,0x00               | extend A into BC. BC is now the offset to add to 0x203D (see code @ 0x2030)
201E: 36 FF         ld   (hl),0xFF            | write #0xFF (255 decimal) to first byte of byte pair, to mark it as "free"
2020: 2C            inc  l
2021: 5E            ld   e,(hl)              | read parameter value from queue entry into E. 
2022: 36 FF         ld   (hl),0xFF            | write #0xFF (255 decimal) to second byte of byte pair, to mark it as "free"
2024: 2C            inc  l
2025: 7D            ld   a,l                 
2026: FE C0         cp   0xC0                 | is HL == 0x4100? If so, comparing L (which will be 0) to #0xC0 (192 decimal) will set the carry flag. 
2028: 30 02         jr   nc,0x202C            | if carry is not set, then we have not reached the end of the queue (0x4100), goto 0x202C
202A: 3E C0         ld   a,0xC0               | otherwise, we have reached end of queue. 
202C: 32 A1 40      ld   (0x40A1),a           | Set lo byte of pointer to 0xC0 (0x40C0 = start of circular queue)
202F: 7B            ld   a,e                 | Now A = parameter to command
2030: 21 3D 20      ld   hl,0x203D            | pointer to jump table beginning @ 0x203D
2033: 09            add  hl,bc               | now HL = pointer to entry in jump table
2034: 5E            ld   e,(hl)
2035: 23            inc  hl
2036: 56            ld   d,(hl)              | DE = pointer read from jump table
2037: 21 0A 20      ld   hl,0x200A            | return address to go to (entry point of PROCESS_CIRCULAR_COMMAND_QUEUE)
203A: E5            push hl                  | push it onto stack, so when we hit a RET it'll return to 0x200A
...
203D: 
    55 20           | pointer to code @ 0x2055  (DRAW_ALIEN_COMMAND)
    5E 20           | pointer to code @ 0x205E  (DELETE_ALIEN_COMMAND) 
    5F 21           | pointer to code @ 0x215F  (DISPLAY_PLAYER_COMMAND)
    A6 21           | pointer to code @ 0x21A6  (UPDATE_PLAYER_SCORE_COMMAND)
    FE 21           | pointer to code @ 0x21FE  (RESET_SCORE_COMMAND)
    31 22           | pointer to code @ 0x2231  (DISPLAY_SCORE_COMMAND)
    F1 22           | pointer to code @ 0x22F1  (PRINT_TEXT)
    B7 24           | pointer to code @ 0x24B7  (DISPLAY_BOTTOM_OF_SCREEN)


204D: E4 7B 73      call po,0x737B
2050: 8E            adc  a,(hl)
2051: 10 FF         djnz 0x2052
2053: FF            rst  0x38
2054: FF            rst  0x38
...
DRAW_ALIEN_COMMAND:
2055: CD E1 20      call 0x20E1               | call GET_ALIEN_CHAR_RAM_ADDR
2058: CD 04 21      call 0x2104               | call CHOOSE_ANIMATION_FRAME_FOR_ALIEN_REJOINING_SWARM
205B: C3 31 21      jp   0x2131               | jump to DRAW_ALIEN
...
205E: CD E1 20      call 0x20E1               | call GET_ALIEN_CHAR_RAM_ADDR
2061: DA 83 25      jp   c,0x2583             | jump to PLOT_CHARACTERS_2_BY_2_ASCENDING
2064: C3 A7 25      jp   0x25A7               | plot two spaces in the same column 
...
2067: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
206A: 47            ld   b,a
206B: E6 0F         and  0x0F                 | mask in lower nibble
206D: 28 2D         jr   z,0x209C             | if lower nibble is 0, goto HANDLE_1UP_2UP_BLINKING 

* A = column of aliens to animate (0-15)
206F: 21 20 41      ld   hl,0x4120            | load HL with pointer to bottommost row of aliens in ALIEN_SWARM_FLAGS
2072: 85            add  a,l
2073: 6F            ld   l,a                 | Now HL points to bottommost alien in the column
2074: 3A 38 42      ld   a,(0x4238)           | read DISABLE_SWARM_ANIMATION flag  
2077: 0F            rrca                     | move bit 0 into carry
2078: D8            ret  c                   | if flag was set, return

* animate each alien in the column
2079: 0E 10         ld   c,0x10               | sizeof a row of aliens in ALIEN_SWARM_FLAGS
207B: 06 06         ld   b,0x06               | number of rows of aliens to process. 
207D: C5            push bc
207E: E5            push hl
207F: 7D            ld   a,l
2080: CB 46         bit  0,(hl)              | test flag in ALIEN_SWARM_FLAGS to see if alien is present
2082: 20 05         jr   nz,0x2089            | if alien is present, goto 0x2089

2084: CD 5E 20      call 0x205E               | call DELETE_ALIEN_COMMAND to erase the alien from the screen
2087: 18 0B         jr   0x2094

2089: CD E1 20      call 0x20E1               | call GET_ALIEN_CHAR_RAM_ADDR
208C: 0E 00         ld   c,0x00               | load randomness value (see docs for CALCULATE_ALIEN_ANIMATION_FRAME_INDEX)
208E: CD 1D 21      call 0x211D               | call CALCULATE_ALIEN_ANIMATION_FRAME_INDEX
2091: CD 31 21      call 0x2131               | call DRAW_ALIEN
...
2099: 10 E2         djnz 0x207D               | repeat until all rows of aliens done
209B: C9            ret
...
209C: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY flag
209F: A7            and  a                   | test if zero
20A0: 28 05         jr   z,0x20A7             | if flag is not set, goto 0x20A7
20A2: 32 AB 40      ld   (0x40AB),a           | set CAN_BLINK_1UP_2UP flag 
20A5: 18 05         jr   0x20AC               | goto BLINK_1UP_or_2UP_TEXT


20A7: 3A AB 40      ld   a,(0x40AB)           | read CAN_BLINK_1UP_2UP
...
20AC: 3A 0D 40      ld   a,(0x400D)           | read CURRENT_PLAYER
20AF: CD 4E 21      call 0x214E               | get character RAM address for where 1UP or 2UP is printed
20B2: 11 E0 FF      ld   de,0xFFE0            | load DE with -32 decimal
20B5: CB 60         bit  4,b                 | test bit 4 of timing variable value
20B7: 28 14         jr   z,0x20CD             | if not set, then draw "1UP" or "2UP"

* erase "1UP" or "2UP" for current player
20B9: 3E 10         ld   a,0x10               | ordinal for empty space character
...
20C0: 3A 0E 40      ld   a,(0x400E)           | read IS_TWO_PLAYER_GAME 
20C3: A7            and  a                   | test flag
20C4: C8            ret  z                   | return if not a two-player game 
20C5: 3A 0D 40      ld   a,(0x400D)           | read CURRENT_PLAYER 
20C8: EE 01         xor  0x01                 | swap between player one and two (0= Player One, 1=Player 2)
20CA: CD 4E 21      call 0x214E               | get character RAM address for where 1UP or 2UP is printed

* plot "1UP" or "2UP" 
20CD: 3C            inc  a                   | A now is the ordinal for the "1" or "2" character 
20CE: 77            ld   (hl),a              | write "1" or "2"
20CF: 19            add  hl,de
20D0: 36 25         ld   (hl),0x25            | 'U'
20D2: 19            add  hl,de
20D3: 36 20         ld   (hl),0x20            | 'P'
20D5: CB 60         bit  4,b
20D7: C0            ret  nz
20D8: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY
20DB: A7            and  a                   
20DC: C0            ret  nz                  | if game is in play, return
20DD: 32 AB 40      ld   (0x40AB),a           | clear CAN_BLINK_1UP_2UP flag.
20E0: C9            ret
...
20E2: E6 0F         and  0x0F                 | now A is the column index of the alien (0-15). *Clears carry*
...
20EE: 78            ld   a,b                 | restore alien index from B (see @0x20E1)  
20EF: 0F            rrca                     | divide A..
20F0: 0F            rrca
20F1: 0F            rrca
20F2: 0F            rrca                     | by 16. Now A is a row index.
20F3: E6 07         and  0x07                 | Ensure A falls between 0 and 7. >7 is invalid. *also clears carry flag*  
...
20FC: 85            add  a,l
20FD: 6F            ld   l,a
20FE: 11 00 50      ld   de,0x5000            | start of character RAM
2101: 19            add  hl,de
2102: F1            pop  af
...
2106: FE 70         cp   0x70                 | is the index < 0x70 (112 decimal)
2108: 38 04         jr   c,0x210E             | yes, index < 0x70, goto 0x210E
210A: 06 80         ld   b,0x80               | this is a flagship row - set bit 7 so code @0x213F knows   
210C: F1            pop  af
210D: C9            ret

210E: E6 0F         and  0x0F                 | A = A MODULO 16. Now A = column of alien
2110: 47            ld   b,a                 | make B = column of alien
2111: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
2114: E6 0F         and  0x0F                 | mask in bits 0..3
2116: 0E 00         ld   c,0x00
2118: B8            cp   b
2119: 30 01         jr   nc,0x211C            | if A> B goto 0x211C
211B: 0D            dec  c                   |   
211C: F1            pop  af
...
211F: FE 70         cp   0x70                 | is the index >= 0x70 (112 decimal) ?
2121: 30 E7         jr   nc,0x210A            | yes, then this is a flagship row, they don't animate, goto 0x210A
2123: 3A 5F 42      ld   a,(0x425F)           | read TIMING_VARIABLE
...
212C: E6 03         and  0x03                 | ensure animation frame index is between 0..3
...
DRAW_ALIEN:
2131: EB            ex   de,hl
2132: 38 09         jr   c,0x213D             | if carry is set, we're drawing an odd row

2134: 21 57 21      ld   hl,0x2157            | load HL with pointer to ALIEN_SWARM_CHARACTERS_SET_1x2
2137: 78            ld   a,b                 | load A with animation frame index
2138: E7            rst  0x20                 | call routine to fetch value @ HL + A
2139: EB            ex   de,hl
213A: C3 A9 25      jp   0x25A9               | jump to PLOT_TWO_CHARACTERS_IN_SAME_COLUMN

* if we get here, we're drawing an "odd" row 
213D: 78            ld   a,b                 | load A with animation frame index
213E: A7            and  a                   | test if its zero
213F: F2 46 21      jp   p,0x2146             | if bit 7 is not set (see docs @ 0x210A) then its not a flagship we're drawing, goto 0x2146
2142: 3E A4         ld   a,0xA4               | starting character for flagship
2144: 18 04         jr   0x214A               | draw the flagship

2146: 21 5B 21      ld   hl,0x215B            | load HL with address of ALIEN_SWARM_CHARACTERS_SET_2x2 list 
2149: E7            rst  0x20                 | call routine to fetch value @ HL + A
214A: EB            ex   de,hl
214B: C3 85 25      jp   0x2585               | jump to PLOT_CHARACTERS_2_BY_2_ASCENDING
...
214E: 21 40 53      ld   hl,0x5340            | 1UP
2151: A7            and  a
2152: C8            ret  z
2153: 21 E0 50      ld   hl,0x50E0            | 2UP
2156: C9            ret
...
2160: 28 39         jr   z,0x219B             | if A is 0, goto DRAW_PLAYER_SHIP
2162: 3D            dec  a                  
2163: 28 22         jr   z,0x2187             | if A was 1 on entry, goto ERASE_PLAYER_SHIP
2165: 3D            dec  a      
...
216A: 2F            cpl
216B: E6 30         and  0x30
216D: C6 C0         add  a,0xC0
216F: 21 DA 51      ld   hl,0x51DA            | character RAM address for top right of explosion
2172: CD 85 25      call 0x2585               | call PLOT_CHARACTERS_2_BY_2_ASCENDING
2175: 21 DC 51      ld   hl,0x51DC            | character RAM address for bottom right of explosion
2178: CD 85 25      call 0x2585               | call PLOT_CHARACTERS_2_BY_2_ASCENDING
217B: 21 1A 52      ld   hl,0x521A            | character RAM address for top left of explosion
217E: CD 85 25      call 0x2585               | call PLOT_CHARACTERS_2_BY_2_ASCENDING
2181: 21 1C 52      ld   hl,0x521C            | character RAM address for bottom left of explosion
2184: C3 85 25      jp   0x2585               | call PLOT_CHARACTERS_2_BY_2_ASCENDING

ERASE_PLAYER_SHIP:
2187: 21 DA 51      ld   hl,0x51DA
218A: 11 1C 00      ld   de,0x001C            | offset to add to HL after drawing characters
218D: 0E 04         ld   c,0x04               | row counter: 4 characters high
218F: 06 04         ld   b,0x04               | column counter: 4 characters wide
2191: 36 40         ld   (hl),0x40            | ordinal for empty character
2193: 23            inc  hl                  | bump to next character on same column
2194: 10 FB         djnz 0x2191               | do until B==0
2196: 19            add  hl,de               | add offset to HL to point to row beneath
2197: 0D            dec  c                   | decrement row counter
2198: 20 F5         jr   nz,0x218F            | if row counter is not 0, goto 0x218F
219A: C9            ret

DRAW_PLAYER_SHIP:
219B: CD 87 21      call 0x2187               | erase existing ship or explosion
219E: 3E 60         ld   a,0x60               | ordinal of first character to plot
21A0: 21 FC 51      ld   hl,0x51FC            | character RAM address
21A3: C3 85 25      jp   0x2585               | jump to PLOT_CHARACTERS_2_BY_2_ASCENDING
...
21A7: CF            rst  0x08                 | assert that it's not GAME OVER 

21A8: CD 90 22      call 0x2290               | call LEA_DE_OF_CURRENT_PLAYER_SCORE. Now DE = pointer to current player's score
21AB: 79            ld   a,c                 | restore A (points value ID) from C
21AC: 81            add  a,c                 | And then multiply ..
21AD: 81            add  a,c                 | .. A by 3.
21AE: 4F            ld   c,a                   
21AF: 06 00         ld   b,0x00               | Extend A into BC. Now BC is an offset into ALIEN_SCORE_TABLE 
21B1: 21 D0 22      ld   hl,0x22D0            | pointer to ALIEN_SCORE_TABLE  
21B4: 09            add  hl,bc               | now HL points to an entry in the score table. 
21B5: A7            and  a                   | clear carry flag
21B6: 06 03         ld   b,0x03               | Players score is 3 bytes in size
...
21BE: 10 F8         djnz 0x21B8               | repeat until all digits in player score have been updated.
...
21CA: 7C            ld   a,h
21CB: 21 AC 40      ld   hl,0x40AC            | read BONUS_GALIXIP_FOR value                
21CE: BE            cp   (hl)                    
21CF: D4 9C 22      call nc,0x229C            | call AWARD_EXTRA_LIFE if our score is higher
21D2: 13            inc  de
21D3: 3A 0D 40      ld   a,(0x400D)           | read CURRENT_PLAYER  
21D6: CD 56 22      call 0x2256               | call DECIDE_TO_DISPLAY_PLAYER_ONE_OR_PLAYER_TWO_SCORE
21D9: D1            pop  de
21DA: 21 AA 40      ld   hl,0x40AA            | point to last byte (first 2 digits) of HI_SCORE
21DD: 06 03         ld   b,0x03               | Players score is 3 bytes in size
...
21E2: 20 05         jr   nz,0x21E9            | if byte is different, we have a new high score, goto 0x21E9
21E4: 1B            dec  de                  | otherwise, bump de 
21E5: 2B            dec  hl                  | and bump hl
21E6: 10 F7         djnz 0x21DF               | repeat until b==0
21E8: C9            ret


UPDATE_HIGH_SCORE:
21E9: CD 90 22      call 0x2290               | Now DE = pointer to current player score
21EC: 21 A8 40      ld   hl,0x40A8            | address of high score
21EF: 06 03         ld   b,0x03               | high score occupies 3 bytes
...
21F5: 10 FA         djnz 0x21F1
21F7: 1B            dec  de
21F8: DD 21 41 52   ld   ix,0x5241            | character RAM address where HIGH SCORE will be drawn
21FC: 18 63         jr   0x2261               | jump to PLOT_SCORE_CHARACTERS
...
RESET_SCORE_COMMAND:
21FE: FE 03         cp   0x03                
2200: 30 26         jr   nc,0x2228           | if A>= 3 then goto RESET_ALL_SCORES_AND_EXTRA_LIFE_FLAGS
2202: F5            push af
2203: 21 A2 40      ld   hl,0x40A2           | address of PLAYER_ONE_SCORE
2206: 11 AD 40      ld   de,0x40AD           | address of PLAYER_ONE_AWARDED_EXTRA_LIFE
2209: A7            and  a                  | test if A is 0
220A: 28 0E         jr   z,0x221A            | if A was 0 on entry, goto 0x221A and reset player 1's score

220C: 21 A5 40      ld   hl,0x40A5           | address of PLAYER_TWO_SCORE           
220F: 11 AE 40      ld   de,0x40AE           | address of PLAYER_TWO_AWARDED_EXTRA_LIFE 
2212: 3D            dec  a                  | if A was 1 on entry, then zero flag will be set after dec.
2213: 28 05         jr   z,0x221A            | if zero flag is set, then A was 1 on entry, goto 0x221A and reset player 2's score
2215: 21 A8 40      ld   hl,0x40A8           | address of HI_SCORE
2218: 5D            ld   e,l
2219: 54            ld   d,h                | DE = HL
221A: 36 00         ld   (hl),0x00           | clear first byte
221C: 23            inc  hl
221D: 36 00         ld   (hl),0x00           | clear second byte
221F: 23            inc  hl
2220: 36 00         ld   (hl),0x00           | clear third byte
2222: EB            ex   de,hl
2223: 36 00         ld   (hl),0x00           | reset the AWARDED_EXTRA_LIFE flag
2225: F1            pop  af
2226: 18 09         jr   0x2231              | Goto DISPLAY_SCORE_COMMAND to display reset scores.

RESET_ALL_SCORES_AND_EXTRA_LIFE_FLAGS:
2228: 3D            dec  a
2229: F5            push af
222A: CD FE 21      call 0x21FE
222D: F1            pop  af
222E: C8            ret  z
222F: 18 F7         jr   0x2228
...
2231: FE 03         cp   0x03
2233: 30 18         jr   nc,0x224D            | if A>= #0x03, goto 0x224D, DISPLAY_ALL_SCORES
2235: A7            and  a                   | test if A is 0
2236: 11 A4 40      ld   de,0x40A4            | pointer to last 2 BCD digits of PLAYER_ONE_SCORE
2239: 28 1B         jr   z,0x2256             | if A is 0, goto 0x2256 (DECIDE_TO_DISPLAY_PLAYER_ONE_OR_PLAYER_TWO_SCORE)
223B: 3D            dec  a                   | if A was 1 on entry, then Z flag is now set
223C: 20 0A         jr   nz,0x2248             
223E: 3A 0E 40      ld   a,(0x400E)           | read IS_TWO_PLAYER_GAME
2241: A7            and  a                   | test if zero
2242: C8            ret  z                   | if it's zero, then we're just in a single player game, return
2243: 11 A7 40      ld   de,0x40A7            | pointer to last 2 BCD digits of PLAYER_TWO_SCORE
2246: 18 0E         jr   0x2256               | goto DECIDE_TO_DISPLAY_PLAYER_ONE_OR_PLAYER_TWO_SCORE

DISPLAY_HIGH_SCORE:
2248: 11 AA 40      ld   de,0x40AA            | pointer to last 2 BCD digits of HI_SCORE
224B: 18 AB         jr   0x21F8

* This displays player scores and high scores
DISPLAY_ALL_SCORES:
224D: 3D            dec  a
224E: F5            push af
224F: CD 31 22      call 0x2231
2252: F1            pop  af
2253: C8            ret  z
2254: 18 F7         jr   0x224D
...
2256: DD 21 81 53   ld   ix,0x5381            | pointer to character RAM location for player one's score
225A: A7            and  a                   | test if A is 0.   
225B: 28 04         jr   z,0x2261             | if A is 0 then we want to draw player one's score, goto 0x2261
225D: DD 21 21 51   ld   ix,0x5121            | pointer to character RAM location for player two's score
...
2261: 21 E0 FF      ld   hl,0xFFE0            | load HL with 0xFFE0 (-32 decimal) 
2264: EB            ex   de,hl               | now HL = pointer to score bytes, DE = offset to add to screen address after plot     
2265: 06 03         ld   b,0x03               | a score is 3 bytes in size..                
2267: 0E 04         ld   c,0x04               | max number of leading zeros that can be skipped. For example,
...
226E: CD 79 22      call 0x2279               | call PLOT_LOWER_NIB_AS_DIGIT to plot the first digit                 
2271: 7E            ld   a,(hl)
2272: CD 79 22      call 0x2279               | call PLOT_LOWER_NIB_AS_DIGIT to plot the second digit 
2275: 2B            dec  hl                  | bump to *previous* BCD byte
2276: 10 F1         djnz 0x2269               | do until all BCD digits in score have been drawn
2278: C9            ret
...
2279: E6 0F         and  0x0F                 | mask in lower nibble
227B: 28 04         jr   z,0x2281             | if the lower nibble is zero, goto 0x2281

* OK, we have a nonzero digit. 
227D: 0E 00         ld   c,0x00               | tell the plot routine to draw all digits, even if they are zero, from now on
227F: 18 07         jr   0x2288               | go plot the character
...
2283: 28 03         jr   z,0x2288             | if we can't skip over any more leading zero digits, then goto 0x2288 to draw "0". 

* Otherwise, we are skipping a leading "0" digit and will print an empty space in its stead..
2285: 3E 80         ld   a,0x80               | when added to 0x90 this will produce 0x10 (16 decimal) - ordinal for empty character
2287: 0D            dec  c                   | decrement count of leading zeros we are allowed to ignore

2288: C6 90         add  a,0x90               | transform A into ordinal of character to be plotted
228A: DD 77 00      ld   (ix+0x00),a          | plot character for score to screen
...
2290: 11 A2 40      ld   de,0x40A2            | address of PLAYER_ONE_SCORE
2293: 3A 0D 40      ld   a,(0x400D)           | read CURRENT_PLAYER           
2296: A7            and  a                   | test if zero
2297: C8            ret  z                   | if it is zero, then current player is player one. Return.
2298: 11 A5 40      ld   de,0x40A5            | address of PLAYER_TWO_SCORE
229B: C9            ret
...
AWARD_EXTRA_LIFE:
229C: 3A 0D 40      ld   a,(0x400D)           | load CURRENT_PLAYER into A
229F: 21 AD 40      ld   hl,0x40AD            | load HL with address of PLAYER_ONE_AWARDED_EXTRA_LIFE flag.
22A2: 85            add  a,l
...
22A7: 36 01         ld   (hl),0x01            | Set "Player has had his extra life" flag
22A9: 3E 01         ld   a,0x01
22AB: 32 C7 41      ld   (0x41C7),a           | set PLAY_EXTRA_LIFE_SOUND flag.
22AE: 21 1D 42      ld   hl,0x421D            | pointer to PLAYER_LIVES
...
22B3: 21 9E 53      ld   hl,0x539E            | address in character RAM
22B6: 0E 05         ld   c,0x05
22B8: 3A 00 42      ld   a,(0x4200)           | read HAS_PLAYER_SPAWNED
22BB: A7            and  a                   | test if flag is set
22BC: 28 03         jr   z,0x22C1                       
22BE: 05            dec  b
22BF: 28 08         jr   z,0x22C9
22C1: 3E 66         ld   a,0x66
22C3: CD 93 25      call 0x2593               | plot 2X2 characters
22C6: 0D            dec  c
22C7: 10 F8         djnz 0x22C1
22C9: 0D            dec  c
22CA: F8            ret  m
22CB: CD 91 25      call 0x2591               | plot spaces to screen
22CE: 18 F9         jr   0x22C9



* Score values for aliens here....  see 0x21B1
...
PRINT_TEXT:
22F1: 21 5C 23      ld   hl,0x235C            | HL = address of TEXTPTRS  
22F4: 87            add  a,a                 | A = A * 2.   This may affect C and PO flags. 
22F5: F5            push af      
22F6: E6 3F         and  0x3F                 | mask in bits 0..5. Now A = a value in range of 0..63
22F8: 5F            ld   e,a     
22F9: 16 00         ld   d,0x00               | Extend A into DE 
22FB: 19            add  hl,de               | HL now points to an entry in the TEXTPTRS lookup table.
22FC: 5E            ld   e,(hl) 
22FD: 23            inc  hl
22FE: 56            ld   d,(hl)              | DE now holds a pointer to a character string to print. See docs @0x235C  
...
2305: 01 E0 FF      ld   bc,0xFFE0            | offset to add to HL after every character write. (-32 in decimal)
2308: F1            pop  af
2309: 38 0E         jr   c,0x2319
230B: FA 23 23      jp   m,0x2323             | if minus flag is set, then we want to scroll text onto screen - goto 0x2323

* HL = pointer to character RAM
* DE = pointer to character to write
230E: 1A            ld   a,(de)              | read character to be drawn   
230F: D6 30         sub  0x30                 
2311: FE 0F         cp   0x0F                 | is this the string terminator, #0x3F?
...
2317: 18 F5         jr   0x230E               | and continue
...
*
* Set text up for scrolling. Invoked by 0x230B within PRINT_TEXT
*
* HL = pointer to character RAM
* DE = pointer to text string to render
*

2323: 22 B5 40      ld   (0x40B5),hl          | store pointer to character RAM in COLUMN_SCROLL_CHAR_RAM_PTR
...
* Subsequent bytes: characters to print, terminated by #0x3F (63 decimal)
*
* For example, lets take the first entry in the table, 7E 23.
*
* 7E 23 forms memory address 0x237E. 

* Note: I suggest you open a memory window in the MAME debugger and view 237E, it'll make this a lot easier to follow.
*
* The first 2 bytes stored at 0x237E are 96 and 52. This forms a character RAM address of 0x5296, where the first character will be drawn.
* The subsequent bytes represent the string "GAME OVER" in (mostly) ASCII. The 0x3F after the "R" terminates the string.
*
...
23D0:  3F 98 53 42 4F 4E 55 53 40 47 41 4C 41 58 49 50  ?.SBONUS@GALIXIP
...
24B8: 28 66         jr   z,0x2520             | if parameter is 0, goto 0x2520 (DISPLAY_LEVEL_FLAGS)
24BA: 3D            dec  a
24BB: 28 2E         jr   z,0x24EB             | if parameter was 1 then goto 0x24EB (DISPLAY_AVAILABLE_CREDIT)
24BD: 3D            dec  a
24BE: 28 08         jr   z,0x24C8             | if parameter was 2 then goto 0x24C8 (DISPLAY_BONUS_GALIXIP_FOR) 
24C0: 3A 1D 42      ld   a,(0x421D)           | read number of lives for current player
24C3: 47            ld   b,a
24C4: CF            rst  0x08                 | assert that it's not GAME OVER (TODO: verify this)
24C5: C3 B3 22      jp   0x22B3               | goto DISPLAY_PLAYER_SHIPS_REMAINING
...
24C8: 3A AC 40      ld   a,(0x40AC)           | read BONUS GALIXIP FOR value
24CB: FE FF         cp   0xFF                 | check if there is any bonus. I think this code is redundant.
24CD: C8            ret  z                   | if no bonus, then return

24CE: 3E 06         ld   a,0x06               | index of BONUS GALIXIP FOR 0000 PTS text string
24D0: CD F1 22      call 0x22F1               | display text on screen
24D3: 3A AC 40      ld   a,(0x40AC)           | read BONUS GALIXIP value
24D6: E6 0F         and  0x0F                 | mask in low nibble
24D8: 32 38 51      ld   (0x5138),a           | write value to character RAM
24DB: 3A AC 40      ld   a,(0x40AC)           | read BONUS GALIXIP value
24DE: E6 F0         and  0xF0                 | mask in high nibble
24E0: 20 01         jr   nz,0x24E3            | if it's !=0, goto 0x24E3
24E2: 3C            inc  a
24E3: 0F            rrca                     | move high nibble...
24E4: 0F            rrca
24E5: 0F            rrca
24E6: 0F            rrca                     | ... into lower nibble, so that A is now a number from 0..9
24E7: 32 58 51      ld   (0x5158),a           | and POKE number to screen RAM, displaying single digit in correct place
...
24EB: 3A 06 40      ld   a,(0x4006)           | read IS_GAME_IN_PLAY flag           
24EE: 0F            rrca                     | move flag into carry
24EF: D8            ret  c                   | if the game is in play, return.
24F0: 3A 11 40      ld   a,(0x4011)           | read PORT_STATE_6800
24F3: E6 C0         and  0xC0                 | mask in dip switch 1 & 2 state
24F5: FE C0         cp   0xC0                 | are both dip switches on?
24F7: 3E 10         ld   a,0x10               | index of text string "FREE PLAY"
24F9: CA F1 22      jp   z,0x22F1             | call PRINT_TEXT

* if we get here, then we're not in FREE PLAY mode. We will display number of credits on screen.
24FC: 3E 05         ld   a,0x05               | index of text string "CREDIT"
24FE: CD F1 22      call 0x22F1               | call PRINT_TEXT
2501: 3A 02 40      ld   a,(0x4002)           | read number of credits
2504: FE 63         cp   0x63                 | compare to 99 decimal
2506: 38 02         jr   c,0x250A             | if A <99 then goto 0x250A
2508: 3E 63         ld   a,0x63               | clamp number of credits to 99
250A: CD 69 25      call 0x2569               | call CONVERT_A_TO_BCD. Now A = BCD equivalent of what it was
250D: 47            ld   b,a                 | save credits as BCD in B
250E: E6 F0         and  0xF0                 | mask in high nibble, which is first digit of BCD
2510: 28 07         jr   z,0x2519             | if the first digit is 0, goto 0x2519. We don't display it.
2512: 0F            rrca                     | shift high nibble...
2513: 0F            rrca
2514: 0F            rrca
2515: 0F            rrca                     | to low nibble.. converting first BCD digit to decimal.
2516: 32 9F 52      ld   (0x529F),a           | Write first digit of credits to character RAM
2519: 78            ld   a,b                 | get credits as BCD into A again. We preserved it in B @0x250D
251A: E6 0F         and  0x0F                 | mask in low nibble, which is second digit of BCD. Converts second BCD digit to decimal.
251C: 32 7F 52      ld   (0x527F),a           | Write second digit of credits to character RAM 
251F: C9            ret                      
...
DISPLAY_LEVEL_FLAGS:
2520: CF            rst  0x08                 | assert that it's not GAME OVER (TODO: verify this)
2521: 3A 20 42      ld   a,(0x4220)           | read HAVE_NO_ALIENS_IN_SWARM
2524: A7            and  a                   | test if flag is set
2525: 28 05         jr   z,0x252C             | if flag is not set, goto 0x252C
2527: 3E 01         ld   a,0x01
2529: 32 D0 41      ld   (0x41D0),a           | set RESET_SWARM_SOUND_TEMPO flag to 1. The swarm tempo will be slow again. 

252C: 3A 1C 42      ld   a,(0x421C)           | read PLAYER_LEVEL.
252F: 3C            inc  a                   | increment it.
2530: FE 30         cp   0x30                 | Compare to #0x30 (48 decimal)
2532: 38 02         jr   c,0x2536             | if A < 48, goto 0x2536
2534: 3E 30         ld   a,0x30               | Level 48 is the limit.
* A = level number (0-48)
2536: CD 69 25      call 0x2569               | convert A to BCD. Now A = BCD equivalent 

* A = level number in BCD            
2539: F5            push af
253A: 21 7E 50      ld   hl,0x507E            | address in character RAM to start drawing flags at
253D: E6 F0         and  0xF0                 | mask in high nibble
253F: 28 10         jr   z,0x2551             | if the high nibble is zero, then goto 0x2551
...
2546: 0E 10         ld   c,0x10               | C is a count of how much space, in characters, we have to plot flags. We start with #0x10 (16 decimal) 
2548: 3E 68         ld   a,0x68               | ordinal of first character of "10" flag to plot
254A: CD 85 25      call 0x2585               | plot the flag with "10" on it
254D: 0D            dec  c                   | A "10" flag takes up 2 spaces..
254E: 0D            dec  c                   | ..so reduce C by 2.
254F: 10 F7         djnz 0x2548               | repeat until B==0
2551: F1            pop  af

* Calculate how many normal red flags we are going to draw
* A=level number in BCD
2552: E6 0F         and  0x0F                 | mask in lower nibble of BCD number. Now A represents how many flags we are going to draw.
2554: 47            ld   b,a                 | B = number of flags to draw
2555: 11 1F 00      ld   de,0x001F            | offset to add to HL after every flag drawn.
2558: 28 08         jr   z,0x2562             | if we don't have any flags to draw then goto 0x2562
255A: 3E 6C         ld   a,0x6C               | ordinal of first character to plot
255C: CD A0 25      call 0x25A0               | draw the normal flag on character map
255F: 0D            dec  c                   | A normal flag takes up just 1 character space, so reduce C by 1
2560: 10 F8         djnz 0x255A               | repeat until B == 0
...
2564: CD 9E 25      call 0x259E               | plot spaces to overwrite any existing flags
2567: 18 F9         jr   0x2562
...
* A = BCD equivalent
* 
* Thanks to Slavo Labsky for his help deciphering the strange "add a,0x00" instruction, which resets the half-carry flag.  

CONVERT_A_TO_BCD:
2569: 47            ld   b,a                 | preserve A in B register
256A: E6 0F         and  0x0F                 | mask in low nibble
256C: C6 00         add  a,0x00               | clears the half carry flag which might affect DAA
256E: 27            daa
256F: 4F            ld   c,a                 | store result in C
2570: 78            ld   a,b                 | restore A to its original value
2571: E6 F0         and  0xF0                 | mask in high nibble
2573: 28 0B         jr   z,0x2580             | if high nibble is zero we don't care, goto 0x2580
...
257B: C6 16         add  a,0x16               | Add 16 hex (which in BCD terms is 16 decimal) to A  (so A will progress in BCD from 0->16->32->48... )
257D: 27            daa
257E: 10 FB         djnz 0x257B               | and repeat until B is 0.
2580: 81            add  a,c                 | add in value of lower nibble preserved @0x256f  
2581: 27            daa                      | ensure A is a valid BCD number
2582: C9            ret                      | and we're out



2583: 3E 2C         ld   a,0x2C               | space character
...
2585: D5            push de
2586: 11 1F 00      ld   de,0x001F            | load de with 31 decimal. This is the width of a row, in characters, minus 1.
2589: CD A0 25      call 0x25A0               | plot 2 characters on same row... 
258C: CD A0 25      call 0x25A0               | and 2 characters on the next row 
258F: D1            pop  de
2590: C9            ret
...
2591: 3E 2E         ld   a,0x2E
2593: D5            push de
2594: 11 DF FF      ld   de,0xFFDF            | load de with -33 decimal as signed word
2597: CD A0 25      call 0x25A0               | plot 2 characters on one row..
259A: C6 FC         add  a,0xFC               | subtract 4 from A
259C: 18 EE         jr   0x258C               | plot 2 characters on row above


259E: 3E 2C         ld   a,0x2C               | space character
...
25A7: 3E 2C         ld   a,0x2C               | space character
...
25A9: D5            push de
25AA: 11 20 00      ld   de,0x0020            | each row is comprised of 0x20 (32 decimal) characters...
25AD: 77            ld   (hl),a              | plot first character        
25AE: C6 02         add  a,0x02
